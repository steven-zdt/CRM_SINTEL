...
class CotizacionesValidator(BaseValidator):
    ...
    def validate(self, dto: dict[str, Any], document_type: str) -> tuple[bool, str | None, list[str]]:
        ...
        - Unidades de medida válidas
        - NO requiere campos de facturas
        ...
        if not (document_type.startswith("inventario") or document_type == "inventario.catalogo"):
            return False, "invalid_document_type", [f"Validador de cotizaciones no puede validar tipo: {document_type}"]
        ...
        for idx, item in enumerate(items):
            ...
            # Validar precio_venta (debe ser numérico válido, opcional pero si está presente debe ser válido)
            ...
            # Si no está presente, se usa 0.00 por defecto (no es error)
