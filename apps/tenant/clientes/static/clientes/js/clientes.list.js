...
(function(w, d) {
    ...
    async function loadClientesTable() {
        ...
        if (!gridElement || gridElement.children.length === 0) {
            ...
            // Destroy stale instance to avoid "Event Target Lookup Error"
            if (state.clientesTable) {
                try { state.clientesTable.destroy(); } catch (e) { /* ignore */ }
    ...
    async function loadContactosTable() {
        ...
        if (!gridElement || gridElement.children.length === 0) {
            ...
            if (state.contactosTable) {
                try { state.contactosTable.destroy(); } catch (e) { /* ignore */ }
    ...
    /**
     * Tab activated events (Custom Event from workspace.js)
     */
