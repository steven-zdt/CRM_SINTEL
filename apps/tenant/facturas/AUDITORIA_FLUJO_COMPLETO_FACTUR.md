...
- OK **Idempotencia y Silent Success**: Importacion por CUFE/CUDE, pre-validacion con `fast_get_cufe`, y actualizacion de anexos si el XML es mas completo.
- OK **Inmutabilidad y Snapshot Pattern**: Facturas y notas credito son historicas, sin edicion, solo eliminacion permitida; snapshot de emisor/receptor.
- OK **Multi-tenant y Zero Trust**: Aislamiento por esquema, validacion de empresa y pertenencia en todos los flujos.
- OK **Frontend modular (namespace `Sintel.Factura`)**: JS modular en `static/facturas/js/factura_*.js`, tab-activated redraw, Offcanvas dedicados, FSD completo.
...
|-- models.py                     # Modelos principales (Factura, ItemFactura, NotaCredito, FacturaAnexos, MailIngestionRun, MailInboxState)
|-- inbox_state.py                # Gestion de estado IMAP (procesamiento incremental)
|-- tasks.py                      # Tareas Celery (ingesta asincrona)
|-- urls.py                       # URLs raiz de la app
|
|-- services/                     # Service Layer (paquete v2.61.8)
|   |-- __init__.py               # Reexporta clases principales
...
|   |-- api_mixins.py             # ServiceMixin para ViewSets
|   +-- services_mail_ingestion.py # Servicio de ingesta por correo
|
|-- utils/                        # Utilidades de parsing
|   +-- ubl_parser.py             # Parser UBL 2.1 (extraccion de datos desde XML)
|
|-- api/                          # Modulo de APIs REST
|   |-- __init__.py
|   |-- viewsets.py               # ViewSets DRF (FacturaViewSet, NotaCreditoViewSet)
|   |-- serializers.py            # Serializers DRF (FacturaListSerializer, FacturaDetailSerializer, etc.)
|   |-- urls.py                   # URLs de API
...
|
|-- static/facturas/js/           # JavaScript modular (namespace: window.Sintel.Factura)
|   |-- factura_utils.js          # Formatters puros, badges, helpers (sin deps)
|   |-- factura_api.js            # SSoT de URLs y endpoints directos
|   |-- factura_table.js          # Tabulator config, columnas, init, refresh
|   |-- factura_ui.js             # Offcanvas, formularios, DOM Shield
|   |-- factura_sync.js           # Sincronizacion correo, eventos, feedback UI
|   +-- factura_main.js           # FacturaController orquestador + tab-activated
|
|-- templates/tenant/facturas/    # Templates HTML (FSD)
|   |-- list_factura.html         # Listado principal con #grid-facturas
|   |-- offcanvas_crear_factura.html
|   |-- offcanvas_editar_factura.html
|   |-- offcanvas_detalle_factura.html
|   |-- offcanvas_importar_factura.html
|   |-- offcanvas_pendientes_factura.html
|   +-- partials/
|       |-- assets_facturas.html  # Carga de scripts JS (orden critico)
|       |-- table.html
|       |-- offcanvas_factura.html
|       +-- offcanvas_ver_factura.html
|
...
3. Template: tenant/core/workspace.html
   ...
   └── Carga JavaScript modular (factura_*.js bajo namespace Sintel.Factura)
   ↓
4. JavaScript: factura_main.js (FacturaController) se inicializa en DOMContentLoaded
   ├── Monta sub-modulos: utils, api, table, ui, sync
   ├── Inicializa Tabulator via TabulatorFactory.create('#grid-facturas', '/api/v1/facturas/')
   ├── Registra listener tab-activated para redraw al activar tab
   └── Configura listeners HTMX y toolbar
   ↓
5. workspace.js: showTab('facturas') -> display:block -> dispatch tab-activated
   ├── Tabulator redraw(true) -> recalcula layout con dimensiones reales
   └── API call: GET /api/v1/facturas/?page=1&page_size=10
   ↓
6. Gateway Directo: config/api_urls.py -> apps.tenant.facturas.api.urls
   ├── GET /api/v1/facturas/ (list)
   ├── GET /api/v1/facturas/{uuid}/ (retrieve)
   ├── POST /api/v1/facturas/upload-ubl/ (upload UBL)
   ├── GET /api/v1/facturas/summary/ (resumen)
   └── DELETE /api/v1/facturas/{uuid}/ (delete)
   ↓
7. App API: apps/tenant/facturas/api/viewsets.py
   ├── FacturaViewSet (hereda BaseTenantViewSet)
   └── NotaCreditoViewSet
   ↓
8. Serializers: apps/tenant/facturas/api/serializers.py
   ├── FacturaListSerializer (lista optimizada)
   └── FacturaDetailSerializer (detalle completo)
   ↓
9. Service Layer: apps/tenant/facturas/services/
   ├── selectors.py -> qs_list(), qs_detail() con only() + select_related
   ├── business_service.py -> guardar_factura_desde_dto(), importar_documento()
   └── crud_service.py -> persistencia transaccional
   ↓
10. Parser: utils/ubl_parser.py + apps/services/document_parser/xml_parser/parser.py
    ├── parse_ubl_to_dict() -- DTO canonico desde XML
    └── fast_get_cufe() -- pre-validacion idempotencia
    ↓
11. Models: apps/tenant/facturas/models.py
    ├── Factura.save() -> ItemFactura.save() x N
    └── FacturaAnexos (XML almacenado)
    ↓
12. Database: PostgreSQL (multi-tenant, schema-per-tenant)
```
...
#### Estructura del Modulo Facturas

```html
<section id="tab-facturas" class="workspace-tab" style="display: none;">
  {% include 'tenant/facturas/list_factura.html' %}
</section>
```
...
**Orden de Carga Critico (definido en `assets_facturas.html`):**
1. `factura_utils.js` - Formatters puros, badges (Sintel.Factura.utils)
2. `factura_api.js` - SSoT de URLs y endpoints directos (Sintel.Factura.api)
3. `factura_table.js` - Tabulator config, columnas, init (Sintel.Factura.table)
4. `factura_ui.js` - Offcanvas, formularios, DOM Shield (Sintel.Factura.ui)
5. `factura_sync.js` - Sincronizacion correo, eventos (Sintel.Factura.sync)
6. `factura_main.js` - FacturaController orquestador (window.Sintel.Factura)
...
### v2.61.8 -- Correccion Pipeline XML + Frontend Tabulator (2026-04-04)

**Backend (`services/business_service.py`):**
- OK Fix gap de persistencia: `guardar_factura_desde_dto()` no guardaba la factura completa porque el DTO canonico usaba campos distintos a los del modelo. Mapeados: `invoice_number` -> `numero`, `issue_date` -> `fecha_emision`, `due_date` -> `fecha_vencimiento`, `supplier.*` -> `emisor_*`, `customer.*` -> `receptor_*`, `line_extension_amount` -> `subtotal`, `tax_amount` -> `impuestos`, `payable_amount` -> `total`.
- OK Fix lookup de Empresa: se buscaba `Empresa.objects.get(nit=...)` pero Empresa no tiene campo `nit`; corregido a buscar por `empresa_id` inyectado desde el ViewSet (Zero Trust SSoT).
- OK Fix campo `cufe`: el DTO del pipeline universal usa `uuid` (no `cufe`); mapeado correctamente para la clave de idempotencia.

**Backend (`apps/services/document_parser/xml_parser/parser.py`):**
- OK Fix extraccion de items (InvoiceLine): `_parse_invoice_ubl21()` no extraia `items` porque iteraba sobre `cac:InvoiceLine` sin considerar namespaces. Corregido con iteracion `local-name()` y extraccion robusta de campos por item.

**Backend (`utils/ubl_parser.py`):**
- OK Fix sobreescritura de CUFE: `importar_factura_desde_ubl()` llamaba a `crear_factura()` (inexistente desde Service Layer refactor); corregido a `guardar_factura_desde_dto()`.
- OK Fix soporte CreditNote/DebitNote: `parse_ubl_to_dict()` ahora detecta root tag `CreditNote`/`DebitNote` ademas de `Invoice`, extrayendo CUDE correctamente.
- OK Fix: eliminado overwrite de CUFE con valor de CreditNote padre cuando se procesa AttachedDocument que contiene Invoice embebido.

**Frontend (`factura_main.js`):**
- OK Fix tabla vacia al navegar `#facturas`: Tabulator se inicializaba en DOMContentLoaded mientras `#tab-facturas` tenia `display: none`, causando que columnas se calcularan con ancho 0. Agregado listener `tab-activated` que ejecuta `tbl.redraw(true)` al activar el tab (patron consistente con `clientes.list.js` y `proveedores_main.js`).

**Frontend (`factura_utils.js`):**
- OK Fix namespace mismatch: el export usaba `w.Sintel.Facturas.utils` (plural) pero todos los consumidores (table, api, sync, main) leian desde `w.Sintel.Factura.utils` (singular). Corregido a singular `Sintel.Factura`.

**Frontend (`factura_ui.js`):**
- OK Fix referencia rota a utils: `actualizarTotalesDOM()` usaba `w.Sintel.Facturas.utils` (plural); corregido a `w.Sintel.Factura.utils` (singular, alineado con namespace real).

---

### v2.61.3 --- Alineacion Backend-Frontend (2026-03-12)
...
|---------|-----|-------|-----|
| v2.61.8 | Tabla facturas vacia al navegar a `#facturas` | Tabulator init en `DOMContentLoaded` con `#tab-facturas` en `display:none`; columnas calculadas con ancho 0 | Agregado listener `tab-activated` en `factura_main.js` que llama `tbl.redraw(true)` al activar tab |
| v2.61.8 | `TypeError: utils.badgeNaturaleza is not a function` | `factura_utils.js` exportaba a `Sintel.Facturas.utils` (plural) pero `factura_table.js` leia de `Sintel.Factura.utils` (singular) | Corregido namespace a `Sintel.Factura.utils` en export y en `factura_ui.js` |
| v2.61.8 | Pipeline XML no persistia facturas (campos vacios) | `business_service.py` usaba nombres de campo del modelo pero DTO canonico usa nombres distintos (`invoice_number`, `supplier.*`) | Mapeado completo DTO canonico a campos modelo Factura |
| v2.61.8 | `Empresa.objects.get(nit=...)` fallaba | Modelo Empresa no tiene campo `nit`; business_service buscaba por NIT | Corregido a usar `empresa_id` del ViewSet (Zero Trust SSoT) |
| v2.61.8 | Items de factura no se extraian del XML | `parser.py` iteraba `cac:InvoiceLine` sin manejar namespaces lxml | Corregido con iteracion `local-name()` y extraccion robusta |
| v2.61.8 | CUFE sobreescrito en AttachedDocument | `ubl_parser.py` tomaba UUID del CreditNote padre, no del Invoice embebido | Eliminado overwrite; preserva CUFE del Invoice interno |
| v2.61.8 | `ubl_parser.py` llamaba a `crear_factura()` inexistente | Funcion eliminada en refactor Service Layer | Corregido a `guardar_factura_desde_dto()` |
| v2.61.3 | `UnboundLocalError: cannot access local variable 're'` | `import re` dentro de `_parse_invoice_ubl21()` marcaba `re` como local | Eliminados `import re` locales (líneas 270 y 316); usa `re` global del módulo |
