...
y NO devuelva 401 cuando el usuario está autenticado con sesión.

Valida que:
1. El endpoint responda 200 cuando el usuario está autenticado con sesión
2. El endpoint NO devuelva 401 si SessionAuthentication está configurado
...
def test_gastos_list_session_ok(client, django_user_model, tenant):
    """
    ...
    r = client.get("/api/v1/gastos/", HTTP_HOST=f"{tenant.schema_name}.sintel.com")
