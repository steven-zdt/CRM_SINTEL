...
class ResolucionDIANNestedSerializer(serializers.ModelSerializer):
    ...
    WARNING: CORRECCIÓN: Asegura que las fechas se serialicen como cadenas ISO.
    WARNING: CORRECCIÓN: Campos de fecha explícitamente declarados para evitar errores de utcoffset.
    WARNING: ALINEADO: Incluye todos los campos del modelo ResolucionDIAN.
    """
    # WARNING: CORRECCIÓN: DateField explícitos (no DateTimeField) porque el modelo usa DateField
    ...
    # WARNING: CORRECCIÓN: DateTimeField explícitos para created_at y updated_at
...
class ResolucionDIANListSerializer(serializers.ModelSerializer):
    ...
    WARNING: REGLA DE EXPOSICIÓN: Mínimo payload, máximo rendimiento.
    """
...
class ResolucionDIANCreateSerializer(serializers.ModelSerializer):
    ...
    WARNING: CORRECCIÓN: Maneja el valor "on" de los checkboxes HTML.
    WARNING: INMUTABILIDAD: Las resoluciones son documentos legales y no deben editarse después de creadas.
    """
    ...
    def validate_vigente(self, value):
        """
        WARNING: CORRECCIÓN v2.60: Maneja el valor "on" de los checkboxes HTML.
...
class ResolucionDIANDetailSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        WARNING: CORRECCIÓN: DRF serializa automáticamente DateField a cadenas ISO,
...
class GastoListSerializer(serializers.ModelSerializer):
    ...
    ds_numero_documento = serializers.CharField(source='documento_soporte.numero_documento', read_only=True)  # WARNING: v2.40: Prefijo + Consecutivo completo
    ds_vendedor = serializers.CharField(source='documento_soporte.vendedor_nombre', read_only=True)
    # WARNING: CORRECCIÓN: DateField (no DateTimeField) porque DocumentoSoporte.fecha es DateField
    ...
    ds_activo = serializers.BooleanField(source='documento_soporte.activo', read_only=True)  # WARNING: v2.40: Campo activo
    ds_anulado = serializers.BooleanField(source='documento_soporte.anulado', read_only=True)
    
    # Campos display para choices (texto legible para el frontend)
    ...
    codigo_contable_display = serializers.CharField(source='get_codigo_contable_display', read_only=True)
...
class DocumentoSoporteDetailSerializer(serializers.ModelSerializer):
    ...
    WARNING: ALINEADO: Incluye todos los campos del modelo DocumentoSoporte.
    WARNING: CORRECCIÓN: Campos de fecha explícitamente declarados para evitar errores de utcoffset.
    """
    ...
    # WARNING: CORRECCIÓN: DateField explícito (no DateTimeField) porque el modelo es DateField
    fecha = serializers.DateField(format='%Y-%m-%d', required=False, read_only=True)
    # WARNING: CORRECCIÓN: DateTimeField explícito para fecha_anulacion (es DateTimeField en el modelo)
    ...
    class Meta:
        ...
        fields = (
            ...
            'subtotal',
            'retefuente_porcentaje',  # WARNING: v2.40: Porcentaje seleccionado de Retefuente
            'retefuente',  # WARNING: v2.40: Valor calculado de Retefuente
            'reteica_porcentaje',  # WARNING: v2.40: Porcentaje seleccionado de ReteICA
            'reteica',  # WARNING: v2.40: Valor calculado de ReteICA
            'total',
            'adjunto',
            'activo',  # WARNING: v2.40: Campo activo
            'anulado',
            'fecha_anulacion',
            'created_at',
            'updated_at',
        )
        # WARNING: v2.40: Campos monetarios y legales son INMUTABLES
        read_only_fields = (
...
class GastoDetailSerializer(serializers.ModelSerializer):
    ...
    WARNING: ALINEADO: Incluye todos los campos del modelo Gasto.
    WARNING: CORRECCIÓN: Campos DateTimeField explícitamente declarados.
    WARNING: Campos display para choices (texto legible para el frontend).
    """
    ...
    def validate_codigo_contable(self, value):
        """
        Valida que el codigo_contable pertenezca a GASTOS_NIIF_CHOICES y aplica strip().
        Es opcional: si viene vacio o None se acepta sin validar.
        """
        if not value:
            return value
        from apps.tenant.gastos.choices.niif_gastos_choices import GASTOS_NIIF_CODIGOS_VALIDOS
        cleaned = str(value).strip()
        if cleaned and cleaned not in GASTOS_NIIF_CODIGOS_VALIDOS:
            raise serializers.ValidationError(
                _("Codigo contable '%s' no pertenece al catalogo NIIF permitido.") % cleaned
            )
