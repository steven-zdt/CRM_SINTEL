...
⚠️ Arquitectura v2.40: Alineado con enfoque API-First y Multi-Tenant.
...
# ⚠️ IMPORTANTE: No incluir el prefijo 'proyectos/' aquí. 
...
# ⚠️ DEBUG: Validación de inicialización en el arranque del servidor
if urlpatterns:
    logger.info(f"✅ ProyectoViewSet (v3.3) registrado correctamente. URLs generadas: {len(urlpatterns)}")
    for url_pattern in urlpatterns:
        logger.debug(f"   - {url_pattern.pattern} -> {getattr(url_pattern, 'name', 'N/A')}")
else:
    logger.error("❌ ERROR CRÍTICO: El router de Proyectos no generó ninguna URL. Verifique el ViewSet.")