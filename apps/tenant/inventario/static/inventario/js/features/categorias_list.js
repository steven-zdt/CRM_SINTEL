...
(function(w, d) {
    ...
    let _listEventsInit = false; // Flag para prevenir registro doble de eventos del grid
    ...
    if (w.Sintel.Inventario.Tables.categorias) {
        if (w.Sintel.Inventario.Tables.categorias && typeof w.Sintel.Inventario.Tables.categorias.destroy === 'function') {
            try {
                w.Sintel.Inventario.Tables.categorias.destroy();
            } catch (error) {
    ...
    function initTable() {
        ...
        w.Sintel.Inventario.Tables.categorias = table;
    ...
    function initListEvents() {
        // Guard: prevenir registro doble de eventos en el grid
        if (_listEventsInit) {
            console.log(`${MOD} Eventos del grid ya registrados, omitiendo...`);
            return;
        }
        _listEventsInit = true;

        const gridElement = d.querySelector(GRID_ID);
        if (!gridElement) {
            console.warn(`${MOD} Elemento ${GRID_ID} no encontrado para eventos`);
            _listEventsInit = false; // Resetear si falla
            return;
        }
        ...
        gridElement.addEventListener('click', async (e) => {
            ...
            if (btnDelete) {
                ...
                try {
                    ...
                    } else if (w.Sintel?.Inventario?.API?.categorias && typeof w.Sintel.Inventario.API.categorias.delete === 'function') {
                        deleteRes = await w.Sintel.Inventario.API.categorias.delete(id);
                    } else {
