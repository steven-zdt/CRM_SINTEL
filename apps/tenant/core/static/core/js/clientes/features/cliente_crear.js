/**
 * Feature: Crear Cliente v2.61
 * ⚠️ Feature-Sliced Architecture: Módulo específico para creación de clientes
...
(function(w, d) {
    ...
    /**
     * Recolectar datos del formulario de creación
     * @returns {Object} Datos del cliente y array de contactos
     */
    function recolectarDatosFormulario() {
        const form = d.querySelector('#form-cliente-crear');
        if (!form) {
            console.error('[cliente.crear] Formulario no encontrado');
            return null;
        }
    ...
    async function guardarCliente() {
        ...
        const response = await w.clientesAPI.create(payload);
        
        if (response && response.id) {
            // Éxito
            if (w.SintelFeedback) {
                w.SintelFeedback.success('Cliente creado exitosamente');
            }
            ...
            // ⚠️ Redirigir a workspace/#clientes después de un breve delay
            setTimeout(() => {
                window.location.href = '/workspace/#clientes';
            }, 500);
        } else {
    ...
    /**
     * Inicializar eventos del formulario de creación
     */
    function initFormularioCrear() {
        const offcanvasEl = d.getElementById('offcanvas-cliente-crear');
        if (!offcanvasEl) {
            console.warn('[cliente.crear] Offcanvas no encontrado');
            return;
        }
        ...
        console.log('[cliente.crear] Formulario de creación inicializado');
    }

    /**
     * Escuchar evento de inicialización desde el template
     */
    d.addEventListener('initClienteCrear', function() {
        console.log('[cliente.crear] Evento initClienteCrear recibido');
        initFormularioCrear();
    });

    // Exponer API pública (opcional)
    w.ClienteCrearModule = {
        init: initFormularioCrear
    };
