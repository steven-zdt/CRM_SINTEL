 ...
 * - w.CategoriasList (definido en categorias_list.js) - Feature: Listado (para recargar tabla)
 */
(function(w, d) {
    'use strict';
    ...
    async function guardarCategoria(e) {
        ...
        try {
            ...
            if (w.SintelFeedback && typeof w.SintelFeedback.success === 'function') {
                ...
                w.SintelFeedback.success(`Categoría ${accion} correctamente`);
            }
            ...
            if (w.CategoriasList && typeof w.CategoriasList.recargar === 'function') {
                w.CategoriasList.recargar();
            } else if (window.SintelInventarioTables && window.SintelInventarioTables.categorias) {
                window.SintelInventarioTables.categorias.replaceData();
            }
    ...
    // ⚠️ v2.61.3: Solo usar htmx:afterSwap, no shown.bs.offcanvas (evita duplicación)
            ...
            if (event.detail.target.id === 'offcanvas-container-categorias') {
                ...
                setTimeout(function() {
                    ...
                    if (offcanvasEl) {
                        ...
                        // ⚠️ Limpiar flag data-init antes de inicializar (nuevo contenido)
                        const form = d.querySelector(FORM_ID);
                        if (form) {
    ...
    // ⚠️ Bootstrap: Solo inicializar si no se inicializó vía HTMX
    // ⚠️ v2.61.3: Usar once: true para evitar múltiples listeners
    if (typeof bootstrap !== 'undefined' && bootstrap.Offcanvas) {
        const offcanvasEl = d.querySelector(OFFCANVAS_ID);
        if (offcanvasEl) {
            offcanvasEl.addEventListener('shown.bs.offcanvas', function() {
                const form = d.querySelector(FORM_ID);
                // Solo inicializar si no tiene data-init (no se inicializó vía HTMX)
                if (form && !form.hasAttribute('data-init')) {
                    console.log(`${MOD} Offcanvas mostrado (sin HTMX), inicializando editor...`);
                    init();
                }
            }, { once: false }); // Mantener listener para múltiples aperturas
        }
    }
