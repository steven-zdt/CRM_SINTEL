...
(function(w, d) {
    ...
    if (window.SintelInventarioTables && window.SintelInventarioTables.main) {
        if (window.SintelInventarioTables.main && typeof window.SintelInventarioTables.main.destroy === 'function') {
            try {
    ...
    async function verKardex(productoId) {
        ...
        try {
            ...
            } else if (w.inventarioAPI && w.inventarioAPI.productos) {
                productoRes = await w.inventarioAPI.productos.get(productoId);
                kardexRes = await w.inventarioAPI.productos.kardex(productoId);
            } else {
    ...
    if (d.readyState === 'loading') {
        ...
        const tabProductos = d.querySelector('#tab-existencias');
