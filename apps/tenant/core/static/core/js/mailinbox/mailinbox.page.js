...
(function (w, d) {
    ...
    return [
      ...
      },
      {
        ...
        formatter: function(cell) {
          ...
          // Botón Probar Conexión (v2.60: Se prueba desde el offcanvas al editar)
          buttons += `
            <button class="btn btn-sm btn-link text-info p-0 me-2" onclick="if(window.mailinboxOffcanvas){window.mailinboxOffcanvas.abrirEditar(${id});}" title="Editar y Probar Conexión">
  ...
  async function probarConexion(id) {
    ...
    // Abrir modal para ingresar password (por seguridad, no se expone desde el backend)
    const modal = d.querySelector('#modal-mailinbox-form');
    if (modal) {
      // Si el modal está abierto y tiene datos, usar esos datos
      const idInput = d.getElementById('mailinbox-id');
      const currentId = idInput ? idInput.value : null;
      
      if (currentId == id) {
        // El modal ya está abierto con esta configuración, usar datos del formulario
        const testData = {
          host: d.getElementById('mailinbox-imap_host')?.value || '',
          port: parseInt(d.getElementById('mailinbox-imap_port')?.value || '993', 10),
          protocol: 'imap',
          username: d.getElementById('mailinbox-imap_username')?.value || '',
          password: d.getElementById('mailinbox-imap_password')?.value || '',
          use_ssl: d.getElementById('mailinbox-imap_ssl')?.checked || false,
          use_starttls: d.getElementById('mailinbox-imap_starttls')?.checked || false
        };
        
        if (!testData.password) {
          if (w.SintelFeedback) {
            w.SintelFeedback.warning('Por favor ingrese la contraseña en el formulario para probar la conexión');
          }
          return;
        }
        
        await ejecutarTestConexion(testData);
      } else {
        // Cargar datos de la configuración y pedir password
        if (w.SintelFeedback) {
          w.SintelFeedback.info('Por favor abra el formulario de edición e ingrese la contraseña para probar la conexión');
        }
      }
    } else {
        if (w.SintelFeedback) {
