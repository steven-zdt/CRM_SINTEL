...
def parse_html_to_chunks(html_bytes: bytes | str) -> dict:
    ...
    try:
        ...
        paragraphs_text = [''.join(p.xpath('.//text()')).strip() for p in paragraphs if ''.join(p.xpath('.//text()')).strip()]
        ...
        if HAS_BS4:
            ...
            paragraphs_text = [p.get_text(strip=True) for p in soup.select("p") if p.get_text(strip=True)]
