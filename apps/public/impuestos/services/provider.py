...
WARNING: POLÍTICA SSoT: Este servicio es la ÚNICA fuente de datos de normativa DIAN
...
def get_regimen_renta_by_codigo(codigo: str) -> dict[str, Any]:
    ...
    WARNING: POLÍTICA SSoT: Recomendado para composición en TENANT_APPS (contabilidad, facturas).
...
@lru_cache(maxsize=256)
def get_contribuyentes_tipos_by_clase(clase: str | None = None) -> list[dict[str, Any]]:
    """
    Obtiene tipos de contribuyente filtrados por clase (PN/PJ) o todos.

    WARNING: POLÍTICA SSoT: Recomendado para composición en TENANT_APPS.
...
@lru_cache(maxsize=256)
def get_responsabilidades_rut(codigos: Iterable[str] | None = None) -> list[dict[str, Any]]:
    """
    Obtiene responsabilidades RUT (todas activas o filtradas por código).

    WARNING: POLÍTICA SSoT: Útil para decidir IVA, retenciones y facturación en TENANT_APPS.
...
def get_perfil_tributario(nombre: str) -> dict[str, Any]:
    ...
    WARNING: POLÍTICA SSoT: Útil para onboarding o validaciones coherentes en TENANT_APPS.
    ...
    """
    p = (
        PerfilTributario.objects.select_related("tipo_contribuyente", "regimen_renta")
        .prefetch_related("responsabilidades")
        .filter(nombre=nombre, activo=True)
        .first()
    )
    if not p:
...
def get_tipo_contribuyente_options() -> list[dict[str, Any]]:
    ...
    return [{"codigo": codigo, "nombre": nombre} for codigo, nombre in get_clase_choices()]
...
def get_responsabilidades_rut_options() -> list[dict[str, Any]]:
    ...
    codigos_en_bd = {
        ...
        for r in ResponsabilidadRUT.objects.filter(activo=True).values(
            "codigo", "es_responsable_iva", "es_no_responsable_iva"
        )
    }
    ...
    return [
        {
            ...
            "es_no_responsable_iva": codigos_en_bd.get(codigo, {}).get(
                "es_no_responsable_iva", False
            ),
        }
...
def search_actividades_economicas(q: str, limit: int = 10) -> list[dict[str, Any]]:
    ...
    return [
        {"codigo": codigo, "nombre": nombre.split(" — ", 1)[1] if " — " in nombre else nombre}
        for codigo, nombre in choices
    ]
...
def get_empresa_form_metadata() -> dict[str, Any]:
    ...
    WARNING: POLÍTICA SSoT: Consume exclusivamente desde catálogos públicos.
    ...
    segmentos_dian_flat = [
        {"value": codigo, "label": nombre} for codigo, nombre in get_segmento_dian_choices()
    ]
    ...
    return {
        ...
        "regimenes_renta": [
            {"codigo": codigo, "nombre": nombre} for codigo, nombre in get_regimen_renta_choices()
        ],
...
def clear_impuestos_cache():
    ...
    WARNING: IMPORTANTE: Llamar desde señales o admin después de modificar catálogos
