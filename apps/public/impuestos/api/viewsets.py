...
⚠️ IMPORTANTE: Todos los ViewSets son ReadOnlyModelViewSet porque
...
class BaseReadWrite(viewsets.ModelViewSet):
    ...
    ⚠️ POLÍTICA SSoT: GET abierto (AllowAny), POST/PUT/PATCH/DELETE solo staff.
    """
    pagination_class = StandardResultsSetPagination
    filter_backends = [DjangoFilterBackend, SearchFilter, OrderingFilter]
    ordering_fields = '__all__'
...
class ContribuyenteTipoViewSet(BaseReadWrite):
    ...
    ⚠️ POLÍTICA SSoT: GET abierto, POST/PUT/PATCH/DELETE solo staff.
    """
...
class ResponsabilidadRUTViewSet(BaseReadWrite):
    ...
    filterset_fields = [
        'activo', 'codigo', 'es_responsable_iva', 'es_no_responsable_iva',
        'es_simple', 'es_facturador_electronico', 'es_gran_contribuyente',
...
class PerfilTributarioViewSet(BaseReadWrite):
    ...
    ⚠️ POLÍTICA SSoT: GET abierto, POST/PUT/PATCH/DELETE solo staff.
    """
    queryset = PerfilTributario.objects.prefetch_related('responsabilidades').select_related(
        'tipo_contribuyente', 'regimen_renta'
    ).all()
    serializer_class = PerfilTributarioSerializer
    filterset_fields = [
        'activo', 'tipo_contribuyente', 'regimen_renta',
        'recomendado_desde', 'recomendado_hasta'
    ]
