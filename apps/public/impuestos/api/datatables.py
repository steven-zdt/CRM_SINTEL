...
class TipoImpuestoDataTablesView(BaseDataTablesView):
    ...
    def get_filtered_queryset(self, queryset, search_value):
        if search_value:
            return (
                queryset.filter(codigo__icontains=search_value)
                | queryset.filter(nombre__icontains=search_value)
                | queryset.filter(descripcion__icontains=search_value)
            )
        return queryset
...
class TarifaIVADataTablesView(BaseDataTablesView):
    ...
    def get_filtered_queryset(self, queryset, search_value):
        if search_value:
            return queryset.filter(codigo__icontains=search_value) | queryset.filter(
...
class ConceptoRetencionDataTablesView(BaseDataTablesView):
    ...
    def format_row(self, obj):
        return [
            ...
            obj.codigo,
            obj.nombre,
            obj.get_tipo_retencion_display()
            if hasattr(obj, "get_tipo_retencion_display")
            else obj.tipo_retencion,
            "✓" if obj.activo else "✗",
        ]
    ...
    def get_filtered_queryset(self, queryset, search_value):
        if search_value:
            return (
                queryset.filter(codigo__icontains=search_value)
                | queryset.filter(nombre__icontains=search_value)
                | queryset.filter(tipo__icontains=search_value)
            )
        return queryset
