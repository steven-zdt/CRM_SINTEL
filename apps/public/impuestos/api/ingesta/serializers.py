...
class DocumentoFuenteCreateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if not file and not url:
            raise serializers.ValidationError("Debes proporcionar un archivo o una URL.")

        if file and url:
            raise serializers.ValidationError("Proporciona archivo o URL, no ambos.")
        ...
        if file:
            ...
            if size_mb > MAX_SIZE_MB:
                raise serializers.ValidationError(f"El archivo supera {MAX_SIZE_MB}MB.")
    ...
    def create(self, validated_data):
        ...
        if archivo and instance.hash_sha256:
            duplicate = (
                DocumentoFuente.objects.exclude(pk=instance.pk)
                .filter(hash_sha256=instance.hash_sha256)
                .first()
            )
            if duplicate:
