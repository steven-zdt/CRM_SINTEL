...
class TipoImpuesto(models.Model):
    ...
    fecha_fin_vigencia = models.DateField(
        blank=True, null=True, verbose_name="Fecha Fin de Vigencia"
    )
...
class TarifaIVA(models.Model):
    ...
    tipo_tarifa = models.CharField(
        max_length=20, choices=TIPO_TARIFA_CHOICES, default="general", verbose_name="Tipo de Tarifa"
    )
    descripcion = models.TextField(blank=True, null=True, verbose_name="Descripción")
    activo = models.BooleanField(default=True, verbose_name="Activo")
    fecha_vigencia = models.DateField(verbose_name="Fecha de Vigencia")
...
class ConceptoRetencion(models.Model):
    ...
    tipo_retencion = models.CharField(
        max_length=20, choices=TIPO_RETENCION_CHOICES, verbose_name="Tipo de Retención"
    )
    porcentaje = models.DecimalField(
        max_digits=5,
        decimal_places=2,
...
class CodigoTributario(models.Model):
    ...
    codigo = models.CharField(
        max_length=10, unique=True, verbose_name="Código", help_text="Código tributario según DIAN"
    )
    nombre = models.CharField(max_length=200, verbose_name="Nombre")
...
class DocumentoFuente(models.Model):
    ...
    url_origen = models.URLField(
        null=True, blank=True, verbose_name="URL Origen", help_text="URL del documento a descargar"
    )
    ...
    tipo = models.CharField(
        max_length=10, choices=TIPO, default="OTRO", db_index=True, verbose_name="Tipo"
    )
    ...
    size_bytes = models.BigIntegerField(
        default=0, verbose_name="Tamaño (bytes)", help_text="Tamaño del archivo en bytes"
    )
    ...
    estado = models.CharField(
        max_length=12, choices=ESTADO, default="RECIBIDO", verbose_name="Estado"
    )
...
class IngestaLog(models.Model):
    ...
    documento = models.ForeignKey(
        DocumentoFuente, on_delete=models.CASCADE, related_name="logs", verbose_name="Documento"
    )
    ...
    nivel = models.CharField(
        max_length=10, choices=NIVEL_CHOICES, default="INFO", verbose_name="Nivel"
    )
    mensaje = models.TextField(verbose_name="Mensaje")
    payload = models.JSONField(
        null=True, blank=True, verbose_name="Payload", help_text="Datos adicionales en formato JSON"
    )
...
class NormaTributaria(models.Model):
    ...
    texto_plano = models.TextField(
        verbose_name="Texto Plano", help_text="Texto de la norma en formato plano"
    )
...
# ============================================================================
# WARNING: POLÍTICA SSoT: Estos modelos son la ÚNICA fuente de verdad para
...
class ContribuyenteTipo(models.Model):
    ...
    WARNING: POLÍTICA SSoT: Este modelo es la única fuente de verdad para tipos de contribuyentes.
    Base legal: Resolución 000013 de 2020 (DIAN) y normativa vigente.
    """

    CLASE_PN, CLASE_PJ = "PN", "PJ"
    CLASE_CHOICES = ((CLASE_PN, "Persona Natural"), (CLASE_PJ, "Persona Jurídica"))
    ...
    activo = models.BooleanField(
        default=True, verbose_name="Activo", help_text="Indica si este tipo está activo"
    )
...
class RegimenRenta(models.Model):
    ...
    WARNING: POLÍTICA SSoT: Este modelo es la única fuente de verdad para regímenes de renta.
    ...
    nombre = models.CharField(max_length=120, verbose_name="Nombre", help_text="Nombre del régimen")
    descripcion = models.TextField(
        blank=True, verbose_name="Descripción", help_text="Descripción detallada del régimen"
    )
    ...
    activo = models.BooleanField(
        default=True, verbose_name="Activo", help_text="Indica si este régimen está activo"
    )
...
class ResponsabilidadRUT(models.Model):
    ...
    WARNING: POLÍTICA SSoT: Este modelo es la única fuente de verdad para responsabilidades RUT.
    Base legal: Resolución 000013 de 2020 (DIAN) y normativa vigente.
    ...
    nombre = models.CharField(
        max_length=160, verbose_name="Nombre", help_text="Nombre de la responsabilidad"
    )
    descripcion = models.TextField(
        blank=True,
    ...
    activo = models.BooleanField(
        default=True, verbose_name="Activo", help_text="Indica si esta responsabilidad está activa"
    )
...
class PerfilTributario(models.Model):
    ...
    WARNING: POLÍTICA SSoT: Este modelo es la única fuente de verdad para perfiles tributarios.
    Permite definir combinaciones recomendadas de características tributarias.
    """

    nombre = models.CharField(
        max_length=160, unique=True, verbose_name="Nombre", help_text="Nombre del perfil tributario"
    )
    ...
    activo = models.BooleanField(
        default=True, verbose_name="Activo", help_text="Indica si este perfil está activo"
    )
