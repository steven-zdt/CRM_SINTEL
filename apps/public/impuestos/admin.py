...
class TarifaIVAAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Configuración', {
            'fields': ('porcentaje', 'tipo_tarifa', 'activo')
        }),
        ('Vigencia', {
            'fields': ('fecha_vigencia', 'fecha_fin_vigencia')
        }),
    )
...
class ConceptoRetencionAdmin(admin.ModelAdmin):
    list_display = ('codigo', 'nombre', 'tipo_retencion', 'porcentaje', 'base_minima', 'activo', 'fecha_vigencia')
    list_filter = ('tipo_retencion', 'activo', 'fecha_vigencia')
    search_fields = ('codigo', 'nombre', 'descripcion')
    ordering = ('tipo_retencion', 'codigo')
    fieldsets = (
        ('Información Básica', {
            'fields': ('codigo', 'nombre', 'tipo_retencion', 'descripcion')
        }),
        ('Configuración', {
            'fields': ('porcentaje', 'base_minima', 'activo')
        }),
        ('Vigencia', {
            'fields': ('fecha_vigencia', 'fecha_fin_vigencia')
        }),
...
class NormaTributariaInline(admin.TabularInline):
    ...
    extra = 0
    readonly_fields = ['articulo', 'tema', 'impuesto', 'vigencia_desde', 'vigencia_hasta', 'created_at']
...
class DocumentoFuenteAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Documento', {
            'fields': ('archivo', 'url_origen', 'fuente', 'tipo', 'fecha_publicacion')
        }),
        ('Metadatos', {
            'fields': ('hash_sha256', 'estado')
        }),
...
class NormaTributariaAdmin(admin.ModelAdmin):
    list_display = ['articulo', 'tema', 'impuesto', 'vigencia_desde', 'vigencia_hasta', 'documento_fuente']
    ...
    fieldsets = (
        ...
        ('Contenido', {
            'fields': ('texto_plano', 'texto_html', 'referencias')
        }),
        ('Auditoría', {
            'fields': ('created_at', 'updated_at'),
            'classes': ('collapse',)
...
class ContribuyenteTipoAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Información Básica', {
            'fields': ('nombre', 'clase', 'segmento_dian')
        }),
        ('Vigencia', {
...
class RegimenRentaAdmin(admin.ModelAdmin):
    ...
    list_display = ['codigo', 'nombre', 'tarifa_base_pj', 'requiere_facturacion_electronica', 'aplica_retenciones', 'activo', 'vigente_desde']
    list_filter = ['activo', 'codigo', 'requiere_facturacion_electronica', 'aplica_retenciones', 'vigente_desde', 'vigente_hasta']
    ...
    fieldsets = (
        ...
        ('Configuración', {
            'fields': ('tarifa_base_pj', 'requiere_facturacion_electronica', 'aplica_retenciones', 'activo')
        }),
        ('Vigencia', {
...
class ResponsabilidadRUTAdmin(admin.ModelAdmin):
    ...
    list_display = ['codigo', 'nombre', 'es_responsable_iva', 'es_no_responsable_iva', 'es_simple', 'es_facturador_electronico', 'es_gran_contribuyente', 'activo']
    list_filter = [
        'activo', 'es_responsable_iva', 'es_no_responsable_iva', 'es_simple',
        'es_facturador_electronico', 'es_gran_contribuyente', 'vigente_desde', 'vigente_hasta'
    ]
    ...
    fieldsets = (
        ...
        ('Características', {
            'fields': (
                'es_responsable_iva', 'es_no_responsable_iva', 'es_simple',
                'es_facturador_electronico', 'es_gran_contribuyente', 'activo'
            )
        }),
        ('Vigencia', {
            'fields': ('vigente_desde', 'vigente_hasta')
...
class PerfilTributarioAdmin(admin.ModelAdmin):
    ...
    list_display = ['nombre', 'tipo_contribuyente', 'regimen_renta', 'activo', 'recomendado_desde', 'recomendado_hasta']
    list_filter = ['activo', 'tipo_contribuyente', 'regimen_renta', 'recomendado_desde', 'recomendado_hasta']
    ...
    fieldsets = (
        ...
        ('Vigencia', {
            'fields': ('recomendado_desde', 'recomendado_hasta', 'activo')
        }),
    )
