...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for tipo_data in tipos_impuesto:
            ...
            if created:
                self.stdout.write(self.style.SUCCESS(f"  OK: {tipo.codigo} - {tipo.nombre}"))
            else:
                self.stdout.write(
                    self.style.WARNING(f"  INFO:  {tipo.codigo} - {tipo.nombre} (ya existe)")
                )
        ...
        for tarifa_data in tarifas_iva:
            ...
            if created:
                self.stdout.write(
                    self.style.SUCCESS(
                        f"  OK: {tarifa.codigo} - {tarifa.nombre} ({tarifa.porcentaje}%)"
                    )
                )
            else:
                self.stdout.write(
                    self.style.WARNING(f"  INFO:  {tarifa.codigo} - {tarifa.nombre} (ya existe)")
                )
        ...
        conceptos_retencion = [
            ...
            {
                "codigo": "ICA01",
                "nombre": "Retención ICA - Servicios",
                "tipo_retencion": "ica",
                "porcentaje": 4.00,
                "base_minima": 0,
            },
            ...
            {
                "codigo": "RENTA01",
                "nombre": "Retención Renta - Servicios",
            ...
            {
                "codigo": "RENTA02",
                "nombre": "Retención Renta - Honorarios",
                "tipo_retencion": "renta",
                "porcentaje": 10.00,
                "base_minima": 0,
            },
        ...
        for concepto_data in conceptos_retencion:
            ...
            if created:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f"  OK: {concepto.codigo} - {concepto.nombre} ({pct})")
                )
            else:
                self.stdout.write(
                    self.style.WARNING(
                        f"  INFO:  {concepto.codigo} - {concepto.nombre} (ya existe)"
                    )
                )
        ...
        codigos_tributarios = [
            ...
            {
                "codigo": "O-13",
                "nombre": "Obligado a Facturar Electrónicamente",
                "tipo": "Responsabilidad",
            },
            {
                "codigo": "O-15",
                "nombre": "Obligado a Facturar Electrónicamente - Régimen Simple",
                "tipo": "Responsabilidad",
            },
        ...
        for codigo_data in codigos_tributarios:
            ...
            if created:
                self.stdout.write(self.style.SUCCESS(f"  OK: {codigo.codigo} - {codigo.nombre}"))
            else:
                self.stdout.write(
                    self.style.WARNING(f"  INFO:  {codigo.codigo} - {codigo.nombre} (ya existe)")
                )
        ...
        for actividad_data in actividades:
            ...
            if created:
                self.stdout.write(
                    self.style.SUCCESS(f"  OK: {actividad.codigo} - {actividad.nombre}")
                )
            else:
                self.stdout.write(
                    self.style.WARNING(
                        f"  INFO:  {actividad.codigo} - {actividad.nombre} (ya existe)"
                    )
                )
        ...
        for regimen_data in regimenes_renta:
            regimen, created = RegimenRenta.objects.get_or_create(
                ...
                defaults={
                    ...
                    "requiere_facturacion_electronica": regimen_data[
            ...
            if created:
                self.stdout.write(self.style.SUCCESS(f"  OK: {regimen.codigo} - {regimen.nombre}"))
            else:
                ...
                if not regimen.activo:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"  OK: {regimen.codigo} - {regimen.nombre} (reactivado)"
                        )
                    )
                else:
                    self.stdout.write(
                        self.style.WARNING(
                            f"  INFO:  {regimen.codigo} - {regimen.nombre} (ya existe)"
                        )
                    )
        ...
        for tipo_data in tipos_contribuyente:
            ...
            if created:
                self.stdout.write(self.style.SUCCESS(f"  OK: {tipo.clase} - {tipo.nombre}"))
            else:
                if not tipo.activo:
                    tipo.activo = True
                    tipo.save()
                    self.stdout.write(
                        self.style.SUCCESS(f"  OK: {tipo.clase} - {tipo.nombre} (reactivado)")
                    )
                else:
                    self.stdout.write(
                        self.style.WARNING(f"  INFO:  {tipo.clase} - {tipo.nombre} (ya existe)")
                    )
        ...
        responsabilidades = [
            {
                ...
                "descripcion": "Responsable del Impuesto al Valor Agregado",
            },
            {
                "codigo": "49",
                "nombre": "No responsable de IVA",
                "descripcion": "No responsable del Impuesto al Valor Agregado",
            },
            {
                "codigo": "47",
                "nombre": "Responsable de IVA como agente de retención",
                "descripcion": "Responsable de IVA como agente de retención (Régimen SIMPLE)",
            },
            {
                "codigo": "52",
                "nombre": "Gran contribuyente",
                "descripcion": "Gran contribuyente según DIAN",
            },
            {
                "codigo": "13",
                "nombre": "Obligado a facturar electrónicamente",
                "descripcion": "Obligado a facturar electrónicamente",
            },
        ]
        ...
        for resp_data in responsabilidades:
            ...
            if created:
                self.stdout.write(self.style.SUCCESS(f"  OK: {resp.codigo} - {resp.nombre}"))
            else:
                if not resp.activo:
                    resp.activo = True
                    resp.save()
                    self.stdout.write(
                        self.style.SUCCESS(f"  OK: {resp.codigo} - {resp.nombre} (reactivado)")
                    )
                else:
                    self.stdout.write(
                        self.style.WARNING(f"  INFO:  {resp.codigo} - {resp.nombre} (ya existe)")
                    )
        ...
        self.stdout.write(
            f"   - Regímenes de Renta: {RegimenRenta.objects.filter(activo=True).count()}"
        )
        self.stdout.write(
            f"   - Tipos de Contribuyente: {ContribuyenteTipo.objects.filter(activo=True).count()}"
        )
        self.stdout.write(
            f"   - Responsabilidades RUT: {ResponsabilidadRUT.objects.filter(activo=True).count()}"
        )
        self.stdout.write(
            "\nOK: El catálogo está disponible para todos los tenants en el esquema public."
        )
