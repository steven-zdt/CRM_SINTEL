...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            type=int,
            default=0,
            help="Límite de registros a exportar (0 = todos)"
        )
    ...
    def handle(self, *args, **options):
        ...
        for item in data:
            if item.get("vigencia_desde"):
                item["vigencia_desde"] = item["vigencia_desde"].isoformat() if hasattr(item["vigencia_desde"], "isoformat") else str(item["vigencia_desde"])
            if item.get("vigencia_hasta"):
                item["vigencia_hasta"] = item["vigencia_hasta"].isoformat() if hasattr(item["vigencia_hasta"], "isoformat") else str(item["vigencia_hasta"])
