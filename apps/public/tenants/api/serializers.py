...
class DomainSerializer(serializers.ModelSerializer):
    ...
    ⚠️ IMPORTANTE: Implementa normalización automática de dominios.
    ...
    def validate_domain(self, value: str) -> str:
        ...
        ⚠️ REGLAS ESTRICTAS (según documentación oficial):
        ...
        # ⚠️ VALIDACIÓN EXPLÍCITA: Rechazar dominios con puerto (según doc oficial)
        ...
        if not normalized:
            raise serializers.ValidationError("El dominio no puede estar vacío después de la normalización")
...
class OnboardTenantWithOwnerSerializer(serializers.Serializer):
    ...
    ⚠️ CONTRATO ESTABLE: Retorna dict con client_id, domain, membership_id, login_url
    ...
    ⚠️ CAMBIO v2.25: dominio_fqdn es OPCIONAL. Si viene vacío o inválido, se autogenera
    como <schema_name>.<TENANT_DOMAIN_BASE> (ej: cliente.sintel.com)
    
    ⚠️ CAMBIO v2.29: NO se aceptan campos de password en onboarding.
    ...
    def validate(self, data):
        ...
        ⚠️ CAMBIO v2.25: Autogeneración de dominio FQDN como <schema>.<TENANT_DOMAIN_BASE>
        ⚠️ CAMBIO v2.29: Rechaza explícitamente campos de password
        """
        # ⚠️ v2.29: Rechazar explícitamente campos de password
        forbidden_fields = {'password', 'password1', 'password2', 'owner_password'}
        ...
        # ⚠️ v2.29: El password se define EXCLUSIVAMENTE en la activación dentro del subdominio del tenant
    ...
    def validate_dominio_fqdn(self, value: str) -> str:
        ...
        ⚠️ CAMBIO v2.25: Si viene vacío o inválido, se autogenera en validate().
...
class ClientSerializer(serializers.ModelSerializer):
    ...
    ⚠️ OPTIMIZACIÓN: No expone membresías por defecto para API pública.
    ...
    # ⚠️ NOTA: memberships solo se incluye si se necesita (usar related_name correcto)
    # memberships = TenantMembershipSerializer(many=True, read_only=True, source='memberships.all')
    
    class Meta:
        model = Client
        fields = ('id', 'schema_name', 'nombre', 'paid_until', 'on_trial', 'created_on', 'is_active', 'domains')
