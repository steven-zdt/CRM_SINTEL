    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            try:
                ...
                self.stdout.write(f"OK: Tenant público encontrado: {tenant.nombre}")
            except Client.DoesNotExist:
                self.stdout.write(
                    self.style.ERROR(
                        "ERROR: El tenant público no existe. Ejecuta primero: python manage.py setup_public_tenant"
                    )
                )
                return
            ...
            if dominio_esperado_obj:
                ...
                if dominio_esperado_obj.tenant == tenant:
                    ...
                    if dominio_esperado_obj.is_primary:
                        self.stdout.write(
                            self.style.SUCCESS(
                                f"OK: El dominio {dominio_esperado} ya está configurado correctamente como dominio principal"
                            )
                        )
                    else:
                        ...
                        Domain.objects.filter(tenant=tenant, is_primary=True).exclude(
                            pk=dominio_esperado_obj.pk
                        ).update(is_primary=False)
                        self.stdout.write(
                            self.style.SUCCESS(
                                f"OK: Dominio {dominio_esperado} actualizado como dominio principal"
                            )
                        )
                else:
                    ...
                    if force:
                        ...
                        self.stdout.write(
                            self.style.WARNING(
                                f"WARNING:  Dominio {dominio_esperado} reasignado desde {old_tenant.nombre} al tenant público"
                            )
                        )
                    else:
                        self.stdout.write(
                            self.style.ERROR(
                                f"ERROR: El dominio {dominio_esperado} está asignado a otro tenant: {dominio_esperado_obj.tenant.nombre}. "
                ...
                dominio_esperado_obj = Domain.objects.create(
                    domain=dominio_esperado, tenant=tenant, is_primary=True
                )
                # Desmarcar otros dominios como no principales
                Domain.objects.filter(tenant=tenant, is_primary=True).exclude(
                    pk=dominio_esperado_obj.pk
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"OK: Dominio {dominio_esperado} creado como dominio principal"
                    )
                )
            ...
            if dominio_principal_final and dominio_principal_final.domain == dominio_esperado:
                ...
                self.stdout.write(
                    self.style.ERROR(
                        f"ERROR: Error: El dominio principal no coincide con el esperado. "
            ...
            self.stdout.write(
                self.style.ERROR(f"ERROR: Error al verificar dominio del tenant público: {e}")
            )
            import traceback
