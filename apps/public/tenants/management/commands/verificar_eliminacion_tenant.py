...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR(f'ERROR: ERROR: El Client "{schema_name}" AÚN EXISTE')
            )
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f'OK: El Client "{schema_name}" NO existe (eliminado correctamente)'
                )
            )
            client_exists = False
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR(f'ERROR: ERROR: El Domain "{domain_name}" AÚN EXISTE')
            )
            self.stdout.write(f"   - ID: {domain.pk}")
            self.stdout.write(
                f"   - Tenant: {domain.tenant.schema_name if domain.tenant else 'None'}"
            )
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f'OK: El Domain "{domain_name}" NO existe (eliminado correctamente)'
                )
            )
            domain_exists = False
        ...
        if client_exists:
            ...
            if memberships.exists():
                self.stdout.write(
                    self.style.ERROR(
                        f"ERROR: ERROR: Existen {memberships.count()} TenantMembership asociadas:"
                    )
                )
                for m in memberships:
                    self.stdout.write(
                        f"   - Usuario: {m.user.email} ({m.user.username}) - Rol: {m.rol}"
                    )
                membership_exists = True
            else:
                self.stdout.write(
                    self.style.SUCCESS(
                        f'OK: No hay TenantMembership asociadas al tenant "{schema_name}"'
                    )
                )
            ...
            if memberships.exists():
                self.stdout.write(
                    self.style.WARNING(
                        f"WARNING:  ADVERTENCIA: Existen {memberships.count()} TenantMembership huérfanas (client eliminado pero membresías quedaron):"
                    )
                )
                for m in memberships:
                    self.stdout.write(
        ...
        try:
            ...
            if schema_exist:
                self.stdout.write(
                    self.style.ERROR(
                        f'ERROR: ERROR: El esquema "{schema_name}" AÚN EXISTE en PostgreSQL'
                    )
                )
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f'OK: El esquema "{schema_name}" NO existe en PostgreSQL (eliminado correctamente)'
                    )
                )
                schema_exists_flag = False
        except Exception as e:
            self.stdout.write(self.style.WARNING(f"WARNING:  Error al verificar el esquema: {e}"))
        ...
        if related_domains.exists():
            self.stdout.write(
                self.style.WARNING(
                    f"WARNING:  ADVERTENCIA: Existen {related_domains.count()} dominios relacionados:"
                )
            )
            for d in related_domains:
                self.stdout.write(
                    f"   - {d.domain} (tenant: {d.tenant.schema_name if d.tenant else 'None'})"
                )
        else:
            self.stdout.write(
                self.style.SUCCESS(f'OK: No hay otros dominios relacionados con "{schema_name}"')
            )
        ...
        if client_exists or domain_exists or membership_exists or schema_exists_flag:
            self.stdout.write(
                self.style.ERROR("ERROR: EL TENANT NO HA SIDO ELIMINADO COMPLETAMENTE")
            )
            self.stdout.write("\n📋 Estado actual:")
            ...
            self.stdout.write(
                f"   - TenantMembership existen: {'SÍ' if membership_exists else 'NO'}"
            )
            self.stdout.write(
                f"   - Esquema en PostgreSQL existe: {'SÍ' if schema_exists_flag else 'NO' if schema_exists_flag is not None else 'ERROR'}"
            )
            ...
            if client_exists:
                ...
                self.stdout.write(
                    f'   >>> client = Client.objects.get(schema_name="{schema_name}")'
                )
                ...
                self.stdout.write("   >>>")
                self.stdout.write(
                    "   >>> # 3. Eliminar Client (esto puede eliminar el esquema si auto_drop_schema=True)"
                )
            ...
            if schema_exists_flag:
                ...
                self.stdout.write(
                    f'   ...     cursor.execute(f"DROP SCHEMA IF EXISTS {schema_name} CASCADE;")'
                )
                self.stdout.write("   >>>")
            self.stdout.write("   >>> # 5. Verificar eliminación")
            self.stdout.write("   >>> from django_tenants.utils import schema_exists")
            self.stdout.write(
                f"   >>> print(f\"Esquema existe: {{schema_exists('{schema_name}')}}\")"
            )
        else:
            self.stdout.write(self.style.SUCCESS("OK: EL TENANT HA SIDO ELIMINADO COMPLETAMENTE"))
            self.stdout.write("\n📋 Verificación completa:")
            self.stdout.write("   OK: Client eliminado")
            self.stdout.write("   OK: Domain eliminado")
            self.stdout.write("   OK: TenantMembership eliminadas")
            self.stdout.write("   OK: Esquema en PostgreSQL eliminado")
