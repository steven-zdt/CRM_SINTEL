...
WARNING: En producción, la activación vendrá desde Core API o Celery Beat por tenant.
WARNING: Mantén credenciales por tenant fuera de settings.py (SSoT/Config por tenant).
"""
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--host", type=str, required=True, help="Servidor de correo (ej: imap.gmail.com)"
        )
        parser.add_argument(
            "--port", type=int, default=993, help="Puerto del servidor (default: 993 para IMAP SSL)"
        )
        parser.add_argument(
            "--protocol",
            type=str,
        ...
        parser.add_argument(
            "--ssl", action="store_true", default=True, help="Usar SSL/TLS (default: True)"
        )
        parser.add_argument("--no-ssl", dest="ssl", action="store_false", help="No usar SSL/TLS")
        parser.add_argument(
            "--username", type=str, required=True, help="Usuario del buzón de correo"
        )
        parser.add_argument(
            "--password", type=str, required=True, help="Contraseña del buzón de correo"
        )
        parser.add_argument(
            "--mailbox",
            type=str,
    ...
    def handle(self, *args, **opts):
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"OK: Tarea encolada exitosamente:\n"
                    f"   - Task ID: {result.id}\n"
                    f"   - Tenant: {schema}\n"
            ...
            self.stdout.write(
                self.style.WARNING(
                    "\nWARNING:  Nota: La tarea se ejecutará de forma asíncrona.\n"
