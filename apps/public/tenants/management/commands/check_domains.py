    ...
    def handle(self, *args, **options):
        ...
        with schema_context(get_public_schema_name()):
            ...
            if domains.exists():
                ...
                for d in domains:
                    status = "✅" if d.tenant.is_active else "⏸️"
                    self.stdout.write(
                ...
                if domain_to_check:
                    ...
                    if domain_obj:
                        self.stdout.write(self.style.SUCCESS(f"\n✅ '{domain_to_check}' está registrado:"))
                        self.stdout.write(f"   - ID: {domain_obj.id}")
                        self.stdout.write(f"   - Tenant: {domain_obj.tenant.schema_name} ({domain_obj.tenant.nombre})")
                        self.stdout.write(f"   - Primary: {domain_obj.is_primary}")
                        self.stdout.write(f"   - Is Active: {domain_obj.tenant.is_active}")
                    else:
                        self.stdout.write(self.style.ERROR(f"\n❌ '{domain_to_check}' NO está registrado en la base de datos"))
                        self.stdout.write(self.style.WARNING("   Necesitas crear un tenant con este dominio"))
            else:
                self.stdout.write(self.style.WARNING("⚠️  No hay dominios registrados en la base de datos"))
