...
⚠️ v2.30: Útil para diagnosticar problemas de migraciones en multi-tenant.
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if schema_name == public_schema:
            raise CommandError(f"Este comando es para schemas de tenant, no para '{public_schema}'.")
        
        # Verificar que existe el Client
        try:
            client = Client.objects.get(schema_name=schema_name)
            self.stdout.write(self.style.SUCCESS(f"✅ Tenant encontrado: {client.nombre} ({schema_name})"))
        except Client.DoesNotExist:
            self.stdout.write(self.style.WARNING(f"⚠️  Schema '{schema_name}' existe pero no hay Client asociado."))
        ...
        try:
            with connection.cursor() as cursor:
                if table_name:
                    ...
                    if not columns:
                        self.stdout.write(self.style.WARNING(f"⚠️  La tabla '{table_name}' no existe en el schema '{schema_name}'."))
                        return
                    
                    self.stdout.write(self.style.SUCCESS(f"\n📋 Columnas de '{table_name}' en schema '{schema_name}':\n"))
                    self.stdout.write(f"{'Columna':<30} {'Tipo':<20} {'Longitud':<10} {'Nullable':<10} {'Default':<20}")
                    ...
                    for col in columns:
                        ...
                        self.stdout.write(f"{col_name:<30} {data_type:<20} {max_len_str:<10} {nullable_str:<10} {default_str:<20}")
                else:
                    ...
                    if not tables:
                        self.stdout.write(self.style.WARNING(f"⚠️  No hay tablas en el schema '{schema_name}'."))
                        return
