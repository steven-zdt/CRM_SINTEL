...
WARNING: v2.30: API-First - Este comando facilita pruebas del flujo de activación.
...
from apps.public.tenants.services.invitations import build_activation_url, generate_invitation_token
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--tenant-domain", type=str, help="Dominio del tenant (ej: cliente.sintel.com)"
        )
        parser.add_argument("--schema", type=str, help="Schema name del tenant (ej: cliente)")
        parser.add_argument(
            "--ttl-minutes", type=int, default=60, help="TTL del token en minutos (default: 60)"
        )
    ...
    def handle(self, *args, **options):
        ...
        with schema_context(get_public_schema_name()):
            ...
            self.stdout.write(
                self.style.SUCCESS(f"OK: Usuario {email} reseteado a contraseña no usable")
            )

            # Generar token de activación
            ttl_hours = ttl_minutes / 60.0
            token = generate_invitation_token(
                user_id=user.id, tenant_id=tenant.id, ttl_hours=ttl_hours
            )
            ...
            self.stdout.write(self.style.SUCCESS("=" * 80))
            self.stdout.write("")
            self.stdout.write(self.style.SUCCESS("OK: Listo para pruebas"))
            self.stdout.write("")
            self.stdout.write("Pruebas sugeridas:")
            self.stdout.write(f"  1. GET {activation_url}")
            self.stdout.write("     → Debe retornar 200 con información del usuario/tenant")
            self.stdout.write(f"  2. POST {activation_url}")
            self.stdout.write("     → Con password1 y password2 para activar")
            self.stdout.write("")
