    ...
    def handle(self, *args, **options):
        ...
        if not skip_migrations:
            ...
            try:
                ...
                self.stdout.write(self.style.SUCCESS('✅ Migraciones del esquema public completadas'))
        ...
        if not schema_exists('public'):
            self.stdout.write(self.style.ERROR('❌ El esquema public no existe. Ejecuta migraciones primero.'))
            return
        ...
        try:
            ...
            if created:
                self.stdout.write(self.style.SUCCESS(f'✅ Tenant público creado: {tenant.nombre}'))
            else:
                self.stdout.write(self.style.WARNING(f'ℹ️  Tenant público ya existe: {tenant.nombre}'))
            ...
            if deleted_count > 0:
                self.stdout.write(self.style.SUCCESS(f'✅ {deleted_count} dominio(s) no deseado(s) eliminado(s)'))
            ...
            if domain_created:
                self.stdout.write(self.style.SUCCESS(f'✅ Dominio creado: {domain.domain}'))
            else:
                if domain.tenant != tenant:
                    domain.tenant = tenant
                    domain.is_primary = True
                    domain.save()
                    self.stdout.write(self.style.SUCCESS(f'✅ Dominio actualizado: {domain.domain}'))
                else:
                    self.stdout.write(self.style.WARNING(f'ℹ️  Dominio ya existe: {domain.domain}'))
            ...
            if '127.0.0.1' != domain_name:
                ...
                if created:
                    self.stdout.write(self.style.SUCCESS(f'✅ Dominio adicional creado: 127.0.0.1'))
                    ...
                    self.stdout.write(self.style.SUCCESS(f'✅ Dominio adicional actualizado: 127.0.0.1'))
            ...
            self.stdout.write(f'   - Dominios: {", ".join([d.domain for d in Domain.objects.filter(tenant=tenant)])}')

        except Exception as e:
            self.stdout.write(self.style.ERROR(f'❌ Error al crear tenant público: {e}'))
            import traceback
