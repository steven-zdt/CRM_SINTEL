...
⚠️ v3.3: Genera tenants con datos realistas para análisis de comportamiento
⚠️ Uso: python manage.py generar_tenants_prueba --cantidad 100 --con-datos
...
base_path = settings.BASE_DIR if hasattr(settings, 'BASE_DIR') else os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(__file__))))
...
NOMBRES_EMPRESAS = [
    "Tecnología", "Soluciones", "Servicios", "Consultoría", "Ingeniería",
    "Construcción", "Comercializadora", "Distribuidora", "Manufactura", "Logística"
]

SECTORES = [
    "Tecnología", "Construcción", "Retail", "Manufactura", "Servicios",
    "Consultoría", "Logística", "Alimentación", "Salud", "Educación"
]

NOMBRES_CLIENTES = [
    "Juan", "María", "Carlos", "Ana", "Luis", "Laura", "Pedro", "Carmen",
    "Roberto", "Patricia", "Fernando", "Sandra", "Miguel", "Diana", "Andrés"
]
...
CIUDADES = [
    ...
    "Pereira", "Santa Marta", "Manizales", "Armenia", "Villavicencio", "Pasto"
]
        ...
        for i in range(num_productos):
            try:
                producto = Producto.objects.create(
                    ...
                    referencia=f"REF-{random.randint(1000, 9999)}" if random.random() > 0.3 else None,
        ...
        for i in range(num_cotizaciones):
            try:
                ...
                cotizacion = Cotizacion.objects.create(
                    ...
                    aiu_admin_porcentaje=Decimal("10.00") if random.random() > 0.5 else Decimal("0.00"),
                    aiu_imprevistos_porcentaje=Decimal("5.00") if random.random() > 0.5 else Decimal("0.00"),
                    aiu_utilidad_porcentaje=Decimal("15.00") if random.random() > 0.5 else Decimal("0.00"),
        ...
        for i in range(num_proyectos):
            try:
                ...
                proyecto = Proyecto.objects.create(
                    ...
                    tipo_servicio=random.choice(["INSTALACION", "MANTENIMIENTO", "CONSULTORIA", "DESARROLLO"]),
                    fase_actual=random.choice(["BORRADOR", "INICIO", "PLANEACION", "EJECUCION", "CIERRE"]),
                    estado_tarea=random.choice(["PENDIENTE", "EN_PROCESO", "COMPLETADA", "CANCELADA"]),
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if con_datos:
            self.stdout.write(self.style.WARNING('⚠️  Modo CON DATOS activado - Esto puede tardar varios minutos'))
        ...
        try:
            ...
            if not admin_base:
                ...
                self.stdout.write(self.style.SUCCESS(f'✅ Usuario admin base creado: {admin_base.email}'))
        except Exception as e:
            self.stdout.write(self.style.ERROR(f'❌ Error creando usuario admin base: {e}'))
        ...
        for i in range(inicio, inicio + cantidad):
            ...
            try:
                ...
                schema_name_esperado = nombre_empresa.lower().replace(' ', '-').replace('.', '').replace('sas', '')[:63]
                
                if skip_existing and Client.objects.filter(schema_name__icontains=str(tenant_index)).exists():
                    self.stdout.write(self.style.WARNING(f'  ⏭️  Tenant {tenant_index} ya existe, omitiendo...'))
                    stats['omitidos'] += 1
                    continue
                ...
                stats['creados'] += 1
                self.stdout.write(self.style.SUCCESS(f'  ✅ Tenant creado: {client.schema_name}'))
                ...
                if con_datos:
                    try:
                        ...
                        logger.error(f"[generar_tenants_prueba] Error poblando datos: {e}", exc_info=True)
                        self.stdout.write(self.style.WARNING(f'  ⚠️  Error poblando datos: {e}'))
                        stats['sin_datos'] += 1
                else:
                    stats['sin_datos'] += 1
        ...
        self.stdout.write('=' * 80)
        self.stdout.write(f'✅ Tenants creados: {stats["creados"]}')
