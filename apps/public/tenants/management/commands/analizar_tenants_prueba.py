...
⚠️ v3.3: Análisis de comportamiento activo de tenants privados
⚠️ Uso: python manage.py analizar_tenants_prueba
...
def analizar_tenant(schema_name: str) -> dict:
    ...
    try:
        with schema_context(schema_name):
            ...
            if empresa:
                ...
                stats['fecha_creacion'] = empresa.created_at.isoformat() if hasattr(empresa, 'created_at') else None
                stats['activo'] = True
            ...
            if ultimo_proyecto and hasattr(ultimo_proyecto, 'updated_at'):
                if not stats['ultima_actividad'] or ultimo_proyecto.updated_at > datetime.fromisoformat(stats['ultima_actividad'].replace('Z', '+00:00')):
    ...
    try:
        ...
        stats['created_on'] = client.created_on.isoformat() if hasattr(client, 'created_on') else None
        ...
        for i, client in enumerate(tenants, 1):
            ...
            try:
                ...
                if stats.get('activo'):
                    self.stdout.write(
                        self.style.SUCCESS(
                            f'  ✅ Activo: {stats["clientes"]} clientes, '
                            f'{stats["cotizaciones"]} cotizaciones, {stats["proyectos"]} proyectos'
                        )
                    )
                else:
                    self.stdout.write(self.style.WARNING('  ⚠️  Sin datos'))
