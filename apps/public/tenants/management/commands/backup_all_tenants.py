    ...
    def handle(self, *args, **options):
        ...
        for tenant in tenants:
            ...
            if skip_public and tenant.schema_name == "public":
                self.stdout.write(self.style.WARNING("⏭️  Omitiendo tenant public"))
                continue
            ...
            try:
                ...
                success_count += 1
                self.stdout.write(self.style.SUCCESS(f"OK: Backup completado: {tenant.nombre}"))
            except Exception as e:
                error_count += 1
                self.stdout.write(
                    self.style.ERROR(f"ERROR: Error en backup de {tenant.nombre}: {str(e)}")
                )

        # Resumen
        self.stdout.write(self.style.SUCCESS("\n📊 Resumen:"))
        self.stdout.write(f"   OK: Exitosos: {success_count}")
