...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if tenant:
            self.stdout.write(
                self.style.SUCCESS(
                    f"OK: Tenant encontrado: {tenant.nombre} (schema: {tenant.schema_name})"
                )
            )
        ...
        if not domain_list:
            self.stdout.write(
                self.style.ERROR("ERROR: No hay dominios configurados para este tenant")
            )
            return
        ...
        for hostname in hostnames_to_test:
            ...
            if domain_exact:
                self.stdout.write(
                    self.style.SUCCESS(f"   OK: Dominio encontrado (exacto): {domain_exact.domain}")
                )
                continue
            ...
            if domain_clean:
                self.stdout.write(
                    self.style.SUCCESS(
                        f"   OK: Dominio encontrado (sin puerto): {domain_clean.domain}"
                    )
                )
                self.stdout.write(
                    self.style.WARNING(
                        "   WARNING:  django-tenants debería usar este dominio automáticamente"
                    )
                )
            else:
                self.stdout.write(self.style.ERROR("   ERROR: Dominio NO encontrado"))
