...
WARNING: OBJETIVO: Garantizar que la landing page (/) sea la vista principal
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for tenant_dir in tenant_dirs:
            ...
            for py_file in tenant_dir.rglob("*.py"):
                ...
                try:
                    ...
                    self.stdout.write(
                        self.style.WARNING(f"   WARNING:  Error al leer {py_file}: {e}")
                    )
        ...
        if not issues_found:
            self.stdout.write(
                self.style.SUCCESS("OK: No se encontraron referencias a /admin/login/")
            )
        else:
            ...
            if critical_issues:
                ...
                self.stdout.write(
                    self.style.ERROR(
                        f"ERROR: Referencias CRÍTICAS encontradas: {len(critical_issues)}"
                    )
                )
                for issue in critical_issues:
                    rel_path = issue["file"].relative_to(base_dir)
                    self.stdout.write(self.style.ERROR(f"   📄 {rel_path}:{issue['line']}"))
            ...
            if comment_issues:
                ...
                self.stdout.write(
                    self.style.WARNING(
                        f"WARNING:  Referencias en comentarios/docstrings: {len(comment_issues)}"
                    )
                )
                for issue in comment_issues[:5]:  # Mostrar solo las primeras 5
                    rel_path = issue["file"].relative_to(base_dir)
                    self.stdout.write(
                        self.style.WARNING(f"   📄 {rel_path}:{issue['line']} (comentario)")
                    )
        ...
        if issues_found and not fix_issues:
            self.stdout.write(
                self.style.WARNING("💡 Ejecuta con --fix para intentar corregir automáticamente")
            )
