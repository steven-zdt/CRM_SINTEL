...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS(f'🔄 Iniciando restauración del tenant: {schema_name}'))

        # Verificar que el tenant existe (o crearlo si se solicita)
        try:
            tenant = Client.objects.get(schema_name=schema_name)
            self.stdout.write(f'✅ Tenant encontrado: {tenant.nombre}')
        except Client.DoesNotExist:
            if create_schema:
                self.stdout.write(self.style.WARNING(f'⚠️  Tenant no existe, se creará el esquema'))
            else:
                raise CommandError(f'Tenant con schema_name "{schema_name}" no existe. Use --create-schema para crearlo.')
        ...
        try:
            ...
            self.stdout.write(self.style.SUCCESS(f'✅ Restauración completada exitosamente'))
