...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not settings.DEBUG:
            self.stdout.write(
                self.style.WARNING(
                    "WARNING:  Este comando solo debe ejecutarse en desarrollo (DEBUG=True)"
                )
            )
            return
        ...
        if not tenants.exists():
            self.stdout.write(self.style.WARNING("WARNING:  No se encontraron tenants privados"))
            return
        ...
        for tenant in tenants:
            ...
            if not primary_domain:
                self.stdout.write(self.style.ERROR("   ERROR: No tiene dominio principal"))
                error_count += 1
                continue
            ...
            if existing:
                if existing.tenant == tenant:
                    if force:
                        ...
                        existing.save()
                        self.stdout.write(
                            self.style.SUCCESS(f"   OK: Dominio '{domain_with_port}' actualizado")
                        )
                        fixed_count += 1
                    else:
                        self.stdout.write(
                            self.style.WARNING(
                                f"   INFO:  Dominio '{domain_with_port}' ya existe (omitiendo)"
                            )
                        )
                        skipped_count += 1
                else:
                    self.stdout.write(
                        self.style.ERROR(
                            f"   ERROR: Dominio '{domain_with_port}' existe pero pertenece a otro tenant"
                        )
                    )
                    error_count += 1
            else:
                ...
                try:
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(f"   OK: Dominio '{domain_with_port}' creado")
                    )
                    fixed_count += 1
                except Exception as e:
                    self.stdout.write(self.style.ERROR(f"   ERROR: Error al crear dominio: {e}"))
                    error_count += 1
        ...
        self.stdout.write("=" * 80)
        self.stdout.write(f"OK: Dominios corregidos: {fixed_count}")
