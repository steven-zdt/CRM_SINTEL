...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=str,
            default='8000',
            help='Puerto a agregar (default: 8000)'
        )
    ...
    def handle(self, *args, **options):
        ...
        if not domain_primary:
            self.stdout.write(self.style.ERROR(f"❌ No hay dominio principal para el tenant '{schema_name}'"))
            return
        ...
        if existing:
            if existing.tenant == tenant:
                self.stdout.write(self.style.WARNING(
                    f"ℹ️  El dominio '{domain_with_port}' ya existe y está correctamente configurado"
                ))
            else:
                self.stdout.write(self.style.ERROR(
                    f"❌ El dominio '{domain_with_port}' existe pero pertenece a otro tenant"
                ))
            return
        
        # Crear dominio con puerto
        domain_new = Domain.objects.create(
        ...
        self.stdout.write(self.style.SUCCESS(
            f"✅ Dominio '{domain_with_port}' creado para el tenant '{schema_name}'"
        ))
        self.stdout.write(
            f"   Ahora puedes acceder a: http://{domain_with_port}/"
        )
