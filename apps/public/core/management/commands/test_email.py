...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("recipient", type=str, help="Email del destinatario para la prueba")
        parser.add_argument(
            "--subject",
            type=str,
    ...
    def handle(self, *args, **options):
        ...
        email_config = {
            ...
            "SERVER_EMAIL": getattr(
                settings, "SERVER_EMAIL", getattr(settings, "DEFAULT_FROM_EMAIL", "NOT SET")
            ),
        ...
        if port and isinstance(port, int):
            if port == 587 and not use_tls:
                self.stdout.write(
                    self.style.WARNING(
                        "WARNING:  ADVERTENCIA: Puerto 587 normalmente requiere EMAIL_USE_TLS=True"
                    )
                )
            elif port == 465 and not use_ssl:
                self.stdout.write(
                    self.style.WARNING(
                        "WARNING:  ADVERTENCIA: Puerto 465 normalmente requiere EMAIL_USE_SSL=True"
                    )
                )
            elif use_tls and use_ssl:
                self.stdout.write(
                    self.style.ERROR(
                        "ERROR: ERROR: EMAIL_USE_TLS y EMAIL_USE_SSL no pueden estar ambos en True"
                    )
                )
...
"""
        ...
        try:
            # WARNING: IMPORTANTE: fail_silently=False para capturar excepciones
            result = send_mail(
                subject=subject,
                message=message,
            ...
            self.stdout.write('  Si el backend es "console", revisa la salida del terminal.')
            self.stdout.write(
                f'  Si el backend es "smtp", verifica la bandeja de entrada de {recipient}'
            )

            # Logging adicional
            logger.info(
                "OK: Prueba de email EXITOSA: from=%s, to=%s, subject='%s', backend=%s, host=%s, port=%s",
            ...
            self.stdout.write(self.style.ERROR(f"\nERROR: ERROR enviando email: {str(e)}"))
            self.stdout.write(self.style.ERROR(f"  Tipo de error: {type(e).__name__}"))

            # Logging detallado del error
            logger.error(
                "ERROR: Prueba de email FALLIDA: from=%s, to=%s, subject='%s', backend=%s, host=%s, port=%s, error=%s",
                email_config["DEFAULT_FROM_EMAIL"],
                recipient,
                subject,
                email_config["EMAIL_BACKEND"],
