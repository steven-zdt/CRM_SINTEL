...
⚠️ v3.3: API-First - Endpoints DRF para acceso público a documentos
⚠️ Seguridad: Validación de hash SHA256 para integridad
"""
import logging
...
def validar_token_acceso(request):
    ...
    ⚠️ API-First: Endpoint público (AllowAny) para validación de tokens
    ...
    try:
        if request.method == 'GET':
            ...
            tipo_documento = request.query_params.get('tipo') or request.query_params.get('tipo_documento')
            documento_id = request.query_params.get('id') or request.query_params.get('documento_id')
            tenant_schema = request.query_params.get('tenant') or request.query_params.get('tenant_schema')
        else:  # POST
            data = request.data
...
def obtener_documento_publico(request):
    ...
    ⚠️ API-First: Endpoint público (AllowAny) para obtener datos de documentos compartidos
    ⚠️ Seguridad: Requiere token válido en query params
    ...
    try:
        ...
        with schema_context(tenant_schema):
            # ⚠️ NOTA: La implementación completa requiere importar los modelos del tenant
