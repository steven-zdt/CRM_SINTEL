...
const jwtAuth = {
    ...
    /**
     * Login: Obtiene tokens desde /api/token/.
     * 
    ...
    async getValidAccessToken() {
        ...
        if (!access) {
            // Primero intentar obtener desde sesión
            try {
        ...
        if (!isValid) {
            ...
            const refresh = this.getRefreshToken();
            if (!refresh) {
                // Si no hay refresh token, intentar obtener desde sesión
                try {
                    const response = await fetch('/console/jwt/from-session/', {
                        method: 'GET',
                        credentials: 'include',
...
// Obtiene JWT token automáticamente desde la sesión de Django
