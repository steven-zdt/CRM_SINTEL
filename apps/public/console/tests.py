...
from apps.public.impuestos.models import TipoImpuesto, TarifaIVA
...
class ConsoleIsolationAndPermissionsTests(TenantTestCase):
    ...
    def setUp(self):
        ...
        public_tenant, _ = TenantClient.objects.get_or_create(
            schema_name='public',
            defaults={'nombre': 'Public Tenant', 'on_trial': False}
        )
...
class ConsoleAPIConsumptionTests(TenantTestCase):
    ...
    def setUp(self):
        ...
        Domain.objects.create(
            ...
            tenant=self.tenant_instance_1,
            is_primary=True
        )
        Domain.objects.create(
            domain='empresa2.localhost',
            tenant=self.tenant_instance_2,
            is_primary=True
        )
    ...
    def test_tenants_api_pagination_follows_standard(self):
        ...
        for i in range(30):
            TenantClient.objects.create(
                schema_name=f'empresa{i+3}',
                nombre=f'Empresa {i+3}',
                on_trial=True
            )
...
class ConsoleBusinessLogicTests(TenantTestCase):
    ...
    def test_create_tenant_handles_validation_errors(self, mock_crear_empresa):
        ...
        mock_crear_empresa.side_effect = ValidationError("El dominio 'test.localhost' ya está en uso")
    ...
    def test_create_tenant_creates_schema(self):
        ...
        # ⚠️ v2.17: Estandarización de subdominios - dominio se construye automáticamente
        result = crear_empresa(
            nombre='Empresa Schema Test',
            email_admin='admin@schema-test.com',
            on_trial=True
        )
...
class ConsoleLegalDataTests(TenantTestCase):
    ...
    def test_impuestos_api_pagination(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data['results']), 20)  # page_size default (StandardResultsSetPagination)
        self.assertIsNotNone(response.data['next'])  # Hay más páginas
...
class ConsoleIntegrationTests(TenantTestCase):
    ...
    def test_full_flow_create_tenant_from_console(self):
        ...
        result = crear_empresa(
            ...
            email_admin='admin@integracion.com',
            on_trial=True
        )
