...
⚠️ IMPORTANTE: Este módulo solo contiene vistas de renderizado (TemplateView).
...
class StaffRequiredMixin(UserPassesTestMixin):
    ...
    def handle_no_permission(self):
        if not self.request.user.is_authenticated:
            return redirect(f"{reverse('admin:login')}?next={self.request.get_full_path()}")
...
class ConsoleTemplateView(LoginRequiredMixin, StaffRequiredMixin, PublicSchemaMixin, TemplateView):
...
def _ensure_public_schema_or_404():
    ...
    ⚠️ NOTA: Esta función NO debe tener decoradores (@login_required, etc.)
...
def impuestos_ingesta_list(request):
    ...
    return render(request, 'console/pages/impuestos/ingesta_list.html', {
...
def impuestos_ingesta_create(request):
    ...
    return render(request, 'console/pages/impuestos/ingesta_create.html', {
...
def impuestos_ingesta_detail(request, pk):
    ...
    return render(request, 'console/pages/impuestos/ingesta_detail.html', {
...
def impuestos_search_results(request):
    ...
    return render(request, 'console/pages/impuestos/search_results.html', {
...
def tipo_save_proxy(request, pk=None):
    ...
    try:
        if pk is None:
            ...
            r = requests.patch(api_url + f"{pk}/", json=data, headers=headers, cookies=request.COOKIES)
        
        status_code = r.status_code
        body = r.json() if r.headers.get("content-type", "").startswith("application/json") else r.text
        
        from django.shortcuts import render
        return render(request, "console/pages/impuestos/forms/tipo_result.html", {
            "status": status_code,
            "body": body,
