...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if active_workers:
                self.stdout.write(self.style.SUCCESS(f"OK: Workers activos: {len(active_workers)}"))
                for worker, tasks in active_workers.items():
                    self.stdout.write(f"   - {worker}: {len(tasks)} tareas activas")
            else:
                self.stdout.write(
                    self.style.ERROR("ERROR: ERROR CRÍTICO: No hay workers de Celery activos")
                )
                self.stdout.write(
                    self.style.WARNING('   💡 Solución: Ejecutar "celery -A config worker -l info"')
                )
                return
        except Exception as e:
            self.stdout.write(self.style.ERROR(f"ERROR: Error al verificar Celery: {e}"))
            return
        ...
        try:
            ...
            if result.state == "PENDING":
                self.stdout.write("")
                self.stdout.write(
                    self.style.WARNING("WARNING:  DIAGNÓSTICO: Tarea en estado PENDING")
                )
                ...
                self.stdout.write(
                    "   4. Hay un problema de conexión con el broker (Redis/RabbitMQ)"
                )
                ...
                try:
                    ...
                    self.stdout.write(
                        self.style.WARNING(f"   WARNING:  No se pudo verificar colas: {e}")
                    )

            elif result.state == "SUCCESS":
                self.stdout.write("")
                self.stdout.write(self.style.SUCCESS("OK: Tarea completada exitosamente"))
                if result.result:
                    self.stdout.write(f"   Resultado: {result.result}")

            elif result.state == "FAILURE":
                self.stdout.write("")
                self.stdout.write(self.style.ERROR("ERROR: Tarea falló"))
                if result.info:
                    self.stdout.write(self.style.ERROR(f"   Error: {result.info}"))

        except Exception as e:
            self.stdout.write(self.style.ERROR(f"ERROR: Error al consultar estado: {e}"))
            import traceback
        ...
        self.stdout.write("")
        self.stdout.write(self.style.SUCCESS("=" * 80))
        self.stdout.write(self.style.SUCCESS("OK: Diagnóstico completado"))
        self.stdout.write(self.style.SUCCESS("=" * 80))
