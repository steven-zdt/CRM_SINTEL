...
class Command(BaseCommand):
        ...
        try:
            ...
            if task_name in task_routes:
                ...
                if queue == "high_priority":
                    ...
                    self.print_warning(
                        f"Tarea '{task_name}' enrutada a cola '{queue}' (esperado: 'high_priority')"
                    )
            else:
        ...
        try:
            ...
            with transaction.atomic():
                client, domain, login_url = crear_tenant(
                    nombre=test_nombre, schema_name=test_schema, admin_user_id=admin_user.id
                )

                # Validar que se creó el Client
                if Client.objects.filter(schema_name=test_schema).exists():
                    self.print_success(f"OK: Client creado: {client.nombre} (ID: {client.id})")
                else:
                    self.print_error("ERROR: Client NO fue creado en la base de datos")
                    return False

                # Validar que se creó el Domain (por señal)
                domain_check = Domain.objects.filter(tenant=client, is_primary=True).first()
                if domain_check:
                    self.print_success(f"OK: Domain creado por señal: {domain_check.domain}")
                else:
                    self.print_error("ERROR: Domain NO fue creado por la señal post_save")
                    return False

                # Validar que se creó el esquema PostgreSQL
                if schema_exists(test_schema):
                    self.print_success(f"OK: Esquema PostgreSQL creado: {test_schema}")
                else:
                    self.print_error(f"ERROR: Esquema PostgreSQL '{test_schema}' NO existe")
                    return False

                # Validar que se creó TenantMembership
                membership = TenantMembership.objects.filter(client=client, user=admin_user).first()
                if membership:
                    self.print_success(
                        f"OK: TenantMembership creado: {admin_user.email} -> {client.nombre} ({membership.rol})"
                    )
                else:
                    self.print_error("ERROR: TenantMembership NO fue creado")
                    return False
    ...
    def prueba_3_celery_eager(self):
        ...
        try:
            ...
            with override_settings(
                ...
                self.print_info(
                    f"Encolando tarea (modo eager): {test_nombre} (schema: {test_schema})"
                )
                ...
                if task.ready():
                    if task.successful():
                        result = task.result
                        self.print_success("OK: Tarea completada exitosamente en modo eager")
                        ...
                        if client:
                            self.print_success(
                                f"OK: Tenant creado: {client.nombre} (ID: {client.id})"
                            )
                            ...
                            self.print_error(
                                "ERROR: Tenant NO fue creado (aunque la tarea fue exitosa)"
                            )
                            return False
                    else:
                        self.print_error(f"ERROR: Tarea falló: {task.info}")
                        return False
                else:
                    self.print_error("ERROR: Tarea no está lista (no debería pasar en modo eager)")
                    return False
    ...
    def prueba_4_cola_real(self):
        ...
        try:
            ...
            if is_eager:
                self.print_warning(
                    "WARNING:  CELERY_TASK_ALWAYS_EAGER está activado. Esta prueba requiere modo real."
                )
            ...
            if active_workers:
                ...
                self.print_error("ERROR: No hay workers activos de Celery")
                self.print_warning(
                    "💡 Solución: Ejecutar 'docker-compose up -d celery' o 'celery -A config worker -l info -Q high_priority,default'"
                )
                return False
            ...
            if stats:
                for worker, worker_stats in stats.items():
                    self.print_info(
                        f"   - {worker}: {worker_stats.get('pool', {}).get('max-concurrency', 'N/A')} workers"
                    )

            # Encolar tarea real
            self.print_info(
                f"Encolando tarea real en cola 'high_priority': {test_nombre} (schema: {test_schema})"
            )
            ...
            task_id = task.id
            self.print_success(f"OK: Tarea encolada. Task ID: {task_id}")
                ...
                for worker, tasks in scheduled.items():
                    for t in tasks:
                        if t.get("request", {}).get("id") == task_id:
                            self.print_success(
                                f"OK: Tarea encontrada en cola programada de {worker}"
                            )
            ...
            if reserved:
                for worker, tasks in reserved.items():
                    for t in tasks:
                        if t.get("request", {}).get("id") == task_id:
                            self.print_success(
                                f"OK: Tarea encontrada en cola reservada de {worker}"
                            )
            ...
            if result.state == "PENDING":
                self.print_error("ERROR: Tarea sigue en PENDING después de 3 segundos")
                ...
                self.print_success("OK: Tarea fue tomada por el worker (estado: STARTED)")
                ...
                if result.ready():
                    if result.successful():
                        self.print_success("OK: Tarea completada exitosamente")
                        ...
                        self.print_error(f"ERROR: Tarea falló: {result.info}")
                        return False
                else:
                    self.print_warning("WARNING:  Tarea no terminó en 60 segundos")
                    return False
                ...
                self.print_warning(f"WARNING:  Estado inesperado: {result.state}")
    ...
    def handle(self, *args, **options):
        ...
        if todas_pasaron:
            self.print_success("OK: TODAS LAS PRUEBAS PASARON")
            ...
            self.print_error("ERROR: ALGUNAS PRUEBAS FALLARON")
