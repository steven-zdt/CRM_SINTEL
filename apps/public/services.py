...
⚠️ v3.3: Service Layer Pattern - Lógica de negocio centralizada
⚠️ API-First: Todos los servicios expuestos vía DRF REST API
⚠️ Seguridad: Validación de hash SHA256 para integridad de documentos
...
def generar_hash_acceso(
    ...
    # Generar HMAC-SHA256
    hash_obj = hmac.new(
        PUBLIC_SHARE_SECRET.encode('utf-8'),
...
def validar_token_documento(
    ...
    ⚠️ API-First: Este método es llamado por los endpoints DRF
...
def obtener_datos_documento_publico(
    ...
    ⚠️ API-First: Este método es llamado por los endpoints DRF públicos
    ⚠️ Seguridad: Solo retorna campos permitidos para vista pública
    ...
    """
    # ⚠️ NOTA: Esta función requiere acceso al schema del tenant
    ...
    if tipo_documento == 'cotizacion':
        estructura_base.update({
            'campos_publicos': [
                ...
                'atencion_a', 'asunto', 'estado',
                'subtotal', 'iva_valor', 'total_neto'
            ]
        })
        ...
        estructura_base.update({
            'campos_publicos': [
                ...
                'fase_actual_display', 'estado_display'
            ]
        })
