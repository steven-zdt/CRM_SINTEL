...
# ⚠️ v2.30: SECRET_KEY debe ser >= 50 caracteres pseudoaleatorios
SECRET_KEY = os.getenv('DJANGO_SECRET_KEY', 'django-insecure-change-me-in-production')
if len(SECRET_KEY) < 50:
    import warnings
    warnings.warn(
        f"⚠️ SECRET_KEY tiene solo {len(SECRET_KEY)} caracteres. "
...
# ⚠️ ESTRUCTURA CRÍTICA: Separación estricta entre SHARED_APPS y TENANT_APPS
# 
# REGLAS DE ORO:
...
# 3. ⚠️ CRÍTICO: NINGUNA app de apps.tenant.* puede estar en SHARED_APPS
...
# ============================================================================
# ⚠️ CRÍTICO: Estas apps SOLO se instalan en el esquema 'public'
# ⚠️ REGLA DE ORO: NINGUNA app que comience con 'apps.tenant.' puede estar aquí
# ⚠️ REGLA DE ORO: TODAS las apps que comienzan con 'apps.public.' DEBEN estar aquí
SHARED_APPS = [
    # django-tenants DEBE ir PRIMERO (requisito obligatorio de django-tenants)
    "django_tenants",
    
    # ⚠️ APPS PÚBLICAS DEL PROYECTO (SOLO esquema public)
    ...
    # ⚠️ NOTA: Estas apps también están en TENANT_APPS porque se necesitan en ambos esquemas
...
# ============================================================================
# ⚠️ CRÍTICO: Estas apps SOLO se instalan en esquemas de tenant (NO en public)
# ⚠️ REGLA DE ORO: NINGUNA app que comience con 'apps.public.' puede estar aquí
# ⚠️ REGLA DE ORO: TODAS las apps que comienzan con 'apps.tenant.' DEBEN estar aquí
TENANT_APPS = [
    # DRF y herramientas API (disponibles en cada tenant para APIs privadas)
    # ⚠️ NOTA: Estas apps también están en SHARED_APPS porque se necesitan en ambos esquemas
    # django-tenants permite que apps estén en ambas listas (se instalan en ambos esquemas)
    ...
    # ⚠️ APPS PRIVADAS: Aplicaciones de negocio por tenant
...
# ============================================================================
# ⚠️ CRÍTICO: Esta fórmula elimina duplicados (apps que están en ambas listas)
# Las apps que están en ambas listas (ej: rest_framework) se instalan en ambos esquemas
# ⚠️ VALIDACIÓN: Verificar que ninguna app.tenant.* esté en SHARED_APPS
INSTALLED_APPS = list(SHARED_APPS) + [app for app in TENANT_APPS if app not in SHARED_APPS]

# ⚠️ VALIDACIÓN DE SEGURIDAD: Verificar segregación estricta de apps
...
if _tenant_apps_in_shared:
    raise ValueError(
        f"❌ ERROR CRÍTICO: Apps de tenant encontradas en SHARED_APPS: {_tenant_apps_in_shared}. "
        f"Las apps de tenant SOLO deben estar en TENANT_APPS."
    )

# 2. Verificar que TODAS las apps públicas NO estén en TENANT_APPS
# ⚠️ CRÍTICO: Cualquier app que comience con 'apps.public.' JAMÁS debe estar en TENANT_APPS
_public_apps_in_tenant = [app for app in TENANT_APPS if app.startswith("apps.public.")]
if _public_apps_in_tenant:
    raise ValueError(
        f"❌ ERROR CRÍTICO: Apps públicas encontradas en TENANT_APPS: {_public_apps_in_tenant}. "
...
if _missing_public_apps:
    raise ValueError(
        f"❌ ERROR CRÍTICO: Apps públicas obligatorias faltantes en SHARED_APPS: {_missing_public_apps}. "
...
if _missing_tenant_apps:
    raise ValueError(
        f"❌ ERROR CRÍTICO: Apps privadas obligatorias faltantes en TENANT_APPS: {_missing_tenant_apps}. "
        f"Estas apps DEBEN estar en TENANT_APPS."
    )

# ⚠️ CONFIGURACIÓN CRÍTICA: Orden de middleware para django-tenants
# ⚠️ ESTÁNDAR: Puerto 80 (HTTP) - ForceNoPortMiddleware normaliza HTTP_HOST antes de TenantMainMiddleware
# ⚠️ v2.26: Orden optimizado para enrutamiento estable por hostname
# 
# Orden crítico:
...
MIDDLEWARE = [
    ...
    'apps.public.core.middleware.ForceNoPortMiddleware',  # ✅ ESTÁNDAR: Normaliza HTTP_HOST eliminando puerto (ANTES de TenantMainMiddleware)
    'django_tenants.middleware.main.TenantMainMiddleware',  # ✅ CRÍTICO: Identifica tenant usando HTTP_HOST normalizado y selecciona URLConf (ROOT_URLCONF o TENANT_URLCONF)
    'apps.public.tenants.middleware_urlconf.TenantSecurityAndURLConfMiddleware',  # ✅ SEGURIDAD + URLConf: Protege ámbito público y establece request.urlconf (después de TenantMainMiddleware)
    'apps.public.tenants.middleware.TenantSecurityMiddleware',  # ✅ SEGURIDAD: Bloquea tenants suspendidos (debe ir después de TenantMainMiddleware)
    'apps.public.core.middleware.RequestContextMiddleware',  # ✅ LOGGING: Añade request_id y schema_name a los logs (después de TenantMainMiddleware para tener request.tenant)
    'apps.public.core.middleware.CSRFTrustedOriginMiddleware',  # ✅ DESARROLLO: Permite dominios arbitrarios en CSRF_TRUSTED_ORIGINS
    'apps.public.core.middleware.HTTPSRedirectMiddleware',  # ✅ DESARROLLO: Redirige HTTPS -> HTTP en DEBUG
    'django.middleware.common.CommonMiddleware',
    'django.middleware.csrf.CsrfViewMiddleware',
    'django.contrib.auth.middleware.AuthenticationMiddleware',  # ✅ CRÍTICO: Proporciona request.user (requerido para require_tenant_membership)
    'apps.public.tenants.authz.require_tenant_membership',  # ✅ SEGURIDAD: Valida membresía del tenant para usuarios autenticados (factory funcional)
    'apps.public.tenants.middleware_admin_guard.block_public_routes_on_tenants',  # ✅ GUARD-RAIL: Bloquea /admin/, /console/ y /api/public/ en cualquier esquema ≠ public
...
TEMPLATES = [
    {
        ...
        'OPTIONS': {
            'context_processors': [
                ...
                'django.template.context_processors.request',  # ✅ CRÍTICO: Requerido por django-tenants
...
# ⚠️ DOMINIO PRINCIPAL Y DEFINITIVO: sintel.com
...
_default_tenant_domain_base = 'sintel.com'  # ⚠️ DOMINIO PRINCIPAL Y DEFINITIVO
...
# ⚠️ ESTÁNDAR: Puerto 80 (HTTP) - Los dominios NO incluyen puerto explícito
...
# ⚠️ SEGURIDAD: Backends de autenticación tenant-aware
...
# ⚠️ CRÍTICO: Requerido para que django-tenants maneje correctamente la creación de esquemas en tests
...
# ⚠️ IMPORTANTE: Django's runserver solo soporta HTTP, NO HTTPS
...
# ⚠️ WARNING: El navegador mostrará un warning si usas HTTP en lugar de HTTPS
...
if DEBUG:
    _dev_hosts = [
        ...
        ".localhost",  # ⚠️ CRÍTICO: Permite subdominios locales (cliente.localhost, etc.)
...
# ⚠️ IMPORTANTE: Permitir credenciales (cookies) en CORS para autenticación por sesión
CORS_ALLOW_CREDENTIALS = True

# 3. CSRF Trusted Origins
# ⚠️ IMPORTANTE: Django NO soporta wildcards (*) en CSRF_TRUSTED_ORIGINS
# ⚠️ SEGURIDAD: Desde Django 4/5 debe incluir esquema (http/https)
...
# ⚠️ v2.30: Nombre de cookie debe coincidir con el que lee el frontend
...
# ⚠️ TENANTS PRIVADOS: Usan /login/ (vista personalizada TenantLoginView)
# ⚠️ ESQUEMA PÚBLICO: Usa /admin/login/ (admin de Django)
# Para tenants privados, la landing page (/) es la vista principal
# ⚠️ CRÍTICO: URL de login para tenants privados (usado por LoginRequiredMixin y decoradores)
...
LOGIN_URL = '/login/'  # ✅ FORZADO: login del TENANT_URLCONF (no /admin/login/)
...
LOGIN_REDIRECT_URL = '/dashboard/'  # ✅ URL por defecto para tenants (se resuelve en TENANT_URLCONF)
LOGOUT_REDIRECT_URL = '/'  # ✅ URL a la que redirigir después del logout (página principal del tenant - landing)

# Django REST Framework (API-First)
# ⚠️ IMPORTANTE: DRF está en TENANT_APPS, por lo que cada tenant tiene sus propias APIs
...
REST_FRAMEWORK = {
    ...
    # Autenticación
    # ⚠️ JWT GLOBAL: APIs REST usan JWT (Authorization: Bearer) por defecto
    # ✅ DEV: SessionAuthentication añadido para workspace (cookies de sesión desde mismo host)
...
# ⚠️ v2.30: Hardening de seguridad JWT
from datetime import timedelta
import secrets
import base64

# ⚠️ v2.30: Configuración de SIGNING_KEY fuerte (32+ bytes)
...
if JWT_SECRET_KEY_ENV:
    ...
    try:
        ...
        if len(decoded_key) < 32:
            import warnings
            warnings.warn(
                f"⚠️ JWT_SECRET_KEY tiene solo {len(decoded_key)} bytes. "
        ...
        if len(JWT_SECRET_KEY_ENV) < 32:
            import warnings
            warnings.warn(
                f"⚠️ JWT_SECRET_KEY tiene solo {len(JWT_SECRET_KEY_ENV)} caracteres. "
    ...
    # ⚠️ ADVERTENCIA: En producción, SIEMPRE usar JWT_SECRET_KEY desde variable de entorno
    if DEBUG:
        # Generar clave de 32 bytes (256 bits) en base64
        jwt_signing_key = base64.b64encode(secrets.token_bytes(32)).decode()
        import warnings
        warnings.warn(
            "⚠️ JWT_SECRET_KEY no configurada. Generada automáticamente para desarrollo. "
        ...
        import warnings
        warnings.warn(
            "⚠️ JWT_SECRET_KEY no configurada en producción. Usando SECRET_KEY como fallback. "
...
SIMPLE_JWT = {
    ...
    # ⚠️ RECOMENDADO: Access tokens cortos (10-20 min), refresh tokens medios (7-30 días)
    ...
    # Algoritmo de firma
    # ⚠️ v2.30: HS256 con clave fuerte (32+ bytes) o migrar a RS256 para producción
...
# ⚠️ NOTA: Para migrar a RS256 en producción, descomentar y configurar:
...
# ============================================================================
# ⚠️ IMPORTANTE: Configuración de email para envío de invitaciones y notificaciones
...
# ⚠️ IMPORTANTE: Si DEFAULT_FROM_EMAIL en .env es ${EMAIL_HOST_USER} o EMAIL_HOST_USER, usar el valor real
...
# ⚠️ IMPORTANTE: La cola puede ser compartida, pero el worker debe cambiar
...
# ⚠️ ROBUSTEZ: Forzar import de módulos de tareas críticos
...
# ============================================================================
# ⚠️ SEGURIDAD: Permite resetear contraseña a unusable para reenvío de tokens
...
# ⚠️ v2.30: Feature flag para migración API-first del dashboard
...
# Feature Flags
# ============================================================================
# ⚠️ ROLLOUT GRADUAL: Feature flags para habilitar funcionalidades de forma controlada
# - Permiten rollback inmediato sin deploy
# - Activación por variable de entorno (FEATURE_*)
# - Por defecto: False (desactivado) para evitar cambios inesperados

# ⚠️ v2.35: Pipeline XML Canónico (SSoT)
# - FEATURE_XML_PIPELINE=True: Usa nuevo pipeline canónico (apps/services/xml_ingest + xml_parser)
# - FEATURE_XML_PIPELINE=False: Usa pipeline legacy (comportamiento actual)
# - Rollout gradual: activar en staging primero, luego producción
FEATURE_XML_PIPELINE = os.getenv("FEATURE_XML_PIPELINE", "false").lower() == "true"

# ⚠️ v2.36: Document Ingest Pipeline Universal (FASE 0)
# - FEATURE_DOCUMENT_PIPELINE=True: Activa el pipeline universal de documentos (document_ingest + document_parser)
...
# ============================================================================
# ⚠️ IMPORTANTE: Configuración de logging para diagnóstico de emails e invitaciones
# ⚠️ OBSERVABILIDAD: request_id es opcional (solo si RequestContextMiddleware está activo)
# - Desarrollo: logging simple a consola, sin request_id
...
LOGGING = {
    ...
    "formatters": {
        # ⚠️ DESARROLLO: Formatter simple sin request_id (no rompe en arranque/import-time)
        "simple": {
        ...
        # ⚠️ OPCIONAL: Formatter con contexto (solo si RequestContextMiddleware está activo)
        # No usar por defecto; el filtro añade request_id/schema_name solo si están disponibles
        "verbose_with_context": {
            "format": "%(asctime)s | %(levelname)s | %(name)s | %(message)s | request_id=%(request_id)s | schema=%(schema_name)s",
        },
        # ⚠️ FASE 5: Formatter JSON estructurado para observabilidad (opcional)
        "json": {
            "format": (
    ...
    "handlers": {
        "console": {
            ...
            # ⚠️ DESARROLLO: Usar formatter simple (sin request_id) por defecto
            # El filtro RequestContextLogFilter añadirá request_id/schema_name si están disponibles
            "formatter": "brief" if DEBUG else "simple",
            "level": "INFO",
        },
    },
    "root": {
    ...
    "loggers": {
        # ⚠️ FASE 5: Canal principal del pipeline de importación (Service Layer)
        "tenant.facturas.import": {
            "handlers": ["console"],
            "level": "INFO",
            "propagate": False,
        },
        # ⚠️ FASE 5: Regla de naturaleza (comparación emisor vs Empresa)
        "tenant.facturas.naturaleza": {
            "handlers": ["console"],
            "level": "INFO",
            "propagate": False,
        },
        # ⚠️ FASE 5: Borrado/rollback
        "tenant.facturas.delete": {
            "handlers": ["console"],
            "level": "INFO",
            "propagate": False,
        },
        ...
        # ⚠️ DESARROLLO: Server logs a WARNING (reducir ruido en dev)
        "django.server": {
            "handlers": ["console"],
            "level": "WARNING" if DEBUG else "INFO",
            "propagate": False,
        },
        # ⚠️ NORMALIZACIÓN: Logger principal de facturas (upload, import, etc.)
        "facturas": {
            "handlers": ["console"],
            "level": "INFO",
            "propagate": False,
        },
        # FORÉNSICA: Logger específico para upload de facturas (mantener compatibilidad)
        "facturas.upload": {
            "handlers": ["console"],
            "level": "INFO",
            "propagate": False,
        ...
        # ⚠️ NORMALIZACIÓN: Logger de Celery
        "mailinbox.api": {
            "handlers": ["console"],
            "level": "INFO",
            "propagate": False,
        ...
        "security.crypto": {
            "handlers": ["console"],
            "level": "WARNING",  # No loguear operaciones de cifrado en INFO
            "propagate": False,
        },
        "celery": {
            "handlers": ["console"],
...
# ⚠️ PRODUCCIÓN: Handler de archivo condicional (solo si DEBUG=False y ruta válida)
# En desarrollo, solo usamos console handler
if not DEBUG:
    import os
    from pathlib import Path
    
    logs_dir = Path(os.getenv("APP_LOG_DIR", "/var/log/app"))
    try:
        ...
        # Adjuntar 'file' solo a loggers que realmente queremos persistir
        # No adjuntar a django.server (Gunicorn maneja el access log)
        for name in ("mailinbox.api", "facturas", "apps.services.document_ingest", "celery", "security.crypto"):
        ...
        logging.warning(f"No se pudo configurar handler de archivo: {e}. Usando solo console.")