...
**Estado:** Sistema funcional - Service Layer Architecture completo en Perfil, Inventario. Correcciones de segmentacion cross-tenant (SEG-1 a SEG-5) aplicadas. Core Membership Bridge ampliado con `check_membership_by_schema`.  
**Fuente Unica de Verdad:** Este documento es la referencia oficial de la arquitectura del proyecto.  
**Ubicacion:** `documentacion/arquitectura_general.md` (UNICA FUENTE DE VERDAD)  
...
1. **Service Layer Pattern**: 
   ...
   - **v2.37+: Patrón `LIST_FIELDS` / `DETAIL_FIELDS` / `qs_list()` / `qs_detail()`**
     ...
     - **qs_list()**: Función que retorna QuerySet optimizado para listados
     - **qs_detail()**: Función que retorna QuerySet optimizado para detalle
     - **Ubicación**: `apps/tenant/{app}/services.py` o `apps/tenant/{app}/services/services.py`
     - **Ejemplo**:
       ```python
...
6. **Single Source of Truth (SSoT)**:
   ...
   - **⚠️ v2.40: ForeignKey a Empresa (Obligatorio)**
     ...
     - **Ejemplo**:
       ...
       ```
   - **✅ v2.61.4: TenantProfile.empresa FK (Garantía SSoT)**
     - **BASE**: TenantProfile es el nexo entre User (public schema) y Empresa (tenant schema)
     - **CAMPO CRÍTICO**: `empresa = ForeignKey('empresa.Empresa', on_delete=CASCADE)`
     - **PROPÓSITO**: Establece contexto multi-tenant explícito para cada usuario
     - **BENEFICIOS**:
       1. **Middleware Context**: `request.user.tenant_profile.empresa_id` proporciona scopping automático
       2. **Service Layer**: Recibe `empresa_id` del perfil para validaciones scoped
       3. **Uniqueness Enforcement**: Constraints `unique_together = [('empresa_id', 'field')]` funcionan correctamente
       4. **No Orphaned Data**: El FK garantiza integridad referencial a nivel BD
       5. **ID Sequences**: Sincronizadas para evitar colisiones en futuro inserts
     - **GARANTÍA**: Todos TENANT_APPS modelos DEBEN vincularse a empresa_id, y TenantProfile.empresa FK es la raíz de esa garantía
     - **DOCUMENTACIÓN COMPLETA**: Ver `documentacion/SOLUCION_COMPLETA_SSoT.md` para detalles de implementación, 3-step sanitation procedure, y validaciones
...
- **Inventario** (`apps/tenant/core/static/core/js/inventario/`)
  - **Feature-Sliced Architecture (v2.61.3):** `features/categorias_list.js`, `categorias_editor.js`, `productos_list.js`, `productos_editor.js`, `servicios_list.js`, `servicios_editor.js`, `activos_list.js`, `activos_editor.js`, `movimientos_list.js`, `movimientos_editor.js`
  - **Service Layer (v2.61.5):** Paquete `services/services.py` con Service Mixins inyectados en ViewSets via herencia
  - **Offcanvas (v2.61.6):** `gestor-offcanvas` estandarizado en Categorias, Productos, Servicios y Activos
  - **Anti-double submit (v2.61.6):** Guards `_listEventsInit` y `_editorInitialized`
  - **Import:** `from apps.tenant.inventario.services.services import ...`
...
### Core API Facade (Arquitectura v2.61.4)

**⚠️ ARQUITECTURA v2.61.4: Core API Facade como Orquestador Único de Workspace**

**Objetivo:**
- Proporcionar endpoints unificados para workspace (`/api/v1/core/v1/{modulo}/`)
- Exponer funcionalidades de apps individuales a través de facades
- Serializers Workspace especializados con URLs absolutas y campos adicionales
- Links centralizados en `CoreLinksViewSet` para descubrimiento de APIs
- Mantener compatibilidad con Gateway Directo (`/api/v1/core/_apps/{modulo}/`)

**Ubicación:** `apps/tenant/core/api/v1/{modulo}/`

**Estructura:**
```
apps/tenant/core/api/v1/
├── facturas/
│   ├── viewsets.py      # FacturaCoreViewSet, ItemFacturaCoreViewSet, NotaCreditoCoreViewSet
│   ├── serializers.py   # Workspace Serializers con URLs absolutas
│   └── urls.py          # Router dedicado
├── cotizaciones/
│   ├── viewsets.py      # CotizacionCoreViewSet, CotizacionItemCoreViewSet, ConfiguracionCotizacionCoreViewSet
│   ├── serializers.py   # Workspace Serializers
│   └── urls.py          # Router dedicado
├── inventario/
│   ├── viewsets.py      # CategoriaItemCoreViewSet, ProductoCoreViewSet, ServicioCoreViewSet, ActivoFijoCoreViewSet, MovimientoInventarioCoreViewSet
│   ├── serializers.py   # Workspace Serializers
│   └── urls.py          # Router dedicado (si aplica)
├── gastos/
│   ├── viewsets.py      # GastoCoreViewSet, ResolucionDIANCoreViewSet
│   └── serializers.py   # Workspace Serializers con adjunto_url
├── contabilidad/
│   ├── viewsets.py      # AsientoContableCoreViewSet, CuentaContableCoreViewSet, etc.
│   └── serializers.py   # Workspace Serializers
├── empleados/
│   ├── viewsets.py      # EmpleadoCoreViewSet, ContratoCoreViewSet, DevengoCoreViewSet
│   └── serializers.py   # Workspace Serializers
├── empresa/
│   ├── viewsets.py      # EmpresaCoreViewSet, MailInboxConfigCoreViewSet
│   └── serializers.py   # Workspace Serializers
└── proyectos/
    ├── viewsets.py      # ProyectoCoreViewSet
    └── serializers.py   # Workspace Serializers
```

**Principios:**
1. **No Copiar Lógica de Negocio**: Los ViewSets facade heredan directamente de los ViewSets de las apps individuales
2. **Herencia de Acciones**: Todas las acciones `@action` se heredan automáticamente
3. **Serializers Especializados**: Workspace Serializers agregan campos adicionales (URLs absolutas, adjuntos, etc.)
4. **Links Centralizados**: `CoreLinksViewSet` proporciona registro centralizado de todas las rutas API/UI

**Endpoints Core API Facade (v2.61.4):**
...
- Acciones heredadas: `importar-ubl`, `summary`, `upload-ubl` (batch processing v2.61.2), `upload-document`, `xml`, `app-response`, `gestor-offcanvas`, etc.
...
- Acciones heredadas: `render-offcanvas`, `exportar-pdf`, `recalcular`, etc.

**Inventario:**
- `/api/v1/core/v1/inventario/categorias/` - CRUD categorías
- `/api/v1/core/v1/inventario/productos/` - CRUD productos
- `/api/v1/core/v1/inventario/servicios/` - CRUD servicios
- `/api/v1/core/v1/inventario/activos/` - CRUD activos fijos
- `/api/v1/core/v1/inventario/movimientos/` - CRUD movimientos

**Gastos:**
- `/api/v1/core/v1/gastos/operativos/` - CRUD gastos
- `/api/v1/core/v1/gastos/resoluciones-dian/` - CRUD resoluciones DIAN
- Acciones heredadas: `summary`, `anular`, `desactivar`, `render-offcanvas`, etc.
...
**Empleados:**
- `/api/v1/core/v1/empleados/gestion/` - CRUD empleados
- `/api/v1/core/v1/empleados/contratos/` - CRUD contratos
- `/api/v1/core/v1/empleados/devengos/` - CRUD devengos

**Empresa:**
- `/api/v1/core/v1/empresa/configuracion/` - CRUD empresa (singleton)
- `/api/v1/core/v1/empresa/mail-inbox/` - CRUD mail inbox config

**Proyectos:**
- `/api/v1/core/v1/proyectos/gestion/` - CRUD proyectos

**Registro en CoreLinksViewSet:**
```python
# apps/tenant/core/api/viewsets.py
{
    "facturas": {
        "api": "/api/v1/core/v1/facturas/facturas/",
        "ui": "/workspace/#facturas"
    },
    "facturas-items": {
        "api": "/api/v1/core/v1/facturas/items-factura/",
        "ui": "/workspace/#facturas"
    },
...
**Ventajas:**
- ✅ Endpoints unificados para workspace
- ✅ Serializers especializados con campos adicionales (URLs absolutas, adjuntos)
- ✅ Descubrimiento de APIs centralizado
- ✅ Compatibilidad con Gateway Directo para acceso directo a apps
- ✅ Herencia automática de acciones `@action`
- ✅ Mantenibilidad mejorada (cambios en apps individuales se reflejan automáticamente)

**Documentación:**
- Facturas: `apps/tenant/facturas/AUDITORIA_FLUJO_COMPLETO.md` ✅ v2.61.2
- Gastos: `apps/tenant/gastos/AUDITORIA_FLUJO_COMPLETO.md` ✅ v2.61.4
- Inventario: `apps/tenant/inventario/AUDITORIA_INVENTARIO.md` ✅ v2.61.4
- Cotizaciones: `apps/tenant/cotizaciones/AUDITORIA_FLUJO_COMPLETO.md` ✅ v2.61
...
**⚠️ v2.60:** Documentación completa disponible en `apps/public/DOCUMENTACION_ONBOARDING_TENANTS.md`
...
- **⚠️ v2.60:** `sintel.com` siempre está incluido en la lista base y se verifica explícitamente para permitir acceso desde el dominio público

**Configuración (v2.23, actualizado v2.60):**
```python
# Desarrollo (DEBUG=True)
# ⚠️ v2.60: Siempre incluir sintel.com en la lista base para permitir acceso desde dominio público
base_allowed_hosts = (
...
# ALLOWED_HOSTS ya incluye sintel.com y .sintel.com desde la lista base
```
...
### Patron Defense-in-Depth: Seguridad Multi-Capa (v2.61.8)

SINTEL implementa un modelo de seguridad en profundidad con **5 capas independientes** que se refuerzan mutuamente. Cada capa puede detener un ataque por si sola, pero juntas garantizan aislamiento total entre tenants.

**Capa 1 - Aislamiento de Esquema PostgreSQL:**
- `django-tenants` resuelve el esquema del tenant por dominio HTTP.
- Cada tenant opera en su propio esquema (`tenant_<schema_name>`).
- Los modelos tenant NUNCA pueden consultar datos de otro esquema sin `_PublicSchemaContext`.
- El esquema `public` contiene solo datos compartidos (`SHARED_APPS`).

**Capa 2 - Middleware de Seguridad (HTTP):**
- `TenantMainMiddleware`: Resuelve tenant por dominio, establece `connection.schema_name`.
- `TenantURLConfMiddleware`: Establece `request.urlconf` correcto (tenant vs public).
- `TenantSecurityMiddleware`: Bloquea acceso a tenants suspendidos (`is_active=False`).
- Orden estricto: Main -> URLConf -> Security (cada uno depende del anterior).

**Capa 3 - Permisos DRF (ViewSet):**
- `IsTenantMember` (OBLIGATORIO en todo ViewSet tenant): Verifica membresia activa via Core Membership Bridge.
- `IsTenantAdminOrReadOnly`: Escritura solo para ADMIN/STAFF.
- `IsTenantProfileAdmin` / `HasTenantRole`: Permisos granulares por rol dentro de la app.
- `BaseTenantViewSet` define `lookup_field="uuid"` para prevenir enumeracion de PKs.

**Capa 4 - Service Layer (Business Service):**
- **Double Semantic Verification (DSV):** Antes de persistir, el `business_service.py` valida que TODAS las entidades/ForeignKeys referenciadas en el payload pertenezcan al tenant actual (prevencion IDOR).
- **Membership verification:** Funciones como `_verify_user_membership()` invocan `check_membership_by_schema()` del Core Membership Bridge para confirmar membresia incluso cuando no hay `request.tenant` disponible.
- **MUTABLE_FIELDS allowlist:** Constante que restringe que campos pueden ser modificados via API, rechazando silenciosamente campos protegidos (`empresa_id`, `user_id`, `rol`, etc.).

**Capa 5 - ORM y Modelo Base:**
- `SintelTenantBaseModel.save()` lanza `ValueError` si `empresa_id` es `None`.
- Indices obligatorios `[empresa]` y `[empresa, -created_at]` en todo modelo tenant.
- `get_queryset()` en ViewSets SIEMPRE filtra por `empresa_id` del tenant actual.
- Queries con `.only()` / `.defer()` obligatorio (Zero Waste).

**Ejemplo de flujo completo (mutacion PATCH en Perfil):**
```
Request PATCH /api/v1/perfil/perfiles/<uuid>/
  -> Capa 2: TenantMainMiddleware resuelve schema
  -> Capa 3: IsTenantMember verifica membresia
  -> Capa 3: IsTenantProfileAdmin verifica rol ADMIN
  -> Capa 4: PerfilBusinessService._verify_user_membership() confirma Target pertenece al tenant
  -> Capa 4: MUTABLE_FIELDS filtra campos no permitidos
  -> Capa 5: CRUDService persiste con @transaction.atomic, empresa_id validado en save()
  -> Response 200 OK
```
...
#### 9. `apps.tenant.perfil` ✅ IMPLEMENTADO (Service Layer Architecture v2.61.8)
**Funcion:** Perfil privado del colaborador (por tenant)
- Datos especificos del usuario dentro del tenant (cargo, departamento, telefono corporativo)
- Avatar del colaborador
- Configuracion de UI (preferencias) almacenada en JSONField
- Relacion OneToOneField con User global (sin "contaminar" el modelo de usuario)
- Sistema de roles por tenant (ADMIN, OPERADOR, VISOR) con permisos granulares
- Auto-Admin Onboarding: asignacion automatica de rol ADMIN al owner del tenant
- Creacion de usuarios desde el modulo (Nuevo Perfil) con username autogenerado
- Offcanvas FSD: crear, editar, detalle (Feature-Sliced Design)

**Estado:** ✅ Service Layer Architecture completo con correcciones de seguridad SEG-1 a SEG-5
**Esquema:** `tenant_<schema_name>`
**Aislamiento:** Cada tenant tiene sus propios perfiles de colaboradores

**Modelos:**
- `TenantProfile(SintelTenantBaseModel)`: Perfil del colaborador
  - `user`: OneToOneField(AUTH_USER_MODEL, `related_name='tenant_profile'`)
  - `empresa`: ForeignKey('empresa.Empresa', `related_name='perfiles_tenant'`)
  - `cargo`, `departamento`, `telefono_corporativo`: Datos corporativos
  - `avatar`: ImageField (upload_to='perfiles/avatars/')
  - `configuracion`: JSONField (preferencias UI)
  - `rol`: CharField(choices=RolTenant.choices, default='OPERADOR', db_index=True)
  - Constraint: `unique_together = ('user', 'empresa')` - un perfil por user+tenant
- `RolTenant`: TextChoices - `ADMIN`, `OPERADOR`, `VISOR`

**Service Layer Architecture (paquete `services/`):**

| Archivo | Responsabilidad |
|---|---|
| `selectors.py` | `PerfilSelector` - QuerySets read-only con LIST_FIELDS/DETAIL_FIELDS, `@staticmethod` |
| `crud_service.py` | `PerfilCRUDService` - Persistencia atomica, MUTABLE_FIELDS allowlist [SEG-3] |
| `business_service.py` | `PerfilBusinessService` - Orquestacion, DSV, membership verification [SEG-1/4] |
| `api_mixins.py` | `PerfilServiceMixin` - Inyeccion en ViewSet (`get_qs_list`, `service_crear_perfil`) |
| `__init__.py` | Reexporta `PerfilSelector`, `PerfilCRUDService`, `PerfilBusinessService`, `RolTenant` |
| `perfil_service.py` | Legacy compatibility: `get_or_create_profile`, `read_profile`, etc. |

**Metodos principales de PerfilBusinessService:**
- `_is_tenant_primary_admin(user)`: Verifica admin primario (Tier 1: owner_email, Tier 2: TenantMembership)
- `_verify_user_membership(user)`: [SEG-1] Defense-in-depth: verifica TenantMembership activa via `check_membership_by_schema()`
- `get_or_initialize_profile(user, empresa)`: Auto-Admin Onboarding con guard de membership [SEG-1]
- `create_profile_for_user(empresa, data)`: Crea User global + perfil tenant con membership check [SEG-4]
- `update_user_profile(user, empresa, data)`: Actualizacion con anti-IDOR DSV en empresa/user FKs
- `update_profile_by_id(profile_id, empresa, data)`: Actualizacion admin-level con anti-IDOR
- `assign_rol(profile_id, empresa, new_rol)`: [RULE 13] DSV completo, valida RolTenant choices
- `delete_profile(profile_id, empresa)`: Eliminacion con validacion de tenant

**MUTABLE_FIELDS (PerfilCRUDService) [SEG-3]:**
```python
...
```
Campos protegidos (`user`, `empresa`, `rol`, `id`) son silenciosamente rechazados por `update_profile()`.

**Permisos (`apps/tenant/perfil/api/permissions.py`):**
- `HasTenantRole`: Verifica `required_roles` contra `perfil.rol` con DSV empresa
- `IsTenantProfileAdmin`: Requiere rol==ADMIN con DSV completo
- `IsTenantProfileOperadorOrAdmin`: Requiere ADMIN o OPERADOR
- `IsTenantAdmin`: Alias semantico de `IsTenantProfileAdmin`
- `get_available_actions(perfil)`: Retorna acciones permitidas segun ROLE_ACTIONS
- `get_permissions_context(perfil, user)`: Contexto UI con `can_edit_users`, `is_owner`, etc.

**APIs REST (PerfilViewSet):**

| HTTP | Endpoint | Permiso | Descripcion |
|------|----------|---------|-------------|
| GET | `/api/v1/perfil/perfiles/` | IsTenantMember | Lista perfiles del tenant |
| POST | `/api/v1/perfil/perfiles/` | IsTenantProfileAdmin | Crear usuario + perfil (Nuevo Perfil) |
| GET | `/api/v1/perfil/perfiles/{id}/` | IsTenantMember | Detalle de perfil |
| PUT | `/api/v1/perfil/perfiles/{id}/` | IsTenantProfileAdmin [SEG-2] | Actualizar perfil completo |
| PATCH | `/api/v1/perfil/perfiles/{id}/` | IsTenantProfileAdmin [SEG-2] | Actualizar perfil parcial |
| DELETE | `/api/v1/perfil/perfiles/{id}/` | IsTenantProfileAdmin | Eliminar perfil |
| GET | `/api/v1/perfil/perfiles/me/` | IsAuthenticated | Perfil del usuario actual (auto-crea si no existe) |
| PATCH | `/api/v1/perfil/perfiles/me/` | IsAuthenticated | Actualizar perfil propio (campos limitados) |
| PATCH | `/api/v1/perfil/perfiles/{id}/assign-rol/` | IsTenantProfileAdmin | Asignar rol (DSV + RolTenant) |
| GET | `/api/v1/perfil/perfiles/render-offcanvas/crear/` | IsTenantMember | Offcanvas HTML crear perfil |
| GET | `/api/v1/perfil/perfiles/{id}/render-offcanvas/editar/` | IsTenantMember | Offcanvas HTML editar perfil |
| GET | `/api/v1/perfil/perfiles/{id}/render-offcanvas/detalle/` | IsTenantMember | Offcanvas HTML detalle perfil |

**Serializers:**
- `TenantProfileSerializer`: Serializer principal con nested user data, `available_actions`, `permissions_context`
- `TenantProfileMeUpdateSerializer`: Partial update para `/me/` (solo cargo, departamento, telefono, config)
- `TenantProfileRolSerializer`: Minimized write surface para `assign_rol` (solo campo `rol`)
- `UserNestedSerializer`: Read-only nested user data (id, username, email, first_name, last_name)

**Correcciones de Seguridad Aplicadas (v2.61.8):**

| ID | Severidad | Descripcion | Archivo |
|----|-----------|-------------|--------|
| SEG-1 | P0 | Membership guard en `get_or_initialize_profile()` via `check_membership_by_schema()` | business_service.py |
| SEG-2 | P1 | ADMIN guard en `_update_profile()` - solo ADMIN puede editar perfiles de otros | viewsets.py |
| SEG-3 | P2 | `MUTABLE_FIELDS` allowlist en `update_profile()` - rechaza `empresa_id`, `rol`, `user_id` | crud_service.py |
| SEG-4 | P2 | Membership check en `create_profile_for_user()` antes de crear perfil para user existente | business_service.py |
| SEG-5 | P3 | `connection.schema_name` incluido en todos los logger calls (observabilidad cross-tenant) | business_service.py |

**Frontend (Feature-Sliced Design):**
- Templates: `apps/tenant/perfil/templates/tenant/perfil/`
  - `offcanvas_crear_perfil.html`, `offcanvas_editar_perfil.html`, `offcanvas_detalle_perfil.html`
  - `list_perfiles.html`, `partials/table.html`, `partials/assets_perfil.html`
- JS: `apps/tenant/perfil/static/perfil/js/`
  - `perfil.main.js`: Orquestador central (namespace `window.Sintel.Perfil`)
  - `perfil.api.js`: SSoT de URLs y endpoints (Gateway Directo)
  - `perfil.table.js`: Tabulator + paginacion remota + acciones
  - `perfil.ui.js`: Offcanvas lifecycle, formularios, DOM Shield
  - `perfil.utils.js`: Formateo, validacion

**Ubicacion:** `apps/tenant/perfil/`
**Documentacion:** `apps/tenant/perfil/AUDITORIA_FLUJO_COMPLETO.md`
...
**Estructura de Servicios Core (v2.61.8):**
- `apps/tenant/core/services/__init__.py`: Punto de entrada del paquete.
- `apps/tenant/core/services/auth_service.py`: SSoT de autenticacion (login, logout, password-reset).
- `apps/tenant/core/services/activation_service.py`: Servicio de activacion de cuentas/tenants.
- `apps/tenant/core/services/password_reset.py`: Flujo de password-reset autonomo.
- `apps/tenant/core/services/landing_adapter.py`: Adaptador que consume servicios de Landing (info, activate).
- `apps/tenant/core/services/landing.py`: Logica de landing pages del tenant.
- `apps/tenant/core/services/orchestration.py`: Orquestacion de datos de multiples TENANT_APPS.
- `apps/tenant/core/services/dashboard.py`: Composicion de datos para dashboard.
- `apps/tenant/core/services/empresa.py`: Servicios de empresa (SSoT).
- `apps/tenant/core/services/empresa_adapter.py`: Adaptador puente para datos de empresa cross-app.
- `apps/tenant/core/services/facturas.py`: Servicios de facturas.
- `apps/tenant/core/services/facturas_maildigester_adapter.py`: Adaptador para ingesta de facturas via email.
- `apps/tenant/core/services/contabilidad.py`: Servicios de contabilidad.
- `apps/tenant/core/services/contabilidad_adapter.py`: Adaptador puente para datos contables cross-app.
- `apps/tenant/core/services/perfil.py`: Servicios de perfil.
- `apps/tenant/core/services/perfil_adapter.py`: Adaptador puente para datos de perfil cross-app.
- `apps/tenant/core/services/selectors.py`: Consultas GET optimizadas (read-only) para Core.
- `apps/tenant/core/services/api_mixins.py`: ServiceMixin para ViewSets de Core.
- `apps/tenant/core/services/membership.py`: **Core Membership Bridge** - UNICA interfaz autorizada para queries cross-schema al esquema public (ver seccion dedicada abajo).

##### Core Membership Bridge (`membership.py`) - Aislamiento Cross-Schema (AGENTS.md Regla 17)

**Proposito:** Encapsula TODA interaccion entre apps tenant y el esquema publico (`apps.public`). Ningun otro modulo tenant (excepto `apps/tenant/api/`) puede importar directamente desde `apps.public`.

**Mecanismo de aislamiento - `_PublicSchemaContext`:**
Context manager que cambia al esquema `public` de PostgreSQL y restaura el esquema original al salir, incluso ante excepciones. Solo cambia si el esquema actual no es ya `public` (evita operaciones redundantes).
...
**API Surface (funciones exportadas):**

| Funcion | Firma | Retorno | Descripcion |
|---------|-------|---------|-------------|
| `check_membership` | `(user, tenant)` | `TenantMembership \| None` | Verifica membresia activa. Retorna instancia o `None`. |
| `check_membership_exists` | `(user, tenant)` | `bool` | Wrapper booleano de `check_membership`. |
| `check_membership_by_schema` | `(user_id, schema_name=None)` | `bool` | **v2.61.8** - Verifica membresia por PK y schema_name. Para uso desde Service Layer sin `request.tenant`. Si `schema_name` es `None`, usa `connection.schema_name`. Retorna `True` si schema es public. |
| `check_admin_membership` | `(user, tenant)` | `TenantMembership \| None` | Verifica membresia con rol ADMIN o STAFF. |
| `check_primary_admin` | `(user_id, schema_name=None)` | `bool` | Verifica si el usuario es admin primario del tenant (`is_primary_admin=True`). |
| `get_user_role` | `(user, tenant)` | `str \| None` | Retorna rol mas alto: ADMIN > STAFF > USER. Prioridad: `{ADMIN:3, STAFF:2, USER:1}`. |
| `get_primary_domain` | `(tenant)` | `str \| None` | Retorna dominio primario del tenant via modelo `Domain`. |
| `verify_invitation` | `(token)` | `dict \| None` | Valida token de invitacion. Delega a `apps.public.tenants.services.invitations`. |

**Patrones de implementacion:**
- Todos los imports de `apps.public` son **lazy** (dentro de `try/except` en el cuerpo de cada funcion, nunca a nivel de modulo).
- Todas las queries usan `_PublicSchemaContext` para garantizar restauracion del esquema.
- Manejo de errores: todas las funciones loguean excepciones con contexto y retornan defaults seguros (`None` o `False`).
- Roles soportados: `ADMIN`, `STAFF`, `USER`.

**Regla de extension:** Si una app tenant necesita una nueva consulta al esquema publico, se DEBE agregar una nueva funcion al bridge (`membership.py`). Queda PROHIBIDO crear imports directos como alternativa.
...
- `POST /api/v1/core/auth/password-reset/request/` - Solicitar reset ⚠️ **PENDIENTE**
- `POST /api/v1/core/auth/password-reset/validate/` - Validar token ⚠️ **PENDIENTE**
- `POST /api/v1/core/auth/password-reset/confirm/` - Confirmar reset ⚠️ **PENDIENTE**
...
- **ROOT_URLCONF (`config.urls_public.py`)**: Se usa cuando se accede al dominio público (sintel.com)
  ...
  - Vista raíz: `PublicIndexView` (TemplateView que renderiza landing page profesional o redirige según estado del usuario)
    - **⚠️ v2.60:** `PublicIndexView` es una `TemplateView` que renderiza `public/core/index.html`
    - **Lógica de redirección inteligente:**
      - Usuario staff/superuser autenticado → redirige a `/console/` (Consola de administración)
      - Usuario normal autenticado → redirige a `/admin/login/` (No tiene acceso a consola)
      - Usuario anónimo → renderiza landing page profesional con Bootstrap 5
    - **Características de la landing page:**
      - Hero section con gradiente y call-to-action
      - Sección de características (Features) con 6 tarjetas
      - CTA final con botón destacado "Acceso Administrador SINTEL"
      - Footer informativo
...
1. **Dominio público (`sintel.com`)** ⚠️ v2.60
   - TenantMainMiddleware establece el esquema `public`.
   - Django usa `ROOT_URLCONF = 'config.urls_public'`.
   - `PublicIndexView` (TemplateView) decide:
     - Usuario staff/superuser autenticado → redirige a `/console/` (Consola de administración)
     - Usuario normal autenticado → redirige a `/admin/login/` (No tiene acceso a consola)
     - Usuario anónimo → renderiza landing page profesional (`public/core/index.html`)
       - Hero section con título "SINTEL - Plataforma ERP/CRM Multi-Tenant en la Nube"
       - Features: Aislamiento Multi-Tenant, Escalabilidad, Gestión Centralizada, API-First, Seguridad, Contabilidad Invisible
       - CTA: Botón "Acceso Administrador SINTEL" → `/admin/login/`
...
**Consola de Administración (Dominio Público - sintel.com):** ⚠️ v2.60
- `/`: Root - `PublicIndexView` (TemplateView)
  - Usuario staff/superuser autenticado → redirige a `/console/`
  - Usuario normal autenticado → redirige a `/admin/login/`
  - Usuario anónimo → renderiza landing page profesional (`public/core/index.html`)
    - Hero section con título y descripción
    - Features: Multi-Tenant, Escalabilidad, Gestión Centralizada, API-First, Seguridad, Contabilidad Invisible
    - CTA: Botón "Acceso Administrador SINTEL" → `/admin/login/`
...
- `/console/tenants/new/`: Formulario de creación de tenant (onboarding)
...
- `config/urls_public.py`: URLs para esquema público (sintel.com) - **ROOT_URLCONF**
  - `PublicIndexView` (TemplateView): Renderiza landing page profesional o redirige según estado del usuario ⚠️ v2.60
    - Template: `public/core/index.html` (Bootstrap 5)
    - Lógica: Staff → `/console/`, Normal → `/admin/login/`, Anónimo → Landing page
...
5. **Crear tenant público y dominios:**
   ```bash
   make setup
   # O ejecutar directamente el script:
   docker exec -it crm_sintel-web-1 python scripts/setup_public_domain.py
   ```
   **⚠️ v2.60:** El script `scripts/setup_public_domain.py` crea/verifica:
   - Tenant público (`schema_name='public'`, `nombre='SINTEL Public'`)
   - Dominios: `sintel.com` (primario), `localhost`, `127.0.0.1`, `0.0.0.0`
   - Idempotente: puede ejecutarse múltiples veces sin problemas
   - **Configuración PYTHONPATH:** Calcula BASE_DIR correctamente e inyecta en `sys.path` antes de `django.setup()`
   - **Campo correcto:** Usa `nombre` (no `name`) para el modelo `Client`
