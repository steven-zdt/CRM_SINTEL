...
def check_debug():
    ...
    if settings.DEBUG:
        print("  ❌ ERROR: DEBUG=True. Debe estar en False en producción.")
        return False
    print("  ✅ DEBUG=False")
    return True
...
def check_allowed_hosts():
    ...
    if missing:
        print(f"  ❌ ERROR: Faltan hosts requeridos: {missing}")
        print(f"  ALLOWED_HOSTS actual: {settings.ALLOWED_HOSTS}")
        return False
    
    print(f"  ✅ ALLOWED_HOSTS contiene los dominios requeridos")
...
def check_csrf_trusted_origins():
    ...
    if not has_https and not settings.DEBUG:
        print("  ⚠️  ADVERTENCIA: No se encontró 'https://sintel.com' en CSRF_TRUSTED_ORIGINS")
        print("  NOTA: El middleware CSRFTrustedOriginMiddleware puede manejar dominios dinámicos")
    
    print(f"  ✅ CSRF_TRUSTED_ORIGINS configurado")
...
def check_middleware_order():
    ...
    if tenant_middleware_idx is None:
        print("  ❌ ERROR: TenantMainMiddleware no encontrado en MIDDLEWARE")
        return False
    
    if session_middleware_idx is not None and tenant_middleware_idx < session_middleware_idx:
        print("  ⚠️  ADVERTENCIA: TenantMainMiddleware debería ir después de SessionMiddleware")
        print("  (Aunque esto puede ser intencional según tu configuración)")
    
    print(f"  ✅ TenantMainMiddleware encontrado en posición {tenant_middleware_idx + 1}")
    return True
...
def check_urlconf():
    ...
    if not hasattr(settings, 'ROOT_URLCONF'):
        print("  ❌ ERROR: ROOT_URLCONF no está configurado")
        return False
    
    if not hasattr(settings, 'TENANT_URLCONF'):
        print("  ❌ ERROR: TENANT_URLCONF no está configurado")
        return False
    
    if settings.ROOT_URLCONF != 'config.urls_public':
        print(f"  ⚠️  ADVERTENCIA: ROOT_URLCONF={settings.ROOT_URLCONF}, esperado: config.urls_public")
    
    if settings.TENANT_URLCONF != 'config.urls_tenant':
        print(f"  ⚠️  ADVERTENCIA: TENANT_URLCONF={settings.TENANT_URLCONF}, esperado: config.urls_tenant")
...
def check_domains():
    ...
    try:
        ...
        print(f"  ✅ Tenant público encontrado: {public_client.nombre}")
    ...
    if not public_domain:
        print("  ⚠️  ADVERTENCIA: No se encontró dominio primario para el tenant público")
        print("  NOTA: Puede ser necesario crear Domain(domain='sintel.com', tenant=public_client, is_primary=True)")
    else:
        print(f"  ✅ Dominio público encontrado: {public_domain.domain}")
