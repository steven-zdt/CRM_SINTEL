...
def prueba_2_creacion_sincrona():
    ...
    try:
        ...
        with transaction.atomic():
            ...
            if Client.objects.filter(schema_name=test_schema).exists():
                print_success(f"✅ Client creado: {client.nombre} (ID: {client.id})")
            else:
                print_error("❌ Client NO fue creado en la base de datos")
                return False
            
            # Validar que se creó el Domain (por señal)
            domain_check = Domain.objects.filter(tenant=client, is_primary=True).first()
            if domain_check:
                print_success(f"✅ Domain creado por señal: {domain_check.domain}")
            else:
                print_error("❌ Domain NO fue creado por la señal post_save")
                return False
            
            # Validar que se creó el esquema PostgreSQL
            if schema_exists(test_schema):
                print_success(f"✅ Esquema PostgreSQL creado: {test_schema}")
            else:
                print_error(f"❌ Esquema PostgreSQL '{test_schema}' NO existe")
                return False
            ...
            if membership:
                print_success(f"✅ TenantMembership creado: {admin_user.email} -> {client.nombre} ({membership.rol})")
            else:
                print_error("❌ TenantMembership NO fue creado")
                return False
            ...
            if task.ready():
                if task.successful():
                    ...
                    if client:
                        ...
                        print_error("❌ Tenant NO fue creado (aunque la tarea fue exitosa)")
                        return False
                else:
                    print_error(f"❌ Tarea falló: {task.info}")
                    return False
            else:
                print_error("❌ Tarea no está lista (no debería pasar en modo eager)")
                return False
...
def prueba_4_cola_real():
    ...
    try:
        ...
        if is_eager:
            print_warning("⚠️  CELERY_TASK_ALWAYS_EAGER está activado. Esta prueba requiere modo real.")
        ...
        if active_workers:
            ...
            print_error("❌ No hay workers activos de Celery")
            print_warning("💡 Solución: Ejecutar 'docker-compose up -d celery' o 'celery -A config worker -l info -Q high_priority,default'")
            return False
        ...
        task_id = task.id
        print_success(f"✅ Tarea encolada. Task ID: {task_id}")
            ...
            for worker, tasks in scheduled.items():
                for t in tasks:
                    if t.get('request', {}).get('id') == task_id:
                        print_success(f"✅ Tarea encontrada en cola programada de {worker}")
        ...
        if reserved:
            for worker, tasks in reserved.items():
                for t in tasks:
                    if t.get('request', {}).get('id') == task_id:
                        print_success(f"✅ Tarea encontrada en cola reservada de {worker}")
        ...
        if result.state == 'PENDING':
            print_error("❌ Tarea sigue en PENDING después de 3 segundos")
            print_warning("💡 DIAGNÓSTICO: El worker NO está procesando la tarea")
            ...
            print_info("💡 Solución: Verificar comando del worker en docker-compose.yaml")
            ...
            print_success("✅ Tarea fue tomada por el worker (estado: STARTED)")
            ...
            if result.ready():
                if result.successful():
                    ...
                    print_error(f"❌ Tarea falló: {result.info}")
                    return False
            else:
                print_warning("⚠️  Tarea no terminó en 60 segundos")
                return False
            ...
            print_warning(f"⚠️  Estado inesperado: {result.state}")
