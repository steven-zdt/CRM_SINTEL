...
⚠️ v2.37: Ejecutar ANTES de modificar el módulo Empresa para validar integridad.
...
Salida:
    ...
    - ❌ FAIL: Algunos checks fallaron (revisar documentación)
"""
import os
import sys
import re
...
def check_serializer():
    ...
    if expected_fields != actual_fields:
        print(f"  ❌ FAIL: Campos no coinciden")
    ...
    if prohibited_fields & actual_fields:
        print(f"  ❌ FAIL: Se expusieron campos prohibidos: {prohibited_fields & actual_fields}")
        return False
    
    print("  ✅ PASS: Serializer correcto")
    return True
...
def check_html():
    ...
    if not html_path.exists():
        print(f"  ❌ FAIL: Archivo no encontrado: {html_path}")
        return False
    ...
    if 'id="tabla-empresa"' not in content:
        print("  ❌ FAIL: No se encontró id='tabla-empresa'")
        return False
    ...
    if len(th_texts) != 7:
        print(f"  ❌ FAIL: Debe tener 7 columnas, pero tiene {len(th_texts)}")
        return False
    
    for i, expected in enumerate(expected_columns):
        if expected not in th_texts[i]:
            print(f"  ❌ FAIL: Columna {i+1} debe ser '{expected}', pero es '{th_texts[i]}'")
            return False
    
    print("  ✅ PASS: HTML correcto")
    return True
...
def check_js():
    ...
    if not js_path.exists():
        print(f"  ❌ FAIL: Archivo no encontrado: {js_path}")
        return False
    ...
    if "TABLE_ID = '#tabla-empresa'" not in content and "const TABLE_ID = '#tabla-empresa'" not in content:
        print("  ❌ FAIL: No se encontró TABLE_ID = '#tabla-empresa'")
        return False
    ...
    if 'async function fetchEmpresaList()' not in content and 'function fetchEmpresaList()' not in content:
        print("  ❌ FAIL: No se encontró función fetchEmpresaList()")
        return False
    ...
    for campo in campos_serializer:
        ...
        if not re.search(pattern, content, re.IGNORECASE):
            print(f"  ❌ FAIL: Campo '{campo}' no está en las columnas de JS")
            return False
    
    print("  ✅ PASS: JS correcto")
    return True
...
def check_endpoint():
    ...
    if response.status_code != 200:
        print(f"  ❌ FAIL: Endpoint retornó {response.status_code}")
        return False
    
    data = response.json()
    if not isinstance(data, list):
        print(f"  ❌ FAIL: Endpoint debe retornar array, pero retornó {type(data)}")
        return False
    
    print("  ✅ PASS: Endpoint correcto")
    return True
