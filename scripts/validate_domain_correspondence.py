...
for client in clients:
    ...
    if primary_domains:
        ...
        print(f"   ✅ Dominio Primary en BD: {primary_domain_db}")
    else:
        primary_domain_db = None
        print(f"   ⚠️  No hay dominio primary en BD")
    ...
    if primary_domain_db:
        if domain_from_serializer == primary_domain_db:
            print(f"   ✅ CORRECTO: El dominio del serializer corresponde al de la BD")
        else:
        ...
        print(f"   ✅ CORRECTO: No hay dominio en BD y serializer retorna '-'")
    else:
...
if not errors and not warnings:
    print("\n✅ TODOS LOS DOMINIOS CORRESPONDEN CORRECTAMENTE")
    sys.exit(0)
else:
