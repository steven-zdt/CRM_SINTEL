...
def test_email_backend():
    ...
    try:
        ...
        print(f"  [WARNING]  Error de conexión: {e}")
        if settings.EMAIL_BACKEND == 'django.core.mail.backends.console.EmailBackend':
            print("  INFO:  Backend de consola (normal en desarrollo)")
            return True
        return False
...
def test_template_rendering():
    ...
    try:
        print("\n[NOTE] Renderizando template HTML...")
        html = render_to_string('emails/owner_invitation.html', context)
        print(f"  [OK] Template HTML renderizado ({len(html)} caracteres)")
        
        print("\n[NOTE] Renderizando template texto plano...")
        text = render_to_string('emails/owner_invitation.txt', context)
        print(f"  [OK] Template texto plano renderizado ({len(text)} caracteres)")
        
        return True
    except Exception as e:
        print(f"  [ERROR] Error renderizando templates: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def test_simple_email(dest_email: str):
    ...
    try:
        ...
        if settings.EMAIL_BACKEND == 'django.core.mail.backends.console.EmailBackend':
            print("  INFO:  (En desarrollo, el email se muestra arriba en la consola)")
        return True
    except Exception as e:
...
def test_invitation_email(dest_email: str):
    ...
    with schema_context('public'):
        ...
        try:
            ...
            if not tenant:
                print("  [ERROR] No se encontró tenant público")
                return False
        except Exception as e:
            print(f"  [ERROR] Error obteniendo tenant: {e}")
            return False
        ...
        try:
            ...
            if user:
                print(f"  [OK] Usuario de prueba existente: {user.email}")
            else:
                ...
                print(f"  [OK] Usuario de prueba creado: {user.email}")
        except Exception as e:
            print(f"  [ERROR] Error obteniendo/creando usuario: {e}")
            import traceback
            traceback.print_exc()
            return False
        ...
        try:
            ...
            print(f"  [OK] URL construida: {activation_url}")
        except Exception as e:
        ...
        try:
            ...
            if result:
                print(f"  [OK] Email de invitación enviado exitosamente")
                print(f"  📧 Destinatario: {user.email}")
                print(f"  🔗 URL de activación: {activation_url}")
                return True
            else:
                print(f"  [WARNING]  Email NO enviado (pero proceso completado)")
                return False
        except Exception as e:
            print(f"  [ERROR] Error en send_invitation_email: {e}")
            import traceback
            traceback.print_exc()
            return False
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        print("[ERROR] ERROR: No se puede determinar email de destino.")
