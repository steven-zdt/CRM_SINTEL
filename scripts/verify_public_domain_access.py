...
def check_public_tenant():
    ...
    try:
        ...
        print(f"  ✅ Tenant público encontrado: {public.nombre} (schema: {public.schema_name})")
        return public
...
def check_public_domain(public):
    ...
    try:
        ...
        print(f"  ✅ Dominio 'sintel.com' encontrado:")
        print(f"     - Tenant: {domain.tenant.nombre}")
        print(f"     - Primary: {domain.is_primary}")
        return domain
    except Domain.DoesNotExist:
        print("  ❌ ERROR: Dominio 'sintel.com' no encontrado para el tenant público")
...
def check_allowed_hosts():
    ...
    if has_sintel:
        print(f"  ✅ ALLOWED_HOSTS incluye sintel.com")
        print(f"     ALLOWED_HOSTS: {allowed_hosts}")
    else:
        print(f"  ❌ ERROR: ALLOWED_HOSTS no incluye sintel.com")
...
def check_urlconf():
    ...
    if root_urlconf == 'config.urls_public':
        ...
        print(f"  ⚠️  ROOT_URLCONF = '{root_urlconf}' (esperado: 'config.urls_public')")
    
    # Verificar que la ruta /console/tenants/ está disponible
    try:
        url = reverse('console:tenants-list')
        print(f"  ✅ Ruta /console/tenants/ disponible: {url}")
        return True
    except NoReverseMatch:
        print(f"  ❌ ERROR: Ruta /console/tenants/ no encontrada")
...
def check_middleware_order():
    ...
    try:
        ...
        if force_no_port_idx < tenant_main_idx:
            print(f"  ✅ ForceNoPortMiddleware ({force_no_port_idx}) está antes de TenantMainMiddleware ({tenant_main_idx})")
        else:
            print(f"  ❌ ERROR: ForceNoPortMiddleware debe estar ANTES de TenantMainMiddleware")
            print(f"     ForceNoPortMiddleware: índice {force_no_port_idx}")
            print(f"     TenantMainMiddleware: índice {tenant_main_idx}")
            return False
    except ValueError as e:
        print(f"  ⚠️  No se pudo verificar orden de middlewares: {e}")
        return False
...
def main():
    ...
    if all_ok:
        ...
        print("\n💡 Si aún tienes problemas de acceso:")
