...
def run_audit():
    ...
    """
    sys.stdout.buffer.write(("INFO: Auditoría: Verificando que endpoints API retornan SOLO JSON...\\n").encode('utf-8'))
    ...
    try:
        with schema_context('public'):
            ...
            if not tenant:
                sys.stdout.buffer.write(("[WARNING]  No se encontró ningún tenant. Creando uno de prueba...\\n").encode('utf-8'))
            ...
            if not domain:
                sys.stdout.buffer.write(("[ERROR] No se encontró dominio para el tenant.\\n").encode('utf-8'))
                sys.exit(1)
        ...
        sys.stdout.buffer.write(f"[ERROR] Error configurando tenant de prueba: {e}\n".encode('utf-8'))
        sys.exit(1)
    ...
    for endpoint in ENDPOINTS_TO_CHECK:
        ...
        with schema_context(tenant.schema_name):
            ...
            if success:
                sys.stdout.buffer.write(b"[OK] OK\n")
            else:
                sys.stdout.buffer.write(f"[ERROR] {message}\n".encode('utf-8'))
    ...
    if errors:
        sys.stdout.buffer.write(f"[ERROR] ERRORES ENCONTRADOS ({len(errors)}):\n".encode('utf-8'))
