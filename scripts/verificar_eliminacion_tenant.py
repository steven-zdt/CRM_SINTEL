...
try:
    ...
    print(f"❌ ERROR: El Client '{SCHEMA_NAME}' AÚN EXISTE")
    ...
    print(f"✅ El Client '{SCHEMA_NAME}' NO existe (eliminado correctamente)")
    client_exists = False
...
try:
    ...
    print(f"❌ ERROR: El Domain '{DOMAIN_NAME}' AÚN EXISTE")
    ...
    print(f"✅ El Domain '{DOMAIN_NAME}' NO existe (eliminado correctamente)")
    domain_exists = False
...
if client_exists:
    ...
    if memberships.exists():
        print(f"❌ ERROR: Existen {memberships.count()} TenantMembership asociadas:")
        for m in memberships:
            print(f"   - Usuario: {m.user.email} ({m.user.username}) - Rol: {m.rol}")
        membership_exists = True
    else:
        print(f"✅ No hay TenantMembership asociadas al tenant '{SCHEMA_NAME}'")
    ...
    if memberships.exists():
        print(f"⚠️  ADVERTENCIA: Existen {memberships.count()} TenantMembership huérfanas (client eliminado pero membresías quedaron):")
        for m in memberships:
            print(f"   - Usuario: {m.user.email} ({m.user.username}) - Rol: {m.rol}")
        membership_exists = True
    else:
...
try:
    ...
    if schema_exist:
        print(f"❌ ERROR: El esquema '{SCHEMA_NAME}' AÚN EXISTE en PostgreSQL")
        ...
        with schema_context(SCHEMA_NAME):
            try:
                ...
                with connection.cursor() as cursor:
                    ...
                    if tables:
                        print(f"   ⚠️  El esquema contiene {len(tables)} tablas:")
                        for table in tables[:10]:  # Mostrar solo las primeras 10
                            print(f"      - {table[0]}")
                        if len(tables) > 10:
                            print(f"      ... y {len(tables) - 10} más")
                    else:
                        print(f"   ⚠️  El esquema existe pero está vacío (sin tablas)")
            except Exception as e:
                print(f"   ⚠️  No se pudo acceder al esquema: {e}")
        
        schema_exists_flag = True
    else:
        print(f"✅ El esquema '{SCHEMA_NAME}' NO existe en PostgreSQL (eliminado correctamente)")
        schema_exists_flag = False
except Exception as e:
    print(f"⚠️  Error al verificar el esquema: {e}")
...
if related_domains.exists():
    print(f"⚠️  ADVERTENCIA: Existen {related_domains.count()} dominios relacionados:")
    for d in related_domains:
        print(f"   - {d.domain} (tenant: {d.tenant.schema_name if d.tenant else 'None'})")
else:
    print(f"✅ No hay otros dominios relacionados con '{SCHEMA_NAME}.sintel'")
...
if client_exists or domain_exists or membership_exists or schema_exists_flag:
    print("❌ EL TENANT NO HA SIDO ELIMINADO COMPLETAMENTE")
    print("\n📋 Estado actual:")
    ...
    print("✅ EL TENANT HA SIDO ELIMINADO COMPLETAMENTE")
    print("\n📋 Verificación completa:")
    print("   ✅ Client eliminado")
    print("   ✅ Domain eliminado")
    print("   ✅ TenantMembership eliminadas")
    print("   ✅ Esquema en PostgreSQL eliminado")
