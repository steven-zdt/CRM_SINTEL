...
def debug_step_by_step():
    ...
    try:
        ...
        with schema_context('public'):
            ...
            print(f"✅ Dominio encontrado: {domain.domain} (primary: {domain.is_primary})")
            print(f"✅ Usuario encontrado: {user.email} (password usable: {user.has_usable_password()})")
    ...
    if payload:
        print(f"✅ Token verificado: user_id={payload['user_id']}, tenant_id={payload['tenant_id']}")
    else:
    ...
    try:
        ...
        print(f"✅ TENANT_URLCONF cargado: config.urls_tenant")
        ...
        if activate_found:
            print(f"✅ Ruta /activate/ encontrada en apps.tenant.landing.urls")
        else:
            print(f"❌ Ruta /activate/ NO encontrada en apps.tenant.landing.urls")
            return 1
        ...
        if response.status_code == 200:
            print(f"✅ Request exitoso (200 OK)")
        elif response.status_code == 302:
            print(f"⚠️  Redirect (302) - Verificar destino")
            print(f"   Location: {response.get('Location', 'N/A')}")
        else:
            print(f"❌ Status inesperado: {response.status_code}")
            
    except Exception as e:
        print(f"❌ Error al verificar resolución: {e}")
        import traceback
        traceback.print_exc()
        return 1
    ...
    try:
        ...
        if csrf_token:
            ...
            print(f"⚠️  CSRF token no encontrado (puede ser normal en tests)")
        ...
        if response_post.status_code == 302:
            ...
            if location.startswith('/dashboard/'):
                print(f"   ✅ Redirect a ruta privada (/dashboard/)")
            elif location.startswith('/console/'):
                print(f"   ❌ PROBLEMA: Redirect a ruta pública (/console/)")
                print(f"   ⚠️  El tenant está siendo direccionado al URLConf público")
                return 1
            elif 'sintel.com' in location and '/console/' in location:
                print(f"   ❌ PROBLEMA: Redirect a dominio público con ruta /console/")
                print(f"   ⚠️  El tenant está siendo direccionado al URLConf público")
                return 1
            else:
        ...
        print(f"❌ Error al simular activación: {e}")
        import traceback
        traceback.print_exc()
        return 1
    ...
    with schema_context('public'):
        ...
        print(f"✅ Usuario verificado: {user_check.email}")
        ...
        if membership:
            print(f"✅ Membresía encontrada: rol={membership.rol}, is_active={membership.is_active}")
        else:
            print(f"❌ Membresía NO encontrada")
            return 1
    ...
    if login_success:
        ...
        if dashboard_response.status_code == 200:
            print(f"✅ Dashboard accesible (200 OK)")
        elif dashboard_response.status_code == 404:
            print(f"❌ Dashboard NO encontrado (404)")
            print(f"   ⚠️  Posible problema: URLConf incorrecto")
            return 1
        else:
            print(f"⚠️  Status inesperado: {dashboard_response.status_code}")
    else:
    ...
    print("=" * 80)
    print("\n💡 Si todos los pasos pasaron, el flujo está funcionando correctamente.")
