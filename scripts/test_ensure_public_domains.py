...
def main():
    ...
    with schema_context('public'):
        ...
        if sintel and sintel.is_primary:
            print("\n   ✅ sintel.com existe y es primario")
        else:
            print("\n   ❌ sintel.com NO existe o NO es primario")
            return 1
    ...
    try:
        ...
        if "sin cambios" in output2.lower() or "ya existe" in output2.lower():
            print("   ✅ Idempotente: No crea duplicados")
        else:
            print("   ⚠️  Advertencia: Puede no ser completamente idempotente")
    except Exception as e:
        print(f"   ❌ Error: {e}")
        return 1
    ...
    print("=" * 60)
    print("\n💡 El comando ensure_public_domains está funcionando correctamente")
