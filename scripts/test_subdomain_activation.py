...
def test_subdomain_activation():
    ...
    try:
        ...
        print(f"\n⚠️  El tenant '{schema_name}' ya existe. Eliminándolo para la prueba...")
    ...
    print(f"\n🚀 Creando tenant con schema '{schema_name}'...")
    try:
        ...
        if domain.domain == dominio_esperado:
            print(f"   ✅ Dominio correcto: {domain.domain}")
        else:
            print(f"   ❌ ERROR: Dominio esperado '{dominio_esperado}', pero se creó '{domain.domain}'")
            return False
        ...
        if login_url == url_esperada:
            print(f"   ✅ URL correcta: {login_url}")
        else:
            print(f"   ⚠️  URL construida: {login_url}")
            print(f"   ⚠️  URL esperada: {url_esperada}")
