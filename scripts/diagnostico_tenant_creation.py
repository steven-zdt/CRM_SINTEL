...
def diagnostico_completo(task_id: str):
    ...
    try:
        ...
        if active_workers:
            print(f"[OK] Workers activos: {len(active_workers)}")
            for worker, tasks in active_workers.items():
                print(f"   - {worker}: {len(tasks)} tareas activas")
        else:
            print("[ERROR] ERROR CRÍTICO: No hay workers de Celery activos")
            print("   [IDEA] Solución: Ejecutar 'celery -A config worker -l info'")
            return False
        ...
        if registered:
            ...
            if 'apps.public.tenants.tasks.onboard_tenant_task' in all_tasks:
                print("[OK] Tarea 'onboard_tenant_task' está registrada")
            else:
                print("[ERROR] ERROR: Tarea 'onboard_tenant_task' NO está registrada")
                print(f"   Tareas disponibles: {list(all_tasks)[:10]}...")
        else:
            print("[WARNING]  No se pudieron obtener tareas registradas")
    except Exception as e:
        print(f"[ERROR] Error al verificar Celery: {e}")
        import traceback
        traceback.print_exc()
        return False
    ...
    try:
        ...
        if result.state == 'PENDING':
            print()
            print("[WARNING]  DIAGNÓSTICO: Tarea en estado PENDING")
            ...
            try:
                    ...
                    for worker, tasks in scheduled.items():
                        for task in tasks:
                            if task.get('request', {}).get('id') == task_id:
                                print(f"     [OK] Tarea encontrada en cola programada de {worker}")
                ...
                if reserved:
                    ...
                    for worker, tasks in reserved.items():
                        for task in tasks:
                            if task.get('request', {}).get('id') == task_id:
                                print(f"     [OK] Tarea encontrada en cola reservada de {worker}")
                
                if not scheduled and not reserved:
                    print("   [ERROR] Tarea NO encontrada en ninguna cola")
                    print("   [IDEA] La tarea puede no haberse encolado correctamente")
            except Exception as e:
                print(f"   [WARNING]  No se pudo verificar colas: {e}")
        
        elif result.state == 'STARTED':
            print()
            print("INFO:  Tarea en ejecución (STARTED)")
            print("   Esto es normal si la tarea está procesándose actualmente")
        
        elif result.state == 'SUCCESS':
            print()
            print("[OK] Tarea completada exitosamente")
            if result.result:
                print(f"   Resultado: {result.result}")
        
        elif result.state == 'FAILURE':
            print()
            print("[ERROR] Tarea falló")
            if result.info:
        ...
        print(f"[ERROR] Error al consultar estado de tarea: {e}")
        import traceback
        traceback.print_exc()
        return False
    ...
    try:
        ...
        if clients_sin_domain:
            print(f"[WARNING]  Clientes sin dominio principal: {len(clients_sin_domain)}")
            for client in clients_sin_domain:
                print(f"   - {client.nombre} (schema: {client.schema_name})")
        else:
            print("[OK] Todos los clientes tienen dominio principal")
    ...
    print("-" * 80)
    print("[IDEA] Para ver logs en tiempo real, ejecutar:")
