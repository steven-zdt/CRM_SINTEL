...
def eliminar_tablas_cotizaciones():
    ...
    try:
        ...
        if not tenants.exists():
            print("\n[WARNING]  No se encontraron tenants. Intentando eliminar en esquema actual...")
            ...
            with connection.cursor() as cursor:
                ...
                for table in tables:
                    try:
                        ...
                        print(f"   [OK] Tabla {table} eliminada del esquema {schema}")
                    except Exception as e:
                        print(f"   [WARNING]  Error eliminando {table}: {e}")
                ...
                try:
                    ...
                    print(f"   [OK] {deleted} registros de migraciones eliminados del esquema {schema}")
                except Exception as e:
                    print(f"   [WARNING]  Error eliminando registros: {e}")
        else:
                ...
                with schema_context(tenant.schema_name):
                    with connection.cursor() as cursor:
                        ...
                        try:
                            ...
                            deleted = cursor.rowcount
                            print(f"      [OK] {deleted} registros de migraciones eliminados")
                        except Exception as e:
                            print(f"      [WARNING]  Error eliminando registros: {e}")
        
        print("\n[OK] Eliminación completada")
        print("\n[NOTE] Próximo paso: Ejecutar 'python manage.py migrate tenant_cotizaciones'")
