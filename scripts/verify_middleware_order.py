...
def main():
    ...
    print("\n" + "=" * 60)
    print("[OK] VALIDACIÓN DE CONFORMIDAD")
    print("=" * 60)
    ...
    if tenant_idx is not None and auth_idx is not None:
        if tenant_idx < auth_idx:
            print(f"[OK] TenantMainMiddleware (posición {tenant_idx}) está ANTES de AuthenticationMiddleware (posición {auth_idx})")
        else:
            print(f"[ERROR] TenantMainMiddleware (posición {tenant_idx}) está DESPUÉS de AuthenticationMiddleware (posición {auth_idx})")
            return 1
    ...
    if force_no_port_idx is not None and tenant_idx is not None:
        if force_no_port_idx < tenant_idx:
            print(f"[OK] ForceNoPortMiddleware (posición {force_no_port_idx}) está ANTES de TenantMainMiddleware (posición {tenant_idx})")
        else:
            print(f"[ERROR] ForceNoPortMiddleware (posición {force_no_port_idx}) está DESPUÉS de TenantMainMiddleware (posición {tenant_idx})")
            return 1
    ...
    if session_idx is not None and force_no_port_idx is not None:
        if session_idx < force_no_port_idx:
            print(f"[OK] SessionMiddleware (posición {session_idx}) está ANTES de ForceNoPortMiddleware (posición {force_no_port_idx})")
        else:
            print(f"[ERROR] SessionMiddleware (posición {session_idx}) está DESPUÉS de ForceNoPortMiddleware (posición {force_no_port_idx})")
            return 1
...
- [OK] CONFORME con la documentación
    """.format(tenant_idx=tenant_idx, auth_idx=auth_idx))
    
    print("=" * 60)
    print("[OK] VALIDACIÓN COMPLETADA: IMPLEMENTACIÓN VÁLIDA")
    print("=" * 60)
