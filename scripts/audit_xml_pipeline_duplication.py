...
⚠️ POLÍTICA SSoT XML: apps/services/xml_ingest + apps/services/xml_parser son la ÚNICA vía.
...
def audit_xml_pipeline():
    ...
    if not SERVICES_XML_DIR.exists():
        print(f"[WARN] ⚠️  Servicio canónico no encontrado: {SERVICES_XML_DIR}")
        print("[WARN] Asegúrate de que apps/services/xml_ingest existe")
    
    if not SERVICES_PARSER_DIR.exists():
        print(f"[WARN] ⚠️  Servicio canónico no encontrado: {SERVICES_PARSER_DIR}")
