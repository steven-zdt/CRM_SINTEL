...
def refactor_module(module_name):
    ...
    if not file_path.exists():
        print(f"[WARNING]  {file_path} no existe, omitiendo...")
        return False
    
    print(f"[NOTE] Refactorizando {module_name}.page.js...")
  ...
  function warn(...args) {{
    ...
    if content != original_content:
        with open(file_path, 'w', encoding='utf-8') as f:
            f.write(content)
        print(f"[OK] {module_name}.page.js refactorizado")
        return True
    else:
        print(f"[WARNING]  {module_name}.page.js no tuvo cambios")
        return False

if __name__ == '__main__':
    print("[LAUNCH] Iniciando refactorización de módulos...\n")
    for module in MODULES:
        try:
            refactor_module(module)
        except Exception as e:
            print(f"[ERROR] Error en {module}: {e}")
    print("\n[OK] Refactorización completada")
