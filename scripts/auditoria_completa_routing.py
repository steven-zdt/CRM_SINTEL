...
def auditoria_completa(domain_name=None):
    ...
    if root_urlconf_path.exists():
        print(f"   ✅ {settings.ROOT_URLCONF} existe")
    else:
        print(f"   ❌ {settings.ROOT_URLCONF} NO existe")
        errors.append(f"ROOT_URLCONF no existe: {root_urlconf_path}")
    
    if tenant_urlconf_path.exists():
        print(f"   ✅ {settings.TENANT_URLCONF} existe")
    else:
        print(f"   ❌ {settings.TENANT_URLCONF} NO existe")
    ...
    if tenant_middleware_idx is not None:
        print(f"   ✅ TenantMainMiddleware en posición {tenant_middleware_idx}")
    else:
        print(f"   ❌ TenantMainMiddleware NO encontrado en MIDDLEWARE")
        errors.append("TenantMainMiddleware no está en MIDDLEWARE")
    
    if force_no_port_idx is not None:
        print(f"   ✅ ForceNoPortMiddleware en posición {force_no_port_idx}")
        if force_no_port_idx < tenant_middleware_idx:
            print(f"   ✅ Orden correcto: ForceNoPortMiddleware → TenantMainMiddleware")
        else:
            print(f"   ❌ Orden incorrecto: ForceNoPortMiddleware debe ir ANTES de TenantMainMiddleware")
            errors.append("Orden incorrecto de middleware")
    else:
        print(f"   ⚠️  ForceNoPortMiddleware no encontrado (puede ser opcional)")
    
    # 1.3 Database Backend
    print("\n1.3 Database Backend:")
    db_engine = settings.DATABASES['default']['ENGINE']
    if 'django_tenants' in db_engine:
        print(f"   ✅ Backend correcto: {db_engine}")
    else:
        print(f"   ❌ Backend incorrecto: {db_engine}")
    ...
    with schema_context('public'):
        ...
        if public_tenant:
            print(f"   ✅ Tenant público encontrado: {public_tenant.nombre}")
        else:
            print(f"   ❌ Tenant público NO encontrado")
        ...
        if domain_name:
            ...
            if domain_obj:
                print(f"   ✅ Dominio encontrado en BD: {domain_obj.domain}")
                ...
                if domain_obj.tenant.schema_name == public_schema:
                    print(f"   ❌ PROBLEMA: El dominio está asociado al tenant PÚBLICO")
                    errors.append(f"Dominio {normalized_domain} está asociado al tenant público")
                else:
                    print(f"   ✅ Dominio asociado a tenant privado correctamente")
            else:
                print(f"   ❌ PROBLEMA CRÍTICO: Dominio NO encontrado en BD")
                ...
                if similar:
                    print(f"   💡 Dominio similar encontrado: {similar.domain}")
        ...
        for tenant in private_tenants[:10]:  # Mostrar primeros 10
            ...
            if primary_domain:
                ...
                print(f"   - {tenant.nombre} (schema: {tenant.schema_name}) → ⚠️ SIN DOMINIO PRIMARIO")
    ...
    if domain_name:
        ...
        try:
            ...
            if resolved_tenant:
                print(f"   ✅ Tenant resuelto: {resolved_tenant.nombre} (schema: {resolved_tenant.schema_name})")
                print(f"   Esquema activo: {resolved_schema}")
                print(f"   URLConf: {resolved_urlconf or 'No establecido'}")
                
                if resolved_schema == public_schema:
                    print(f"   ❌ PROBLEMA: Se resolvió como tenant PÚBLICO")
                    errors.append(f"Middleware resolvió {normalized_domain} como tenant público")
                else:
                    print(f"   ✅ Tenant privado resuelto correctamente")
                    
                    # Verificar URLConf
                    if resolved_urlconf == settings.TENANT_URLCONF:
                        print(f"   ✅ URLConf correcto: TENANT_URLCONF")
                    else:
                        print(f"   ❌ URLConf incorrecto: {resolved_urlconf} (esperado: {settings.TENANT_URLCONF})")
                        if resolved_urlconf is None:
                            print(f"   💡 NOTA: TenantURLConfMiddleware debería establecer esto automáticamente")
                        errors.append(f"URLConf incorrecto: {resolved_urlconf}")
            else:
                print(f"   ❌ PROBLEMA CRÍTICO: No se pudo resolver el tenant")
                print(f"   Esquema activo: {resolved_schema}")
    ...
    try:
        ...
        if 'activate' in str(urlpatterns):
            print(f"   ⚠️  ADVERTENCIA: /activate/ encontrado en ROOT_URLCONF (no debería estar)")
            warnings.append("/activate/ encontrado en ROOT_URLCONF")
    except Exception as e:
        print(f"   ❌ Error al cargar ROOT_URLCONF: {e}")
    ...
    try:
        ...
        if activate_found:
            print(f"   ✅ /activate/ encontrado en apps.tenant.landing.urls")
        else:
            print(f"   ❌ /activate/ NO encontrado en apps.tenant.landing.urls")
            errors.append("/activate/ no encontrado en TENANT_URLCONF")
    except Exception as e:
        print(f"   ❌ Error al cargar TENANT_URLCONF: {e}")
    ...
    if domain_name:
        ...
        if normalized_domain in allowed_hosts or '*' in allowed_hosts or any(normalized_domain.endswith(h.lstrip('.')) for h in allowed_hosts if h.startswith('.')):
            print(f"   ✅ {normalized_domain} permitido en ALLOWED_HOSTS")
        else:
            print(f"   ⚠️  {normalized_domain} puede no estar permitido en ALLOWED_HOSTS")
    ...
    if domain_name and tenant_domain_base:
        if tenant_domain_base in domain_name:
            print(f"   ✅ Dominio contiene TENANT_DOMAIN_BASE")
        else:
            print(f"   ⚠️  Dominio no contiene TENANT_DOMAIN_BASE")
            ...
            if domain_obj:
                ...
                try:
                    ...
                    if schema_exists(tenant.schema_name):
                        print(f"   ✅ Esquema {tenant.schema_name} existe en PostgreSQL")
                    else:
                        print(f"   ❌ PROBLEMA CRÍTICO: Esquema {tenant.schema_name} NO existe")
                        errors.append(f"Esquema {tenant.schema_name} no existe en PostgreSQL")
                except Exception as e:
                    print(f"   ⚠️  No se pudo verificar esquema: {e}")
    ...
    print(f"\n✅ Verificaciones exitosas: {80 - len(errors) - len(warnings)}")
