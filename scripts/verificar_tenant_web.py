...
try:
    ...
    if schema_exists(tenant.schema_name):
        print(f"✅ Esquema PostgreSQL '{tenant.schema_name}' existe")
    else:
        print(f"❌ Esquema PostgreSQL '{tenant.schema_name}' NO existe")
    print("")
    ...
    if not domains.exists():
        ...
        print(f"✅ Tiene {domains.count()} dominio(s):")
        for d in domains:
    ...
    if not primary_domain:
        print("❌ NO tiene dominio principal (is_primary=True)")
    else:
        print(f"✅ Dominio principal: {primary_domain.domain}")
        
        # Verificar formato
        expected_domain = f"{tenant.schema_name}.{settings.TENANT_DOMAIN_BASE}"
        if primary_domain.domain != expected_domain:
            print(f"⚠️  Formato: '{primary_domain.domain}' (esperado: '{expected_domain}')")
        else:
            print(f"✅ Formato correcto: {expected_domain}")
    print("")
    ...
    if not admin_membership:
        print("❌ NO tiene administrador principal asignado")
    else:
        print(f"✅ Administrador principal: {admin_membership.user.email}")
    ...
    if primary_domain:
        ...
        try:
            ...
            if response.status_code == 404:
                print(f"❌ Acceso web falla: HTTP 404 (tenant no encontrado)")
            elif response.status_code in [200, 302, 301]:
                print(f"✅ Acceso web funciona: HTTP {response.status_code}")
            else:
                print(f"⚠️  Acceso web retorna: HTTP {response.status_code}")
        except Exception as e:
            print(f"❌ Error al probar acceso web: {e}")
    else:
        print("⚠️  No se puede probar acceso web (no hay dominio principal)")
