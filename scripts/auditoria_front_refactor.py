...
def generate_markdown_report(violations: Dict, total: int) -> str:
    ...
    if not violations:
        lines.append('[OK] **No se encontraron infracciones.**')
    else:
...
def main():
    ...
    print(f'[OK] Auditoría completada: {total} infracciones encontradas en {len(violations)} archivos')
    ...
    # Exit code != 0 si hay infracciones
    if total > 0:
        print(f'\n[ERROR] CI fallará: {total} infracciones detectadas')
        exit(1)
    else:
        print('\n[OK] CI pasará: 0 infracciones')
        exit(0)
