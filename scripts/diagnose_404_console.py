...
def main():
    ...
    with schema_context('public'):
        try:
            ...
            print("   [ERROR] Tenant público no encontrado")
            return 1
    ...
    if settings.ROOT_URLCONF == 'config.urls_public':
        ...
        print("   [ERROR] Debe ser 'config.urls_public'")
        return 1
    ...
    try:
        ...
        print(f"   [OK] reverse('console:tenants-list') = {url}")
        
        # Probar resolve
        match = resolve('/console/tenants/')
        print(f"   [OK] resolve('/console/tenants/') = {match.view_name}")
        print(f"      View: {match.func}")
    except Exception as e:
        print(f"   [ERROR] Error al resolver ruta: {e}")
        return 1
    ...
    try:
        ...
        if console_paths:
            print(f"   [OK] Console incluido en urls_public: {console_paths}")
        else:
            print("   [ERROR] Console NO encontrado en urls_public")
            return 1
    except Exception as e:
        print(f"   [ERROR] Error al verificar urls_public: {e}")
        return 1
    ...
    if tenant_middleware_idx is not None:
        print(f"   [OK] TenantMainMiddleware encontrado en posición {tenant_middleware_idx}")
        print(f"      Middleware antes: {middleware[:tenant_middleware_idx+1]}")
    else:
        print("   [ERROR] TenantMainMiddleware NO encontrado")
        return 1
