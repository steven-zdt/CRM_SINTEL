...
def test_email_config():
    ...
    try:
        ...
        print(f"  ⚠️  Error de conexión SMTP: {e}")
        if settings.EMAIL_BACKEND == 'django.core.mail.backends.console.EmailBackend':
            print("  ℹ️  Usando backend de consola (normal en desarrollo)")
        else:
            print("  ❌ Revisa la configuración SMTP en .env")
            return False
    ...
    try:
        ...
        if settings.EMAIL_BACKEND == 'django.core.mail.backends.console.EmailBackend':
            print("  ℹ️  (En desarrollo, el email se muestra en la consola)")
        return True
    except Exception as e:
...
def test_invitation_service():
    ...
    try:
        ...
        if payload:
            print(f"  ✅ Token válido: user_id={payload['user_id']}, tenant_id={payload['tenant_id']}")
        else:
        ...
        print(f"  ❌ Error en servicio de invitaciones: {e}")
        import traceback
        traceback.print_exc()
        return False
