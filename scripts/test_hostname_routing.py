...
def test_hostname_routing():
    ...
    with schema_context('public'):
        ...
        if sintel_domain and sintel_domain.is_primary:
            print("   ✅ sintel.com existe y es primario")
        else:
            print("   ❌ sintel.com NO existe o NO es primario")
            print("   💡 Ejecuta: python manage.py ensure_public_domains")
            return 1
    ...
    with schema_context('public'):
        ...
        if test_tenant:
            ...
            if test_domain:
                print(f"   ✅ Tenant de prueba: {test_tenant.schema_name}")
                print(f"   ✅ Dominio: {test_domain.domain}")
                test_domain_name = test_domain.domain
            else:
                print(f"   ⚠️  Tenant {test_tenant.schema_name} sin dominio primario")
                test_domain_name = None
        else:
            print("   ⚠️  No hay tenants privados para probar")
            test_domain_name = None
    ...
    try:
        ...
        if response.status_code in [200, 302, 403]:  # 200 OK, 302 redirect, 403 forbidden (sin auth)
            print(f"      ✅ Status: {response.status_code} (esperado para ROOT_URLCONF)")
            print(f"      ✅ URL resuelta correctamente en {settings.ROOT_URLCONF}")
        else:
            print(f"      ⚠️  Status: {response.status_code} (puede ser 404 si no está autenticado)")
    ...
    if test_domain_name:
        ...
        try:
            ...
            if response.status_code in [200, 302, 400]:  # 200 OK, 302 redirect, 400 bad request (token inválido)
                print(f"      ✅ Status: {response.status_code} (esperado para TENANT_URLCONF)")
                print(f"      ✅ URL resuelta correctamente en {settings.TENANT_URLCONF}")
            elif response.status_code == 404:
                print(f"      ❌ Status: 404 (ruta no encontrada - posible problema de enrutamiento)")
                print(f"      💡 Verifica que apps.tenant.landing.urls esté incluido en TENANT_URLCONF")
            else:
    ...
    try:
        ...
        if console_included:
            print("      ✅ /console/ incluido en ROOT_URLCONF")
        else:
            print("      ❌ /console/ NO incluido en ROOT_URLCONF")
    except Exception as e:
        print(f"      ❌ Error: {e}")
    ...
    try:
        ...
        if landing_included:
            print("      ✅ apps.tenant.landing.urls incluido en TENANT_URLCONF")
            ...
            if activate_found:
                print("      ✅ /activate/ encontrado en apps.tenant.landing.urls")
            else:
                print("      ❌ /activate/ NO encontrado en apps.tenant.landing.urls")
        else:
            print("      ❌ apps.tenant.landing.urls NO incluido en TENANT_URLCONF")
    except Exception as e:
        print(f"      ❌ Error: {e}")
    ...
    if force_no_port_idx is not None and tenant_middleware_idx is not None:
        if force_no_port_idx < tenant_middleware_idx:
            print("   ✅ Orden correcto: ForceNoPortMiddleware → TenantMainMiddleware")
        else:
            print("   ❌ Orden incorrecto: TenantMainMiddleware debe ir DESPUÉS de ForceNoPortMiddleware")
            return 1
    else:
        print("   ⚠️  No se encontraron middlewares esperados")
        return 1
    ...
    print("📋 RESUMEN")
    print("=" * 60)
    print("✅ Configuración de enrutamiento verificada")
    print("\n💡 Para pruebas reales de enrutamiento:")
