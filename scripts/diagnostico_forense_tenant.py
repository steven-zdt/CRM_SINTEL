...
def diagnostico_completo(hostname_input: str):
    ...
    try:
        ...
        print(f"   [OK] Client model: {Client}")
        print(f"   [OK] Domain model: {Domain}")
    except Exception as e:
        print(f"   [ERROR] Error al obtener modelos: {e}")
        return
    print()
    ...
    try:
        ...
        if not all_domains.exists():
            print("   [WARNING]  NO HAY DOMINIOS REGISTRADOS")
        else:
            ...
            for domain in all_domains:
                ...
                primary_mark = "[OK]" if domain.is_primary else "  "
                match_mark = "[TARGET]" if domain.domain == hostname_puro or domain.domain == hostname_input else "  "
            ...
            if exact_match:
                print(f"   [OK] COINCIDENCIA EXACTA ENCONTRADA:")
                ...
                print(f"   [ERROR] NO SE ENCONTRÓ COINCIDENCIA EXACTA para '{hostname_puro}'")
            ...
            if ':' in hostname_input:
                ...
                if exact_match_with_port:
                    print(f"   [OK] COINCIDENCIA CON PUERTO ENCONTRADA:")
                    ...
                    print(f"   [WARNING]  NO SE ENCONTRÓ COINCIDENCIA CON PUERTO para '{hostname_input}'")
                    print(f"   INFO:  django-tenants usa SOLO el hostname sin puerto para resolver")
    except Exception as e:
        print(f"   [ERROR] Error al consultar dominios: {e}")
        import traceback
        traceback.print_exc()
    print()
    ...
    try:
        ...
        try:
            ...
            if resolved_tenant:
                ...
                print(f"   [ERROR] NO SE PUDO RESOLVER EL TENANT")
                print(f"      Esquema activo: {resolved_schema}")
                print(f"      URLConf que se usará: {settings.ROOT_URLCONF}")
                print(f"      [WARNING]  Esto significa que se usará el esquema público")
        finally:
    ...
    if tenant_domain_base:
        ...
        if hostname_puro.startswith('home.'):
            ...
            if hostname_puro == expected_domain:
                print(f"   [OK] El hostname coincide con el patrón esperado")
            else:
                print(f"   [WARNING]  El hostname NO coincide con el patrón esperado")
                print(f"      Esperado: {expected_domain}")
                print(f"      Recibido: {hostname_puro}")
    else:
        print("   [WARNING]  TENANT_DOMAIN_BASE no está configurado en settings.py")
    print()
    ...
    try:
        ...
        if not exact_match:
            print("   [ERROR] PROBLEMA DETECTADO: No existe un dominio registrado para este hostname")
            print()
            print("   [IDEA] SOLUCIÓN:")
            ...
            print("   [OK] El dominio está registrado correctamente")
            print(f"      Tenant: {exact_match.tenant.nombre}")
            print(f"      Schema: {exact_match.tenant.schema_name}")
            print()
            print("   [IDEA] Si aún así no funciona, verifica:")
