...
def test_routing():
    ...
    if force_no_port_idx is not None and tenant_middleware_idx is not None:
        if force_no_port_idx < tenant_middleware_idx:
            print("   ✅ ForceNoPortMiddleware está ANTES de TenantMainMiddleware (correcto)")
        else:
            print("   ❌ ForceNoPortMiddleware está DESPUÉS de TenantMainMiddleware (incorrecto)")
    ...
    print("   ⚠️  Nota: Esta es una simulación. Para pruebas reales, usa curl o el navegador.")
    ...
    try:
        ...
        print(f"      ✅ Debe usar: {settings.ROOT_URLCONF}")
    ...
    if test_domain_name:
        ...
        try:
            ...
            if activate_found:
                print(f"      ✅ Ruta /activate/ encontrada en TENANT_URLCONF")
                print(f"      ✅ Debe usar: {settings.TENANT_URLCONF}")
            else:
                print(f"      ❌ Ruta /activate/ NO encontrada en TENANT_URLCONF")
                results.append(False)
        except Exception as e:
            print(f"      ❌ Error: {e}")
            results.append(False)
