...
if 'apps.tenant.gastos' in settings.TENANT_APPS:
    print("   [OK] apps.tenant.gastos está en TENANT_APPS")
else:
    print("   [ERROR] ERROR: apps.tenant.gastos NO está en TENANT_APPS")
...
try:
    ...
    print("   [OK] GastoViewSet importado correctamente")
    print(f"   Queryset: {GastoViewSet.queryset}")
    print(f"   Basename esperado: 'gastos'")
except ImportError as e:
    print(f"   [ERROR] ERROR: No se pudo importar GastoViewSet: {e}")
    sys.exit(1)
...
try:
    ...
    print("   [OK] Router importado correctamente")
    ...
    print(f"   [ERROR] ERROR: No se pudo importar router: {e}")
    import traceback
    traceback.print_exc()
    sys.exit(1)
...
try:
    ...
    for pattern in api_urlpatterns:
        if hasattr(pattern, 'pattern') and 'gastos' in str(pattern.pattern):
            gastos_found = True
            print(f"   [OK] URL encontrada: {pattern.pattern}")
            break
    if not gastos_found:
        print("   [ERROR] ERROR: No se encontró 'gastos' en config/api_urls.py")
except Exception as e:
    print(f"   [WARNING]  ADVERTENCIA: No se pudo verificar config/api_urls.py: {e}")
...
for action_name in actions:
    if hasattr(GastoViewSet, action_name):
        print(f"   [OK] Acción '{action_name}' encontrada")
    else:
        print(f"   [WARNING]  Acción '{action_name}' no encontrada")
...
for url_name in test_urls:
    try:
        ...
        try:
            ...
            print(f"   [ERROR] No se pudo resolver '{url_name}': {e}")
