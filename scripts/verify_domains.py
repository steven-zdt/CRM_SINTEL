    ...
    try:
        ...
        if dominio_principal:
            ...
            if dominio_principal.domain == dominio_esperado:
                print(f"[OK] CORRECTO: El dominio principal coincide con TENANT_DOMAIN_BASE")
                return True
            else:
                print(f"[ERROR] ERROR: El dominio principal ({dominio_principal.domain}) NO coincide con TENANT_DOMAIN_BASE ({dominio_esperado})")
                print(f"   Ejecuta: python manage.py ensure_public_domain")
                return False
        else:
            print(f"[ERROR] ERROR: No se encontró dominio principal para el tenant público")
            print(f"   Ejecuta: python manage.py ensure_public_domain")
            return False
            
    except Client.DoesNotExist:
        print(f"[ERROR] ERROR: El tenant público no existe")
...
def simular_creacion_tenant():
    ...
    try:
        ...
        if Client.objects.filter(schema_name=schema_name).exists():
            print(f"[WARNING]  El tenant '{schema_name}' ya existe. Eliminándolo para la prueba...")
        ...
        if domain.domain == dominio_esperado:
            print(f"[OK] CORRECTO: El dominio creado ({domain.domain}) coincide con el esperado ({dominio_esperado})")
            ...
            print(f"[ERROR] ERROR: El dominio creado ({domain.domain}) NO coincide con el esperado ({dominio_esperado})")
...
def main():
    ...
    print("\n" + "=" * 60)
    print("[LAUNCH] VERIFICACIÓN DE POLÍTICA ESTRICTA DE SUBDOMINIOS")
    print("=" * 60)
    print(f"\n📋 Configuración:")
    ...
    # Resumen final
    print("\n" + "=" * 60)
    print("[CHART] RESUMEN DE VERIFICACIONES")
    print("=" * 60)
    print(f"   Verificación 1 (Tenant Público): {'[OK] PASÓ' if resultado_1 else '[ERROR] FALLÓ'}")
    print(f"   Verificación 2 (Subdominios Auto): {'[OK] PASÓ' if resultado_2 else '[ERROR] FALLÓ'}")
