...
class DockerLintTests(unittest.TestCase):
    ...
    def test_dockerfile_lint(self):
        ...
        try:
            ...
            self.skipTest(
                "Hadolint no está instalado. Instalar con: brew install hadolint"
            )
...
class ContainerStructureTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.project_root = Path(__file__).parent.parent.parent
        self.cst_config = (
            self.project_root / "tests" / "docker" / "container-structure-test.yaml"
        )

    def test_cst_config_exists(self):
        """Test: Configuración de CST existe."""
        self.assertTrue(
            self.cst_config.exists(), "container-structure-test.yaml no encontrado"
        )
    ...
    def test_container_structure(self):
        ...
        try:
            ...
            if result.returncode != 0:
                self.fail(
                    f"Container Structure Test falló:\n{result.stdout}\n{result.stderr}"
                )
        except FileNotFoundError:
