...
- Dominio Público (sintel.com): Debe usar config.urls_public
- Dominio Privado (cliente.sintel.com): Debe usar config.urls_tenant
...
class TestPublicDomainRouting:
    """
    Tests para validar el enrutamiento del dominio público (sintel.com).
    """
    ...
    def test_public_anonymous_access_redirects_to_login(self):
        ...
        Escenario:
        - Usuario anónimo accede a http://sintel.com/
        ...
        # Simular request a http://sintel.com/ (raíz)
        response = client.get(
            '/',
        ...
        assert response.status_code != 404, \
            "NO debe dar error 404 (Not Found)"
    ...
    def test_public_authenticated_staff_redirects_to_console(self):
        ...
        Escenario:
        - Usuario staff autenticado accede a http://sintel.com/
    ...
    def test_console_tenants_page_still_works(self):
        ...
        assert 'error' not in content.lower()[:200] or '404' not in content[:200], \
...
class TestPrivateDomainRouting:
    """
    Tests para validar el enrutamiento de dominios privados (cliente.sintel.com).
    """
    ...
    def test_private_tenant_anonymous_redirects_to_login(self):
        ...
        Escenario:
        - Usuario anónimo accede a http://cliente.sintel.com/
        ...
        domain = DomainFactory(
            tenant=tenant,
            domain="test-routing.sintel.com",
            is_primary=True
        )
        ...
        # Simular request a http://test-routing.sintel.com/ (raíz)
        response = client.get(
            '/',
    ...
    def test_private_tenant_authenticated_shows_dashboard(self):
        ...
        Escenario:
        - Usuario autenticado accede a http://cliente.sintel.com/
        ...
        from tests.public.tenants.factories import ClientFactory, DomainFactory, TenantMembershipFactory
        ...
        domain = DomainFactory(
            tenant=tenant,
            domain="test-dashboard.sintel.com",
            is_primary=True
        )
        ...
        # Simular request a http://test-dashboard.sintel.com/ (raíz)
        response = client.get(
            '/',
            HTTP_HOST=domain.domain
        )
        ...
        if response.status_code == 302:
            assert '/login/' in response.url or '/admin/login/' in response.url or '/dashboard/' in response.url, \
...
class TestRoutingIsolation:
    ...
    def test_tenant_api_not_accessible_from_public_domain(self):
        ...
        Escenario:
        - Intentar acceder a /api/v1/facturas/ desde sintel.com (dominio público)
    ...
    def test_public_console_not_accessible_from_tenant_domain(self):
        ...
        Escenario:
        - Intentar acceder a /console/tenants/ desde cliente.sintel.com (dominio privado)
        - Debe retornar 404 Not Found
        ...
        domain = DomainFactory(
            tenant=tenant,
            domain="test-isolation.sintel.com",
            is_primary=True
        )
    ...
    def test_jwt_endpoints_available_in_both_domains(self):
        ...
        Escenario:
        - /api/token/ debe estar disponible desde sintel.com (público)
        - /api/token/ debe estar disponible desde cliente.sintel.com (privado)
        """
        from apps.public.tenants.models import Client as TenantClient, Domain
        from tests.public.tenants.factories import ClientFactory, DomainFactory
        ...
        # Crear tenant y dominio
        tenant = ClientFactory(
            nombre="Empresa Test JWT",
            schema_name="test_jwt",
            is_active=True
        )
        
        domain = DomainFactory(
            tenant=tenant,
            domain="test-jwt.sintel.com",
            is_primary=True
        )
...
class TestRoutingIntegration:
    ...
    def test_public_to_console_flow(self):
        ...
        Escenario:
        1. Usuario anónimo accede a sintel.com/ -> redirige a /admin/login/
        2. Usuario se autentica
        3. Usuario accede a sintel.com/ -> redirige a /console/
        4. Usuario accede a sintel.com/console/tenants/ -> 200 OK
        """
        # Asegurar que estamos en esquema public
        connection.set_schema_to_public()
        ...
        response1 = client.get('/', HTTP_HOST='sintel.com')
        ...
        response2 = client.get('/', HTTP_HOST='sintel.com')
        assert response2.status_code == 302
        assert response2.url == '/console/'
        
        # Paso 4: Usuario accede a consola de tenants
        response3 = client.get('/console/tenants/', HTTP_HOST='sintel.com')
    ...
    def test_tenant_to_api_flow(self):
        ...
        Escenario:
        1. Usuario anónimo accede a cliente.sintel.com/ -> redirige a login
        2. Usuario se autentica
        3. Usuario accede a cliente.sintel.com/api/v1/empresa/ -> debe responder (200 o 404 según implementación)
        """
        from apps.public.tenants.models import Client as TenantClient, Domain, TenantMembership
        from tests.public.tenants.factories import ClientFactory, DomainFactory, TenantMembershipFactory
        ...
        # Crear tenant y dominio
        tenant = ClientFactory(
            nombre="Empresa Test API Flow",
            schema_name="test_api_flow",
            is_active=True
        )
        
        domain = DomainFactory(
            tenant=tenant,
            domain="test-api-flow.sintel.com",
            is_primary=True
        )
