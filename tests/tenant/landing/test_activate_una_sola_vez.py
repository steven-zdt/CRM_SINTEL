...
class TestActivacionUnaSolaVez:
    ...
    def test_activacion_muestra_formulario_si_password_usable(self, django_user_model):
        ...
        with schema_context("public"):
            tenant = TenantClient.objects.create(
                schema_name="test_tenant", nombre="Test Tenant", is_active=True
            )
            domain = Domain.objects.create(
                tenant=tenant, domain="test-tenant.localhost", is_primary=True
            )
        ...
        assert (
            "activate" in response.content.decode().lower()
    ...
    def test_activacion_funciona_sin_password_usable(self, django_user_model):
        ...
        with schema_context("public"):
            tenant = TenantClient.objects.create(
                schema_name="test_tenant2", nombre="Test Tenant 2", is_active=True
            )
            domain = Domain.objects.create(
                tenant=tenant, domain="test-tenant2.localhost", is_primary=True
            )
    ...
    def test_activacion_no_funciona_dos_veces(self, django_user_model):
        ...
        with schema_context("public"):
            tenant = TenantClient.objects.create(
                schema_name="test_tenant3", nombre="Test Tenant 3", is_active=True
            )
            domain = Domain.objects.create(
                tenant=tenant, domain="test-tenant3.localhost", is_primary=True
            )
            user = django_user_model.objects.create(
        ...
        ), "Token aún válido, debe mostrar formulario (v2.29)"
