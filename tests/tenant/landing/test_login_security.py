...
⚠️ OBJETIVO: Garantizar que solo usuarios con TenantMembership activa
...
class TestLoginSecurity(SintelTenantTestCase):
    ...
    def test_escenario_a_acceso_legitimo(self):
        ...
        # ⚠️ IMPORTANTE: AuthenticationForm usa 'username', no 'email'
        ...
        # ⚠️ ASSERT OBLIGATORIO 1: Debe haber redirección (302) si el login fue exitoso
        self.assertEqual(
            response.status_code,
            302,
            f"❌ VIOLACIÓN DE SEGURIDAD: El login legítimo debe redirigir (302) al dashboard. "
        ...
        self.assertIn(
            '/dashboard/',
            response.url,
            f"❌ VIOLACIÓN DE SEGURIDAD: Después de login exitoso, debe redirigir a /dashboard/. "
        ...
        self.assertNotEqual(
            ...
            302,
            "❌ VIOLACIÓN DE SEGURIDAD: Después de login exitoso, el usuario debe poder acceder "
    ...
    def test_escenario_b_intrusion_cross_tenant(self):
        ...
        # ⚠️ CRÍTICO: El usuario tiene credenciales válidas globalmente,
        ...
        # ⚠️ ASSERT OBLIGATORIO 1: El formulario debe ser inválido
        self.assertEqual(
            response.status_code,
            200,  # Django muestra el formulario con errores (200 OK)
            "❌ VIOLACIÓN DE SEGURIDAD: El login cross-tenant debe mostrar formulario con errores (200 OK), "
        ...
        self.assertIsNotNone(
            form,
            "❌ VIOLACIÓN DE SEGURIDAD: El formulario debe estar en el contexto de la respuesta."
        )
        
        self.assertTrue(
            form.errors,
            "❌ VIOLACIÓN DE SEGURIDAD: El formulario DEBE tener errores cuando un usuario "
            "sin membresía intenta loguearse. Si no tiene errores, la validación de membresía no está funcionando."
        )
        
        # ⚠️ ASSERT OBLIGATORIO 3: El mensaje de error debe estar presente
        ...
        self.assertTrue(
            has_authorization_error,
            f"❌ VIOLACIÓN DE SEGURIDAD: El mensaje de error debe indicar falta de autorización. "
            f"Errores encontrados: {error_messages}"
        )
        
        # ⚠️ ASSERT OBLIGATORIO 4: El usuario NO debe estar autenticado
        # Verificar que la sesión NO contiene _auth_user_id
        session = self.client_tenant_b.session
        self.assertNotIn(
            '_auth_user_id',
            session,
            "❌ VIOLACIÓN DE SEGURIDAD: El usuario NO debe estar autenticado después de un "
        ...
        # ⚠️ ASSERT OBLIGATORIO 5: NO debe haber redirección
        self.assertNotEqual(
            response.status_code,
            302,
            "❌ VIOLACIÓN DE SEGURIDAD: NO debe haber redirección (302) después de un "
    ...
    def test_escenario_c_usuario_sin_membresia(self):
        ...
        self.assertEqual(
            response.status_code,
            200,  # Django muestra el formulario con errores (200 OK)
            "❌ VIOLACIÓN DE SEGURIDAD: El login sin membresía debe mostrar formulario con errores (200 OK), "
            "no debe redirigir (302). Si redirige, significa que el login fue exitoso, lo cual es un fallo de seguridad."
        )
        ...
        self.assertTrue(
            ...
            f"❌ VIOLACIÓN DE SEGURIDAD: Debe haber mensajes de error. "
            f"Errores encontrados: {error_messages}"
        )
