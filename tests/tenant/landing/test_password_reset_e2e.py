...
def tenant_with_user(tenant_factory, user_factory):
    ...
    with schema_context(get_public_schema_name()):
        ...
        domain = Domain.objects.create(
            tenant=tenant, domain="e2etest.localhost", is_primary=True
        )
...
def test_password_reset_full_flow_e2e(mock_send_email, api_client, tenant_with_user):
    ...
    request_response = api_client.post(
        request_url, {"email": user.email}, HTTP_HOST=domain.domain, format="json"
    )
    ...
    validate_url = (
        f"/api/v1/landing/auth/password-reset/validate/?uidb64={uidb64}&token={token}"
    )
    ...
    confirm_url = (
        f"/api/v1/landing/auth/password-reset/confirm/?uidb64={uidb64}&token={token}"
    )
    ...
    assert not user.check_password(
        original_password
    ), "La contraseña original NO debe funcionar"
    ...
    assert (
        invalid_validate_response.status_code == status.HTTP_400_BAD_REQUEST
...
def test_password_reset_token_reuse_prevention(api_client, tenant_with_user):
    ...
    assert (
        "inválido" in str(second_response.data).lower()
        or "expirado" in str(second_response.data).lower()
    )


@pytest.mark.django_db
def test_password_reset_different_tenant_isolation(
    api_client, tenant_factory, user_factory
):
    """
    ...
    with schema_context(get_public_schema_name()):
        ...
        domain1 = Domain.objects.create(
            tenant=tenant1, domain="tenant1.localhost", is_primary=True
        )

        tenant2 = tenant_factory(schema_name="tenant2", nombre="Tenant 2")
        domain2 = Domain.objects.create(
            tenant=tenant2, domain="tenant2.localhost", is_primary=True
        )
        ...
        TenantMembership.objects.create(
            client=tenant1, user=user, rol="ADMIN", is_active=True
        )
        TenantMembership.objects.create(
            client=tenant2, user=user, rol="ADMIN", is_active=True
        )
