...
def tenant_with_user(tenant_factory, user_factory):
    ...
    with schema_context(get_public_schema_name()):
        ...
        domain = Domain.objects.create(tenant=tenant, domain="testtenant.localhost", is_primary=True)
...
def test_password_reset_request_200_success(mock_send_email, api_client, tenant_with_user):
    ...
    assert "/api/v1/landing/auth/password-reset/confirm/" in call_args[0][2]  # reset_url
...
def test_password_reset_validate_400_invalid_token(api_client, tenant_with_user):
    ...
    assert response.status_code == status.HTTP_400_BAD_REQUEST
    assert "inválido" in response.data["detail"].lower() or "expirado" in response.data["detail"].lower()
...
def test_password_reset_confirm_400_invalid_token(api_client, tenant_with_user):
    ...
    assert response.status_code == status.HTTP_400_BAD_REQUEST
    assert "inválido" in str(response.data).lower() or "expirado" in str(response.data).lower()
