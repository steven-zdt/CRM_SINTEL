...
⚠️ OBJETIVO CRÍTICO: Garantizar que la raíz de un tenant (/) siempre muestre
la Landing Page Pública con status 200 OK para usuarios anónimos, sin redirección
automática al login.

⚠️ REQUISITO DE NEGOCIO:
...
class TestRoutingGuarantee(SintelTenantTestCase):
    ...
    ⚠️ INQUEBRANTABLE: Este test debe pasar SIEMPRE. Si falla, significa que
    ...
    def setUp(self):
        ...
        # ⚠️ VITAL: Configurar HTTP_HOST para que el middleware de routing funcione
    ...
    def test_anonymous_user_root_access_200_ok(self):
        ...
        ⚠️ ASSERT OBLIGATORIO 1: Status debe ser 200 OK
        ⚠️ ASSERT OBLIGATORIO 2: Template debe ser tenant/landing/index.html
        ⚠️ ASSERT DE NEGACIÓN: NO debe haber redirección (status no es 301 ni 302)
        ...
        self.assertEqual(
            ...
            200,
            "❌ VIOLACIÓN DE REGLA: La raíz debe retornar 200 OK para usuarios anónimos. "
        ...
        # ⚠️ ASSERT DE NEGACIÓN: NO debe haber redirección
        self.assertNotIn(
            response.status_code,
            [301, 302],
            "❌ VIOLACIÓN DE REGLA: La raíz NO debe redirigir (301/302) a usuarios anónimos. "
        ...
        self.assertTemplateUsed(
            ...
            msg_prefix="❌ VIOLACIÓN DE REGLA: El template usado debe ser 'tenant/landing/index.html'. "
        ...
        self.assertIn(
            ...
            "❌ VIOLACIÓN DE REGLA: La respuesta debe ser HTML, no una redirección."
        )
    ...
    def test_anonymous_user_no_redirect_to_login(self):
        ...
        self.assertNotEqual(
            response.status_code,
            302,
            "❌ VIOLACIÓN DE REGLA: La raíz NO debe redirigir (302) a usuarios anónimos."
        )
        ...
        if hasattr(response, 'url'):
            self.assertNotIn(
                'login',
                response.url.lower(),
                "❌ VIOLACIÓN DE REGLA: La respuesta NO debe redirigir a ninguna URL de login. "
        ...
        self.assertNotIn(
            'Location',
            response,
            "❌ VIOLACIÓN DE REGLA: La respuesta NO debe tener header 'Location' "
    ...
    def test_authenticated_user_redirects_to_dashboard(self):
        ...
        ⚠️ ASSERT: Debe redirigir (302) a /dashboard/
        ...
        # ⚠️ ASSERT: Debe redirigir (302) al dashboard
        self.assertEqual(
            response.status_code,
            302,
            "❌ VIOLACIÓN DE REGLA: Usuarios autenticados deben ser redirigidos (302) "
        ...
        self.assertIn(
            '/dashboard/',
            response.url,
            "❌ VIOLACIÓN DE REGLA: La redirección debe ser a '/dashboard/', no a otra URL. "
    ...
    def test_anonymous_user_can_access_landing_template(self):
        ...
        self.assertTrue(
            has_html,
            "❌ VIOLACIÓN DE REGLA: El template debe renderizar HTML válido. "
    ...
    def test_root_url_resolves_to_landing_view(self):
        ...
        self.assertEqual(
            ...
            TenantLandingView,
            "❌ VIOLACIÓN DE REGLA: La URL raíz (/) debe resolver a TenantLandingView. "
        ...
        self.assertEqual(
            match.url_name,
            'index',
            "❌ VIOLACIÓN DE REGLA: El nombre de la URL debe ser 'index'. "
        ...
        self.assertEqual(
            match.app_name,
            'tenant_landing',
            "❌ VIOLACIÓN DE REGLA: El app_name debe ser 'tenant_landing'. "
            "Si es otro nombre, significa que la configuración de URLs está incorrecta."
        )
