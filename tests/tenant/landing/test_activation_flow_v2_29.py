...
[WARNING] v2.29: Cambios en el comportamiento de activación:
...
def test_activate_get_with_valid_token_and_usable_password(
    client, tenant_factory, user_factory
):
    """
    [WARNING] v2.29: GET /activate con token válido y usuario con password usable
    ...
    user.set_password("OldPassword123!")  # [WARNING] Password usable
    user.save()
    ...
    assert "activate.html" in [
        t.name for t in response.templates
    ], "Debe renderizar activate.html"
    assert "form" in response.context, "Debe incluir el formulario en el contexto"


@pytest.mark.django_db
def test_activate_post_updates_password_even_if_usable(
    client, tenant_factory, user_factory
):
    """
    [WARNING] v2.29: POST /activate permite actualizar la contraseña incluso si el usuario ya tiene una usable.
    """
    # Crear tenant y usuario CON password usable
    tenant = tenant_factory(schema_name="acme", nombre="Acme SAS")
    domain = Domain.objects.create(
    ...
    user = user_factory(email="owner@acme.com")
    user.set_password("OldPassword123!")  # [WARNING] Password usable existente
    user.save()
    ...
    ), "Debe redirigir después de actualización exitosa"
    assert (
        "http://acme.localhost/dashboard/" in response.url
        or "https://acme.localhost/dashboard/" in response.url
    ...
    assert user.check_password(
        "NewPassword123!"
    ), "El password debe ser el nuevo establecido"
    assert not user.check_password(
        "OldPassword123!"
    ), "El password antiguo no debe funcionar"


@pytest.mark.django_db
def test_activate_post_redirect_absolute_url(client, tenant_factory, user_factory):
    """
    [WARNING] v2.29: POST /activate redirige con URL absoluta al dashboard del tenant.
    """
    ...
    assert response.url.startswith("http://") or response.url.startswith(
        "https://"
...
def test_activate_token_one_time_use(client, tenant_factory, user_factory):
    """
    [WARNING] v2.29: Token puede usarse múltiples veces mientras sea válido (no expirado).
    ...
    # [WARNING] Nota: Actualmente el token no se invalida, solo expira por tiempo
