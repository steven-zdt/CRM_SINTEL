...
def test_activate_post_success(client, tenant_factory, user_factory):
    ...
    ), "El usuario debe tener password usable después de activación"
    ...
    assert response.status_code in (
        200,
        302,
    ), "El usuario debe poder acceder al dashboard"
...
def test_activate_cross_tenant_isolation(client, tenant_factory, user_factory):
    ...
    domain_a = Domain.objects.create(
        domain="acme.localhost", tenant=tenant_a, is_primary=True
    )

    tenant_b = tenant_factory(schema_name="beta", nombre="Beta SAS")
    domain_b = Domain.objects.create(
        domain="beta.localhost", tenant=tenant_b, is_primary=True
    )
    ...
    ), "Debe redirigir a /login/ (token no corresponde al tenant)"
