...
def test_login_success_redirects_to_dashboard(client, tenant_with_domain, user_owner, membership):
    """
    ...
    assert location.endswith("/dashboard/") or "/dashboard/" in location, \
        f"Se esperaba redirección a /dashboard/, pero se recibió: {location}"


def test_forbidden_without_membership(client, tenant_with_domain, user_without_membership):
    """
    ...
    assert dashboard_response.status_code in (302, 403), \
...
def test_login_with_invalid_credentials(client, tenant_with_domain):
    ...
    assert "error" in content.lower() or "inválid" in content.lower() or "incorrect" in content.lower(), \
        "Se esperaba un mensaje de error en la respuesta"


def test_authenticated_user_redirected_from_login(client, tenant_with_domain, user_owner, membership):
    """
