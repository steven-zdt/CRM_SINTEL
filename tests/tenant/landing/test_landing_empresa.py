...
class TestLandingEmpresaAPI:
    ...
    def test_api_empresa_publica_sin_autenticacion(self):
        ...
        assert 'nit' not in data, "El campo 'nit' NO debe estar presente (información sensible)"
        assert 'dv' not in data, "El campo 'dv' NO debe estar presente (información sensible)"
        assert 'regimen_tributario' not in data, "El campo 'regimen_tributario' NO debe estar presente (información sensible)"
        assert 'direccion' not in data, "El campo 'direccion' NO debe estar presente (información privada)"
        assert 'telefono' not in data, "El campo 'telefono' NO debe estar presente (información privada)"
    ...
    def test_api_empresa_estado_cero(self):
        ...
        with override_settings(ROOT_URLCONF=settings.TENANT_URLCONF):
            ...
            assert 'empresa' in data['error'].lower() or 'configurada' in data['error'].lower(), (
...
class TestLandingViewTrafficController:
    ...
    def test_redireccion_usuario_autenticado(self):
        ...
        with schema_context(get_public_schema_name()):
            tenant = TenantClient.objects.create(
                schema_name='test_traffic',
                nombre='Test Traffic',
                is_active=True
            )
            Domain.objects.create(
                tenant=tenant,
                domain='test-traffic.com',
                is_primary=True
            )
    ...
    def test_renderizado_usuario_anonimo(self):
        ...
        with schema_context(get_public_schema_name()):
            tenant = TenantClient.objects.create(
                schema_name='test_render',
                nombre='Test Render',
                is_active=True
            )
            Domain.objects.create(
                tenant=tenant,
                domain='test-render.com',
                is_primary=True
            )
        ...
        with override_settings(ROOT_URLCONF=settings.TENANT_URLCONF):
            ...
            assert 'empresa_configurada' in response.context, (
                "El contexto debe incluir 'empresa_configurada'"
            )
            assert response.context['empresa_configurada'] == True, (
                "empresa_configurada debe ser True cuando existe empresa"
            )
            assert 'razon_social' in response.context, (
    ...
    def test_estado_cero_sin_empresa(self):
        ...
        with schema_context(get_public_schema_name()):
            ...
            Domain.objects.create(
                tenant=tenant,
                domain='test-estado-cero-view.com',
                is_primary=True
            )
        ...
        with override_settings(ROOT_URLCONF=settings.TENANT_URLCONF):
            ...
            assert response.context['empresa_configurada'] == False, (
                "empresa_configurada debe ser False cuando no existe empresa"
            )
            
            # Verificar que el HTML contiene el mensaje genérico
            content = response.content.decode('utf-8')
            assert 'nuevo espacio de trabajo' in content.lower() or 'bienvenido' in content.lower(), (
...
class TestLandingTemplateEmpresa:
    ...
    def test_template_con_logo(self):
        ...
        with schema_context(get_public_schema_name()):
            tenant = TenantClient.objects.create(
                schema_name='test_logo',
                nombre='Test Logo',
                is_active=True
            )
            Domain.objects.create(
                tenant=tenant,
                domain='test-logo.com',
                is_primary=True
            )
        ...
        with override_settings(ROOT_URLCONF=settings.TENANT_URLCONF):
            ...
            assert 'Empresa con Logo S.A.' in content, (
    ...
    def test_template_sin_logo_muestra_razon_social(self):
        ...
        with schema_context(get_public_schema_name()):
            tenant = TenantClient.objects.create(
                schema_name='test_sin_logo',
                nombre='Test Sin Logo',
                is_active=True
            )
            Domain.objects.create(
                tenant=tenant,
                domain='test-sin-logo.com',
                is_primary=True
            )
        ...
        with override_settings(ROOT_URLCONF=settings.TENANT_URLCONF):
            ...
            assert 'Empresa Sin Logo S.A.' in content, (
                "La razón social debe estar presente en el HTML"
            )
            # Verificar que hay un botón de login
            assert 'Iniciar Sesión' in content or 'Acceder' in content, (
