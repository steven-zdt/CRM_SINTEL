...
⚠️ OBJETIVO: Verificar el flujo completo de activación desde onboarding hasta activación exitosa.
...
class TestActivationDevFlow:
    ...
    def test_onboarding_creates_owner_with_unusable_password(self, tenant_factory, user_factory):
        """
        ...
        domain = Domain.objects.create(domain="testcorp.localhost", tenant=tenant, is_primary=True)
        
        # Simular creación de owner como en onboarding
        user = user_factory(email="owner@testcorp.com")
        user.set_unusable_password()  # ⚠️ CRÍTICO: Como en onboarding
        user.save()
        
        # Verificar que NO tiene contraseña usable
        assert not user.has_usable_password(), "El owner debe crearse sin contraseña usable"
    ...
    def test_generate_activation_token_for_unusable_user(self, tenant_factory, user_factory):
        """
        ...
        assert "/api/v1/landing/auth/activate/" in activation_url, "La URL debe apuntar a la API"
        assert f"token={token}" in activation_url, "La URL debe incluir el token"
        assert "testcorp.localhost" in activation_url, "La URL debe incluir el dominio del tenant"
    
    def test_get_activation_endpoint_with_unusable_password(self, tenant_factory, user_factory):
        """
    ...
    def test_get_activation_endpoint_with_usable_password_returns_409(self, tenant_factory, user_factory):
        """
        ...
        domain = Domain.objects.create(domain="testcorp.localhost", tenant=tenant, is_primary=True)
        user = user_factory(email="owner@testcorp.com")
        user.set_password("ExistingPassword123!")  # ⚠️ Contraseña usable
        user.save()
        ...
        # ⚠️ v2.30: Debe retornar 409 CONFLICT
    ...
    def test_post_activation_endpoint_successful_activation(self, tenant_factory, user_factory):
        """
        ...
        assert user.has_usable_password(), "El usuario debe tener contraseña usable después de activación"
        assert user.check_password("NewPassword123!"), "La contraseña debe ser la establecida"
    ...
    def test_activation_url_points_to_api_endpoint(self, tenant_factory, user_factory):
        ...
        # Verificar que la URL apunta a la API
        assert "/api/v1/landing/auth/activate/" in activation_url, \
        ...
        assert "/activate?" not in activation_url or "/api/v1/landing/auth/activate/" in activation_url, \
