...
class TestTenantLoginUnboundLocalError:
    ...
    def test_redirect_url_siempre_definida_en_exito(self):
        ...
        for email, expected_role in [
            ...
            assert response.status_code == status.HTTP_200_OK, f"Login falló para {email}"
            assert 'redirect_url' in response.data, f"redirect_url no está en respuesta para {email}"
            assert response.data['redirect_url'] is not None, f"redirect_url es None para {email}"
            assert response.data['redirect_url'] != '', f"redirect_url está vacía para {email}"
