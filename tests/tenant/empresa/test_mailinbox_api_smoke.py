...
⚠️ v2.37: Alineado con arquitectura SSoT y API-First.
"""
...
class MailInboxConfigAPISmokeTest(TestCase):
    ...
    def test_list_endpoint_exists(self):
        ...
        self.assertIn(response.status_code, [401, 403, 404])  # 404 si no está registrado
    ...
    def test_test_connection_endpoint_exists(self):
        ...
        # Sin autenticación, debería retornar 401 o 403
        response = self.client.post('/api/v1/empresa/mail-inbox-config/test-connection/')
        self.assertIn(response.status_code, [401, 403, 404])
