...
[WARNING] v2.37: Alineado con arquitectura API-First y UI modular.
"""
...
class TestEmpresaModalesSmoke(SintelTenantTestCase):
    ...
    [WARNING] IMPORTANTE: Estos tests validan que:
    ...
    def test_api_detail_returns_correct_fields(self):
        ...
        response = self.client.get(
            f"/api/v1/empresas/{empresa.id}/", HTTP_ACCEPT="application/json"
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_list_serializer_exposes_minimal_fields(self):
        ...
        # [WARNING] NOTA: 'id' ahora está en LIST para poder hacer retrieve desde el frontend
        prohibited_fields = [
            "razon_social",
            "logo",
            "website",
        ...
        for field in prohibited_fields:
            self.assertNotIn(
                field, empresa_data, f"Campo '{field}' NO debe estar en LIST"
            )
