...
[WARNING] POLÍTICA API-First: La presentación está en shells estáticos que consumen la API.
"""
...
class TestEmpresasShellConsumesAPI(TestCase):
    ...
    def test_empresas_shell_exists(self):
        ...
        self.assertTrue(
            shell_path.exists(), f"El shell estático no existe en: {shell_path}"
        )
    ...
    def test_empresas_shell_consumes_api(self):
        ...
        self.assertTrue(
            found, "El shell estático debe consumir la API /api/v1/empresas/"
        )
    ...
    def test_empresas_shell_no_absolute_urls(self):
        ...
        absolute_url_pattern = re.compile(
            r'(http://|https://)(?!cdn\.tailwindcss\.com|cdnjs\.cloudflare\.com)[^\s"\'<>]+'
        )
        matches = absolute_url_pattern.findall(content)

        # Filtrar matches que sean CDNs permitidos
        forbidden_matches = [
            m for m in matches if not any(cdn in str(m) for cdn in allowed_absolute)
        ]
