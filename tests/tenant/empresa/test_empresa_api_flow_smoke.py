...
class EmpresaAPIFlowSmokeTestCase(SintelTenantTestCase):
    ...
    def test_patch_empresa_persists_changes(self):
        ...
        # Obtener CSRF token
        csrf_token = self.client.get('/empresa/', HTTP_HOST='test-tenant.sintel.com').cookies.get('csrftoken')
        ...
        response = self.client.patch(
            ...
            HTTP_HOST='test-tenant.sintel.com',
        ...
        get_response = self.client.get(get_url, HTTP_HOST='test-tenant.sintel.com')
        get_data = json.loads(get_response.content)
        
        if get_data.get('empresa'):
            self.assertEqual(get_data['empresa']['razon_social'], 'Test Empresa Actualizada S.A.')
