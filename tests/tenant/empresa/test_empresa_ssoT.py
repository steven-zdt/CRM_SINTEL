...
[WARNING] POLÍTICA SSoT: apps/tenant/empresa/api es la ÚNICA fuente de datos empresariales.
"""
...
class TestEmpresaSSoT(SintelTenantTestCase):
    ...
    def test_empresa_api_is_canonical(self):
        ...
        for field in canonical_fields:
            self.assertIn(
                field, empresa, f"Campo canónico '{field}' no está en la respuesta"
            )
    ...
    def test_empresa_service_provider(self):
        ...
        self.assertEqual(
            empresa_data["nit_completo"], f"{empresa_data['nit']}-{empresa_data['dv']}"
        )
