...
@pytest.mark.django_db
def test_endpoint_200_when_authenticated_with_membership(client, django_user_model, tenant):
    """
    ...
    # Debe responder 200
    assert resp.status_code == 200, f"Debe responder 200 cuando el usuario tiene membresía. Got {resp.status_code}"
...
@pytest.mark.django_db
def test_endpoint_403_when_authenticated_without_membership(client, django_user_model, tenant):
    """
    ...
    user = django_user_model.objects.create(username="testuser2", email="test2@example.com")
    ...
    assert resp.status_code in (401, 403), f"Debe responder 401 o 403 cuando no hay membresía. Got {resp.status_code}"
