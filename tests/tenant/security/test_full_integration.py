...
⚠️ OBJETIVO: Este test valida que todos los componentes de seguridad funcionan
...
class TestFullIntegrationSecurity(SintelTenantTestCase):
    ...
    def test_1_barrera_de_backend_nivel_api_core(self):
        ...
        # ⚠️ ASSERT OBLIGATORIO: El backend debe retornar None
        self.assertIsNone(
            user,
            "❌ VIOLACIÓN DE SEGURIDAD: El backend TenantAwareBackend debe retornar None "
    ...
    def test_2_feedback_de_formulario_nivel_ux(self):
        ...
        # ⚠️ ASSERT OBLIGATORIO 1: NO debe haber redirección (login fallido)
        self.assertNotEqual(
            response.status_code,
            302,
            "❌ VIOLACIÓN DE UX: El login debe fallar (200 OK con errores), no redirigir (302). "
            "Si redirige, significa que el login fue exitoso, lo cual es un fallo de seguridad."
        )
        
        # ⚠️ ASSERT OBLIGATORIO 2: Debe haber errores en el formulario
        form = response.context.get('form') if hasattr(response, 'context') else None
        self.assertIsNotNone(
            form,
            "❌ VIOLACIÓN DE UX: El formulario debe estar en el contexto de la respuesta."
        )
        
        self.assertTrue(
            form.errors,
            "❌ VIOLACIÓN DE UX: El formulario DEBE tener errores cuando un usuario "
            "sin membresía intenta loguearse."
        )
        
        # ⚠️ ASSERT OBLIGATORIO 3: El mensaje debe ser específico sobre membresía
        error_messages = []
        ...
        has_membership_error = (
            'no tienes acceso' in error_text or
            'acceso a la empresa' in error_text or
            'contacta a tu administrador' in error_text or
            'membresía' in error_text
        )
        
        self.assertTrue(
            has_membership_error,
            f"❌ VIOLACIÓN DE UX: El mensaje de error debe ser específico sobre membresía/acceso. "
        ...
        # ⚠️ ASSERT OBLIGATORIO 4: NO debe contener el mensaje genérico de Django
        has_generic_error = (
            'por favor, introduzca un nombre de usuario y clave correctos' in error_text or
            'username and password' in error_text.lower()
        )
        
        self.assertFalse(
            has_generic_error,
            f"❌ VIOLACIÓN DE UX: El mensaje NO debe ser genérico de Django. "
    ...
    def test_3_acceso_exitoso_y_perfil_happy_path(self):
        ...
        # ⚠️ ASSERT OBLIGATORIO 1: Debe haber redirección (login exitoso)
        self.assertEqual(
            response.status_code,
            302,
            f"❌ VIOLACIÓN: El login exitoso debe redirigir (302). Status recibido: {response.status_code}"
        )
        
        # ⚠️ ASSERT OBLIGATORIO 2: Debe redirigir al dashboard
        self.assertIn(
            '/dashboard/',
            response.url,
            f"❌ VIOLACIÓN: Después de login exitoso, debe redirigir a /dashboard/. URL recibida: {response.url}"
        )
        
        # ⚠️ ASSERT OBLIGATORIO 3: El usuario debe estar autenticado en la sesión
        ...
        self.assertNotEqual(
            ...
            302,
            "❌ VIOLACIÓN: Después de login exitoso, el usuario debe poder acceder al dashboard. "
            "Si redirige al login, significa que el usuario no está autenticado."
        )
        
        # ⚠️ ASSERT OBLIGATORIO 4: Verificar que se puede consultar el TenantProfile
        # (Opcional, pero valida la integración con el sistema de perfiles)
        try:
            perfil = obtener_o_crear_perfil(self.user)
            self.assertIsNotNone(
                perfil,
                "❌ VIOLACIÓN: Después de login exitoso, debe poder consultarse el TenantProfile del usuario."
            )
        except Exception as e:
            # Si falla, no es crítico para este test, pero es bueno validarlo
            self.fail(f"❌ VIOLACIÓN: No se pudo consultar el TenantProfile después del login: {e}")
    ...
    def test_4_proteccion_de_tenant_publico(self):
        ...
        domain_public = Domain.objects.filter(tenant=tenant_public, is_primary=True).first()
        ...
        # ⚠️ ASSERT OBLIGATORIO: El backend debe permitir acceso al tenant público
        self.assertIsNotNone(
            user,
            "❌ VIOLACIÓN: El backend debe permitir acceso al tenant 'public' sin membresía."
        )
        self.assertEqual(
            user.id,
            superuser.id,
    ...
    def test_5_aislamiento_cruzado_cross_check(self):
        ...
        # ⚠️ CASO 1: Usuario A intenta loguearse en Tenant B (donde NO tiene membresía)
        login_url = reverse('tenant_landing:login')
        ...
        self.assertNotEqual(
            response.status_code,
            302,
            "❌ VIOLACIÓN DE AISLAMIENTO: El login cross-tenant debe fallar, no redirigir."
        )
        
        # ⚠️ ASSERT OBLIGATORIO 2: Debe haber errores en el formulario
        form = response.context.get('form') if hasattr(response, 'context') else None
        self.assertIsNotNone(form, "❌ VIOLACIÓN: El formulario debe estar en el contexto.")
        self.assertTrue(
            form.errors,
            "❌ VIOLACIÓN DE AISLAMIENTO: El formulario debe tener errores en login cross-tenant."
        )
        
        # ⚠️ CASO 2: Verificar que el backend también bloquea directamente
        from django.test import RequestFactory
        factory = RequestFactory()
        request = factory.get('/')
        ...
        # ⚠️ ASSERT OBLIGATORIO 3: El backend debe retornar None
        self.assertIsNone(
            user,
            "❌ VIOLACIÓN DE AISLAMIENTO: El backend debe retornar None para login cross-tenant."
        )
        
        # ⚠️ CASO 3: Usuario B intenta loguearse en Tenant A (donde NO tiene membresía)
        response = self.client_tenant_a.post(
            login_url,
            {
        ...
        # ⚠️ ASSERT OBLIGATORIO 4: NO debe haber redirección
        self.assertNotEqual(
            response.status_code,
            302,
            "❌ VIOLACIÓN DE AISLAMIENTO: El login cross-tenant (B->A) debe fallar."
        )
        
        # ⚠️ ASSERT OBLIGATORIO 5: Debe haber errores
        form = response.context.get('form') if hasattr(response, 'context') else None
        self.assertTrue(
            form.errors if form else False,
            "❌ VIOLACIÓN DE AISLAMIENTO: El formulario debe tener errores en login cross-tenant (B->A)."
        )
