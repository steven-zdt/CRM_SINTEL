...
⚠️ CRÍTICO: Este test valida que los tenants privados NO pueden acceder
...
class PublicRouteIsolationPenetrationTests(SintelTenantTestCase):
    ...
    ⚠️ OBJETIVO: Garantizar que rutas exclusivas del esquema público
    (como /console/) devuelvan 404 Not Found cuando se accede desde
    un tenant privado.
    
    ⚠️ REGLA DE ORO: Si devuelve 200, 302 o 403, significa que la ruta
    ...
    def test_console_route_returns_404_in_private_tenant(self):
        ...
        ⚠️ RESULTADO ESPERADO: 404 Not Found
        ⚠️ FALLO CRÍTICO: Si devuelve 200, 302 o 403, la ruta EXISTE en el tenant.
        """
        # Intentar acceder a /console/ desde tenant privado
        response = self.client.get('/console/')
        
        # ⚠️ VALIDACIÓN CRÍTICA: Debe devolver 404 (ruta no existe)
        self.assertEqual(
            response.status_code,
            404,
            "❌ VULNERABILIDAD: /console/ existe en tenant privado. "
        ...
        self.assertNotEqual(
            response.status_code,
            302,
            "❌ VULNERABILIDAD: /console/ redirige en tenant privado. "
        ...
        self.assertNotEqual(
            response.status_code,
            403,
            "❌ VULNERABILIDAD: /console/ existe pero está protegida en tenant privado. "
    ...
    def test_console_tenants_route_returns_404_in_private_tenant(self):
        ...
        self.assertEqual(
            response.status_code,
            404,
            "❌ VULNERABILIDAD: /console/tenants/ existe en tenant privado. "
            "Debe devolver 404 Not Found."
        )
    ...
    def test_public_api_route_returns_404_in_private_tenant(self):
        ...
        self.assertEqual(
            response.status_code,
            404,
            "❌ VULNERABILIDAD: /api/public/v1/ existe en tenant privado. "
            "Debe devolver 404 Not Found."
        )
    ...
    def test_admin_api_route_returns_404_in_private_tenant(self):
        ...
        self.assertEqual(
            response.status_code,
            404,
            "❌ VULNERABILIDAD: /api/admin/v1/ existe en tenant privado. "
            "Debe devolver 404 Not Found."
        )
    ...
    def test_control_dashboard_route_works_in_private_tenant(self):
        ...
        ⚠️ RESULTADO ESPERADO: 200 OK (si autenticado) o 302 Redirect (si no autenticado)
        ...
        # ⚠️ VALIDACIÓN: Debe funcionar (200 o 302, NO 404)
        self.assertIn(
            response.status_code,
            [200, 302],
            "✅ Control: /dashboard/ debe funcionar en tenant privado. "
        ...
        if response.status_code == 200:
            self.assertNotEqual(
                ...
                0,
                "✅ Control: /dashboard/ debe retornar contenido en tenant privado."
            )
    ...
    def test_control_landing_route_works_in_private_tenant(self):
        ...
        ⚠️ RESULTADO ESPERADO: 200 OK
        Esto confirma que el tenant está activo y las rutas privadas funcionan.
        """
        # Acceder a / desde tenant privado
        response = self.client.get('/')
        
        # ⚠️ VALIDACIÓN: Debe funcionar (200, NO 404)
        self.assertEqual(
            response.status_code,
            200,
            "✅ Control: / (landing) debe funcionar en tenant privado. "
            "Si devuelve 404, hay un problema con el routing del tenant."
        )
    ...
    def test_control_tenant_api_route_works_in_private_tenant(self):
        ...
        ⚠️ RESULTADO ESPERADO: 200 OK o 401/403 (según autenticación)
        ...
        # ⚠️ VALIDACIÓN: Debe funcionar (200, 401, 403, NO 404)
        self.assertIn(
            response.status_code,
            [200, 401, 403],
            "✅ Control: /api/v1/ debe funcionar en tenant privado. "
            "Si devuelve 404, hay un problema con el routing del tenant."
        )
    ...
    def test_console_route_with_authenticated_user_still_returns_404(self):
        ...
        ⚠️ RESULTADO ESPERADO: 404 Not Found (incluso con usuario autenticado)
        ⚠️ IMPORTANTE: El usuario puede ser staff, pero la ruta NO debe existir en el tenant.
        """
        ...
        self.assertEqual(
            response.status_code,
            404,
            "❌ VULNERABILIDAD: /console/ existe en tenant privado incluso con usuario autenticado. "
    ...
    def test_multiple_public_routes_return_404(self):
        ...
        ⚠️ RESULTADO ESPERADO: Todas deben devolver 404 Not Found
        """
        public_routes = [
            '/console/',
            '/console/tenants/',
        ...
        for route in public_routes:
            with self.subTest(route=route):
                ...
                self.assertEqual(
                    response.status_code,
                    404,
                    f"❌ VULNERABILIDAD: {route} existe en tenant privado. "
                    "Debe devolver 404 Not Found."
                )
