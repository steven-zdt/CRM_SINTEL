...
⚠️ CRÍTICO: Estas pruebas validan que el aislamiento entre tenants
...
class CrossTenantIsolationTests(TenantTestCase):
    ...
    def test_cross_access_view_denied(self):
        ...
        has_error_message = (
            'permisos' in content_lower or
            'acceso' in content_lower or
            'denied' in content_lower or
    ...
    def test_cross_access_api_denied(self):
        ...
        if response.status_code == 403:
            self.assertIn(
                ...
                'denied' in str(response.data).lower(),
                True,
                "El mensaje de error debe indicar que el acceso fue denegado"
            )
    ...
    def test_public_access_redirects_to_login(self):
        ...
        self.assertIn(
            ...
            '/' in response.url,  # Puede redirigir a la landing page
            True,
    ...
    def test_leak_test_tenant_a_data_not_in_tenant_b(self):
        ...
        empresas = response.data if isinstance(response.data, list) else response.data.get('results', [])
        
        self.assertEqual(
            len(empresas),
            1,
            "Debe haber solo una empresa en Tenant A"
        )
    ...
    def test_user_a_can_access_own_tenant(self):
        ...
        self.assertGreaterEqual(
            len(empresas),
            1,
            "Usuario A debe poder ver los datos de su propio tenant"
        )
