...
class TestDashboardAccess(SintelTenantTestCase):
    ...
    def test_anonymous_redirect(self):
        ...
        [WARNING] IMPORTANTE: LoginRequiredMixin debe redirigir usuarios no autenticados ANTES
        de que se ejecute el método dispatch que verifica la membresía.
        """
        # Cliente anónimo (sin autenticación)
        # [WARNING] CRÍTICO: Configurar HTTP_HOST para que el middleware de routing funcione
        ...
        self.assertTrue(
            "/login" in redirect_url
            or redirect_url == "/"
            or "/login/" in redirect_url
    ...
    def test_cross_tenant_access_denied(self):
        ...
        [WARNING] Esto valida que estar logueado no es suficiente; se requiere membresía.
        """
        ...
        other_tenant = TenantClient.objects.create(
            schema_name="other_tenant", nombre="Other Tenant", is_active=True
        )
        Domain.objects.create(
            tenant=other_tenant, domain="other-tenant.sintel.local", is_primary=True
        )
        # Crear membresía para el otro tenant (NO para el tenant actual)
        TenantMembership.objects.create(
            client=other_tenant, user=user_hacker, rol="ADMIN"
        )
        ...
        has_error_message = (
            "permisos" in content
            or "acceso" in content
            or "denied" in content
            or "forbidden" in content
            or "membresía" in content
            or "membership" in content
            or "403" in content
            or "denegado" in content
        )
        self.assertTrue(
            has_error_message,
    ...
    def test_authorized_access(self):
        ...
        if (
            ...
            and hasattr(response, "templates")
            and response.templates
        ):
            template_names = [t.name for t in response.templates if hasattr(t, "name")]
            has_dashboard_template = any(
                "dashboard" in name.lower() for name in template_names
            )
            self.assertTrue(
