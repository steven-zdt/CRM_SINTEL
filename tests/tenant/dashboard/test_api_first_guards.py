...
class TestAPIFirstGuards(TestCase):
                ...
                if 'TemplateView' in content:
                    ...
                    try:
                        ...
                        for node in ast.walk(tree):
                            if isinstance(node, ast.ClassDef):
                                for base in node.bases:
                                    if isinstance(base, ast.Name) and base.id == 'TemplateView':
                                        self.fail(
                                            f"❌ TemplateView encontrado en {file_path}: "
                        ...
                        if 'class' in content and 'TemplateView' in content:
                            self.fail(
                                f"❌ Posible TemplateView en {file_path}. "
                        ...
                        for i, line in enumerate(lines, 1):
                            if pattern in line and not line.strip().startswith('#'):
                                ...
                                if '"""' not in line and "'''" not in line:
                                    self.fail(
                                        f"❌ Llamada a {pattern} encontrada en {file_path}:{i}. "
                                        f"Usa API-First en su lugar."
                                    )
    ...
    def test_no_templates_in_dashboard(self):
        ...
        if templates_path.exists():
            ...
            if html_files:
                self.fail(
                    f"❌ Templates HTML encontrados en apps/tenant/dashboard/templates/: "
    ...
    def test_dashboard_urls_only_redirect_to_static(self):
        ...
        if not urls_file.exists():
            self.fail("❌ apps/tenant/dashboard/urls.py no existe.")
        ...
        with open(urls_file, 'r', encoding='utf-8') as f:
            ...
            if 'TemplateView' in content:
                self.fail(
                    "❌ TemplateView encontrado en apps/tenant/dashboard/urls.py. "
    ...
    def test_dashboard_views_only_redirect(self):
        ...
        with open(views_file, 'r', encoding='utf-8') as f:
            ...
            if 'TemplateView' in content:
                self.fail(
                    "❌ TemplateView encontrado en apps/tenant/dashboard/views.py. "
                    "Solo se permiten RedirectView para redirigir a shells estáticos."
                )
            
            if 'render(' in content or 'render_to_response(' in content:
                self.fail(
                    "❌ Llamadas a render() encontradas en apps/tenant/dashboard/views.py. "
