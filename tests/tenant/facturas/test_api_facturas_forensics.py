...
class TestFacturasAPIForensics(SintelTenantTestCase):
    ...
    def test_upload_missing_xml_returns_400(self):
        ...
        data = r.json()
        assert data == {"error": "missing_xml", "message": "Falta archivo o contenido XML."}
    ...
    def test_upload_preview_ok(self):
        ...
        f.name = "inv.xml"
        r = self.api_client.post(url, data={"file": f, "naturaleza": "VENTA"}, format="multipart")
    ...
    def test_upload_invalid_naturaleza_returns_400(self):
        ...
        f = io.BytesIO(SAMPLE_INVOICE)
        f.name = "inv.xml"
        r = self.api_client.post(url, data={"file": f, "naturaleza": "INVALIDO"}, format="multipart")
