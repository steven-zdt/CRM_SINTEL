...
class TestPerfilService(SintelTenantTestCase):
    ...
    def test_a_crear_perfil_via_service_layer(self):
        ...
        self.assertIsNotNone(
            perfil, "[ERROR] VIOLACIÓN: El servicio debe crear el perfil si no existe."
        )
        self.assertEqual(
            perfil.user,
            self.user,
            "[ERROR] VIOLACIÓN: El perfil debe estar asociado al usuario correcto.",
        )
        self.assertEqual(
            perfil.cargo,
            "Contador Senior",
            "[ERROR] VIOLACIÓN: El cargo debe ser el especificado.",
        )
        self.assertEqual(
            perfil.departamento,
            "Finanzas",
            "[ERROR] VIOLACIÓN: El departamento debe ser el especificado.",
        )
        self.assertEqual(
            perfil.telefono_corporativo,
            "6012345678",
            "[ERROR] VIOLACIÓN: El teléfono debe ser el especificado.",
        )
        ...
        self.assertIsInstance(
            ...
            dict,
            "[ERROR] VIOLACIÓN: La configuración debe ser un diccionario.",
        )
    ...
    def test_a_obtener_perfil_existente_via_service_layer(self):
        ...
        perfil_original = TenantProfile.objects.create(
            user=self.user, cargo="Contador Senior", departamento="Finanzas"
        )
        ...
        self.assertEqual(
            perfil.cargo,
            "Contador Senior",
            "[ERROR] VIOLACIÓN: El cargo debe ser el del perfil existente.",
        )
    ...
    def test_a_actualizar_configuracion_ui(self):
        ...
        perfil_actualizado = actualizar_configuracion_ui(
            self.user, "modo_oscuro", True, merge=True
        )
...
class TestPerfilAPI(SintelTenantTestCase):
    ...
    def test_b_consumir_endpoint_me_y_verificar_datos_combinados(self):
        ...
        data = response.json()
        self.assertIn(
            "id", data, "[ERROR] VIOLACIÓN: La respuesta debe incluir el ID del perfil."
        )
        self.assertIn(
            "cargo", data, "[ERROR] VIOLACIÓN: La respuesta debe incluir el cargo."
        )
        self.assertEqual(
            data["cargo"],
            "Contador Senior",
            "[ERROR] VIOLACIÓN: El cargo debe ser el correcto.",
        )
        self.assertEqual(
            data["departamento"],
            "Finanzas",
            "[ERROR] VIOLACIÓN: El departamento debe ser el correcto.",
        )
        self.assertEqual(
            data["telefono_corporativo"],
            "6012345678",
            "[ERROR] VIOLACIÓN: El teléfono debe ser el correcto.",
        )

        # Verificar que los datos del User global están presentes (nested)
        self.assertIn(
            "user_id", data, "[ERROR] VIOLACIÓN: La respuesta debe incluir user_id."
        )
        self.assertIn(
            "user_email",
            data,
            "[ERROR] VIOLACIÓN: La respuesta debe incluir user_email.",
        )
        self.assertIn(
            "user_username",
            data,
            "[ERROR] VIOLACIÓN: La respuesta debe incluir user_username.",
        )
        self.assertEqual(
            data["user_email"],
            self.user.email,
            "[ERROR] VIOLACIÓN: El email debe ser el del usuario actual.",
        )
        self.assertEqual(
            data["user_username"],
            self.user.username,
            "[ERROR] VIOLACIÓN: El username debe ser el del usuario actual.",
        )
    ...
    def test_b_actualizar_perfil_via_endpoint_me(self):
        ...
        self.assertEqual(
            data["cargo"],
            "Contador Senior",
            "[ERROR] VIOLACIÓN: El cargo debe ser actualizado.",
        )
        self.assertEqual(
            data["departamento"],
            "Finanzas",
            "[ERROR] VIOLACIÓN: El departamento debe ser actualizado.",
        )
        self.assertEqual(
            data["telefono_corporativo"],
            "6012345678",
            "[ERROR] VIOLACIÓN: El teléfono debe ser actualizado.",
        )
        ...
        self.assertEqual(
            perfil.cargo,
            "Contador Senior",
            "[ERROR] VIOLACIÓN: El cargo debe estar actualizado en la BD.",
        )
        self.assertEqual(
            perfil.departamento,
            "Finanzas",
            "[ERROR] VIOLACIÓN: El departamento debe estar actualizado en la BD.",
        )
...
class TestPerfilAislamiento(SintelTenantTestCase):
    ...
    def setUp(self):
        ...
        self.domain_b = Domain.objects.create(
            domain="tenant-b-perfil.sintel.local", tenant=self.tenant_b, is_primary=True
        )

        # Crear membresía del usuario en Tenant B
        self.membership_b = TenantMembership.objects.create(
            client=self.tenant_b, user=self.user, rol="ADMIN"
        )
    ...
    def test_c_verificar_aislamiento_via_api(self):
        ...
        connection.set_schema(self.tenant.schema_name)
        perfil_a = obtener_o_crear_perfil(
            self.user, defaults={"cargo": "Contador Senior", "departamento": "Finanzas"}
        )
        ...
        perfil_b = obtener_o_crear_perfil(
            self.user,
            defaults={"cargo": "Auxiliar Administrativo", "departamento": "RRHH"},
        )
