...
class TestPerfilService(SintelTenantTestCase):
    ...
    def test_a_crear_perfil_via_service_layer(self):
        ...
        self.assertIsNone(
            perfil_existente,
            "[ERROR] VIOLACIÓN: El perfil no debería existir antes de llamar al servicio."
        )
        ...
        self.assertIsNotNone(perfil, "[ERROR] VIOLACIÓN: El servicio debe crear el perfil si no existe.")
        self.assertEqual(perfil.user, self.user, "[ERROR] VIOLACIÓN: El perfil debe estar asociado al usuario correcto.")
        self.assertEqual(perfil.cargo, 'Contador Senior', "[ERROR] VIOLACIÓN: El cargo debe ser el especificado.")
        self.assertEqual(perfil.departamento, 'Finanzas', "[ERROR] VIOLACIÓN: El departamento debe ser el especificado.")
        self.assertEqual(perfil.telefono_corporativo, '6012345678', "[ERROR] VIOLACIÓN: El teléfono debe ser el especificado.")
        
        # Verificar que el perfil tiene configuración por defecto (dict vacío)
        self.assertIsInstance(perfil.configuracion, dict, "[ERROR] VIOLACIÓN: La configuración debe ser un diccionario.")
    ...
    def test_a_obtener_perfil_existente_via_service_layer(self):
        ...
        self.assertEqual(
            perfil.id,
            perfil_original.id,
            "[ERROR] VIOLACIÓN: El servicio debe retornar el perfil existente, no crear uno nuevo."
        )
        self.assertEqual(perfil.cargo, 'Contador Senior', "[ERROR] VIOLACIÓN: El cargo debe ser el del perfil existente.")
    ...
    def test_a_actualizar_configuracion_ui(self):
        ...
        self.assertEqual(
            ...
            True,
            "[ERROR] VIOLACIÓN: La configuración debe actualizar la clave especificada."
        )
        self.assertEqual(
            perfil_actualizado.configuracion['densidad_tablas'],
            'normal',
            "[ERROR] VIOLACIÓN: La configuración debe preservar valores existentes cuando merge=True."
        )
...
class TestPerfilAPI(SintelTenantTestCase):
    ...
    def test_b_consumir_endpoint_me_y_verificar_datos_combinados(self):
        ...
        self.assertEqual(
            ...
            200,
            f"[ERROR] VIOLACIÓN: El endpoint debe retornar 200 OK. Status recibido: {response.status_code}"
        )
        
        # Verificar que los datos del perfil están presentes
        data = response.json()
        self.assertIn('id', data, "[ERROR] VIOLACIÓN: La respuesta debe incluir el ID del perfil.")
        self.assertIn('cargo', data, "[ERROR] VIOLACIÓN: La respuesta debe incluir el cargo.")
        self.assertEqual(data['cargo'], 'Contador Senior', "[ERROR] VIOLACIÓN: El cargo debe ser el correcto.")
        self.assertEqual(data['departamento'], 'Finanzas', "[ERROR] VIOLACIÓN: El departamento debe ser el correcto.")
        self.assertEqual(data['telefono_corporativo'], '6012345678', "[ERROR] VIOLACIÓN: El teléfono debe ser el correcto.")
        
        # Verificar que los datos del User global están presentes (nested)
        self.assertIn('user_id', data, "[ERROR] VIOLACIÓN: La respuesta debe incluir user_id.")
        self.assertIn('user_email', data, "[ERROR] VIOLACIÓN: La respuesta debe incluir user_email.")
        self.assertIn('user_username', data, "[ERROR] VIOLACIÓN: La respuesta debe incluir user_username.")
        self.assertEqual(data['user_email'], self.user.email, "[ERROR] VIOLACIÓN: El email debe ser el del usuario actual.")
        self.assertEqual(data['user_username'], self.user.username, "[ERROR] VIOLACIÓN: El username debe ser el del usuario actual.")
    ...
    def test_b_actualizar_perfil_via_endpoint_me(self):
        ...
        data = response.json()
        self.assertEqual(data['cargo'], 'Contador Senior', "[ERROR] VIOLACIÓN: El cargo debe ser actualizado.")
        self.assertEqual(data['departamento'], 'Finanzas', "[ERROR] VIOLACIÓN: El departamento debe ser actualizado.")
        self.assertEqual(data['telefono_corporativo'], '6012345678', "[ERROR] VIOLACIÓN: El teléfono debe ser actualizado.")
        
        # Verificar en la base de datos
        perfil.refresh_from_db()
        self.assertEqual(perfil.cargo, 'Contador Senior', "[ERROR] VIOLACIÓN: El cargo debe estar actualizado en la BD.")
        self.assertEqual(perfil.departamento, 'Finanzas', "[ERROR] VIOLACIÓN: El departamento debe estar actualizado en la BD.")
...
class TestPerfilAislamiento(SintelTenantTestCase):
    ...
    def test_c_verificar_aislamiento_entre_tenants(self):
        ...
        self.assertNotEqual(
            perfil_a.id,
            perfil_b.id,
            "[ERROR] VIOLACIÓN DE AISLAMIENTO: Los perfiles en diferentes tenants deben tener IDs diferentes."
        )
        ...
        self.assertNotEqual(
            perfil_a.cargo,
            perfil_b.cargo,
            "[ERROR] VIOLACIÓN DE AISLAMIENTO: Los cargos deben ser diferentes en diferentes tenants."
        )
        self.assertNotEqual(
            perfil_a.departamento,
            perfil_b.departamento,
            "[ERROR] VIOLACIÓN DE AISLAMIENTO: Los departamentos deben ser diferentes en diferentes tenants."
        )
        self.assertNotEqual(
            perfil_a.telefono_corporativo,
            perfil_b.telefono_corporativo,
            "[ERROR] VIOLACIÓN DE AISLAMIENTO: Los teléfonos deben ser diferentes en diferentes tenants."
        )
        ...
        self.assertEqual(
            perfiles_en_a.count(),
            1,
            "[ERROR] VIOLACIÓN DE AISLAMIENTO: En Tenant A solo debe existir un perfil para este usuario."
        )
        self.assertEqual(
            perfiles_en_a.first().cargo,
            'Contador Senior',
            "[ERROR] VIOLACIÓN DE AISLAMIENTO: El perfil en Tenant A debe tener el cargo correcto."
        )
        ...
        self.assertEqual(
            perfiles_en_b.count(),
            1,
            "[ERROR] VIOLACIÓN DE AISLAMIENTO: En Tenant B solo debe existir un perfil para este usuario."
        )
        self.assertEqual(
            perfiles_en_b.first().cargo,
            'Auxiliar Administrativo',
            "[ERROR] VIOLACIÓN DE AISLAMIENTO: El perfil en Tenant B debe tener el cargo correcto."
        )
    ...
    def test_c_verificar_aislamiento_via_api(self):
        ...
        self.assertNotEqual(
            data_a['cargo'],
            data_b['cargo'],
            "[ERROR] VIOLACIÓN DE AISLAMIENTO: Los cargos deben ser diferentes en diferentes tenants (API)."
        )
        self.assertNotEqual(
            data_a['departamento'],
            data_b['departamento'],
            "[ERROR] VIOLACIÓN DE AISLAMIENTO: Los departamentos deben ser diferentes en diferentes tenants (API)."
        )
        ...
        self.assertEqual(
            data_a['user_email'],
            data_b['user_email'],
            "[ERROR] VIOLACIÓN: El email del usuario global debe ser el mismo en ambos tenants."
        )
