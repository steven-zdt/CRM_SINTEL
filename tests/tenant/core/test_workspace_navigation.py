...
class WorkspaceNavigationTest(SimpleTestCase):
    ...
    def test_empleados_link_in_sidebar(self):
        ...
        self.assertIn(
            'href="#empleados"', content, "Link de empleados no encontrado en sidebar"
        )
        self.assertIn(
            'data-view="empleados"',
            content,
            "Atributo data-view de empleados no encontrado",
        )
        self.assertIn(
            "👥 Empleados", content, "Texto del link de empleados no encontrado"
        )
    ...
    def test_gastos_link_in_sidebar(self):
        ...
        # Buscar el link en el sidebar
        self.assertIn(
            'href="#gastos"', content, "Link de gastos no encontrado en sidebar"
        )
        self.assertIn(
            'data-view="gastos"', content, "Atributo data-view de gastos no encontrado"
        )
    ...
    def test_empleados_section_exists(self):
        ...
        content = TEMPLATE_PATH.read_text(encoding="utf-8", errors="ignore")
        self.assertIn(
            'id="view-empleados"', content, "Sección view-empleados no encontrada"
        )
        self.assertIn(
            "tenant/empleados/partials/list.html",
            content,
            "Partial de list de empleados no incluido",
        )
    ...
    def test_gastos_section_exists(self):
        ...
        self.assertIn(
            "tenant/gastos/partials/list.html",
            content,
            "Partial de list de gastos no incluido",
        )
    ...
    def test_setActiveView_includes_empleados_and_gastos(self):
        ...
        self.assertIn(
            '"empleados"', sections_str, "empleados no está en la lista de secciones"
        )
        self.assertIn(
            '"gastos"', sections_str, "gastos no está en la lista de secciones"
        )
    ...
    def test_currentViewFromHash_includes_empleados_and_gastos(self):
        ...
        self.assertIn(
            '"empleados"', views_str, "empleados no está en la lista de views válidas"
        )
        self.assertIn(
            '"gastos"', views_str, "gastos no está en la lista de views válidas"
        )
    ...
    def test_js_files_included(self):
        ...
        content = TEMPLATE_PATH.read_text(encoding="utf-8", errors="ignore")
        self.assertIn(
            "tenant/empleados/devengos.page.js", content, "JS de devengos no incluido"
        )
        self.assertIn(
            "tenant/gastos/gastos.page.js", content, "JS de gastos no incluido"
        )
