...
class TestBootSequenceSmoke:
    ...
    def test_migrate_schemas_tenant_flag_no_argument(self):
        ...
        try:
            ...
            assert 'ignored explicit argument' not in error_msg.lower(), \
    ...
    def test_boot_sequence_logs_expected_messages(self):
        ...
        try:
            ...
            call_command('migrate_schemas', '--shared', '--fake-initial', verbosity=1, stdout=out)
            output = out.getvalue()
            ...
            out_tenant = StringIO()
            try:
                call_command('migrate_schemas', '--tenant', '--fake-initial', verbosity=1, stdout=out_tenant)
