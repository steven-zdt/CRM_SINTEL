...
⚠️ v2.37: Este test actúa como salvaguarda para prevenir cambios que rompan la funcionalidad.
...
class TestEmpresaIntegridad(TenantTestCase):
    ...
    def test_serializer_campos_minimos(self):
        ...
        campos_obligatorios = {'nit', 'direccion', 'telefono', 'email_contacto', 'regimen_tributario', 'moneda'}
        self.assertTrue(campos_obligatorios.issubset(set(data.keys())),
                       f"Faltan campos obligatorios. Campos presentes: {list(data.keys())}")
        
        # Campos PROHIBIDOS (no deben estar)
        campos_prohibidos = {'id', 'razon_social', 'logo', 'website', 'created_at', 'updated_at', 'dv'}
        campos_presentes_prohibidos = [f for f in campos_prohibidos if f in data]
        self.assertEqual(len(campos_presentes_prohibidos), 0,
                        f"Se expusieron campos prohibidos: {campos_presentes_prohibidos}")
        
        # Validar alias
        self.assertEqual(data['regimen_tributario'], 'ORDINARIO',
    ...
    def test_endpoint_formato_respuesta(self):
        ...
        data = response.json()
        self.assertIsInstance(data, list, "La respuesta debe ser un array")
        self.assertEqual(len(data), 1, "Con empresa, debe retornar array con 1 elemento")
    ...
    def test_html_columnas_exactas(self):
        ...
        self.assertEqual(len(th_elements), 7, 
        ...
        for i, expected in enumerate(expected_columns):
            self.assertIn(expected, th_elements[i], 
    ...
    def test_js_selector_coincide_html(self):
        ...
        self.assertIn('id="tabla-empresa"', content, 
                     "HTML debe tener id='tabla-empresa'")
        ...
        self.assertIn("TABLE_ID = '#tabla-empresa'", js_content or "const TABLE_ID = '#tabla-empresa'",
    ...
    def test_columnas_js_alineadas_serializer(self):
        ...
        columns_match = re.search(r'const columns\s*=\s*\[(.*?)\];', js_content, re.DOTALL)
        self.assertIsNotNone(columns_match, "No se encontró definición de 'columns' en JS")
        ...
        for campo in campos_serializer:
            ...
            self.assertTrue(re.search(pattern, columns_content, re.IGNORECASE),
    ...
    def test_modulos_independientes(self):
        ...
        self.assertEqual(mailinbox_count, 1,
