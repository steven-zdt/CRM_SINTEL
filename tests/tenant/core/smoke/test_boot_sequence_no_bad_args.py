...
⚠️ IMPORTANTE: Estos tests simulan la lógica de arranque sin ejecutar realmente
...
class TestBootSequenceNoBadArgs:
    ...
    def test_migrate_schemas_tenant_flag_no_stray_all(self):
        ...
        assert 'all' not in cmd_args, "El argumento 'all' no debe aparecer en la lista de argumentos"
        ...
        if tenant_index + 1 < len(cmd_args):
            assert cmd_args[tenant_index + 1] != 'all', "El siguiente argumento después de --tenant no debe ser 'all'"
