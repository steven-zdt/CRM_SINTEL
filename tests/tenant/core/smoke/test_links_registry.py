...
class TestLinksRegistry(SintelTenantTestCase):
    ...
    def test_links_registry_returns_200(self):
        ...
        response = self.client.get(
            "/api/v1/core/links/", HTTP_HOST=f"{self.tenant.schema_name}.sintel.net.co"
        )
    ...
    def test_links_registry_structure(self):
        ...
        for app in expected_apps:
            ...
            if app in data:
                self.assertIn(
                    "api", data[app], f"Link Registry para '{app}' debe incluir 'api'"
                )
        ...
        for app, links in data.items():
            if "api" in links:
                ...
                api_response = self.client.get(
                    api_url, HTTP_HOST=f"{self.tenant.schema_name}.sintel.net.co"
                )
