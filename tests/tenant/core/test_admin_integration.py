...
from apps.public.tenants.models import Client, Domain as PublicDomain
...
class TenantAdminIntegrationTests(TenantTestCase):
    ...
    def setup_tenant(cls, tenant_user=None):
        ...
        tenant = TenantClient.objects.create(
            schema_name='test_admin',
            nombre='Test Admin Tenant',
            is_active=True
        )
        Domain.objects.create(
            domain='test-admin.sintel.com',
            tenant=tenant,
            is_primary=True
        )
        return tenant
    ...
    def setup_domain(cls, domain):
        ...
        if not domain.domain or domain.domain == 'localhost':
            domain.domain = f"{domain.tenant.schema_name}.sintel.com"
    ...
    def setUp(self):
        ...
        self.client = Client(HTTP_HOST=f"{self.tenant.schema_name}.sintel.com")
    ...
    def test_admin_index_only_shows_tenant_models(self):
        ...
        ✅ Debe pasar: Solo modelos de TENANT_APPS deben aparecer
        """
        ...
        self.assertEqual(
            ...
            200,
            "El admin debe ser accesible para usuarios staff"
        )
        ...
        for model_name in public_model_names:
            self.assertNotIn(
                model_name,
                content,
                f"❌ VULNERABILIDAD: '{model_name}' aparece en el admin de tenant. "
    ...
    def test_admin_uses_tenant_admin_site(self):
        ...
        ✅ Debe pasar: El admin debe usar tenant_admin_site, no admin.site
        """
