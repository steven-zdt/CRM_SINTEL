...
al dominio público (sintel.net.co) o a un tenant privado (cliente.sintel.net.co) y sirva
...
class RoutingLogicTests(SintelTenantTestCase):
    ...
    [WARNING] NOTA: SintelTenantTestCase ya configura:
    ...
    def test_public_domain_loads_public_urlconf(self):
        """
        Escenario Público: Petición a sintel.net.co/ debe cargar urls_public.
        ...
        with schema_context(public_schema):
            ...
            try:
                ...
                public_client = TenantClient.objects.create(
                    schema_name=public_schema, nombre="SINTEL Global"
                )

            public_domain = Domain.objects.filter(
                tenant=public_client, is_primary=True
            ).first()

            if not public_domain:
                # Crear dominio si no existe
                public_domain = Domain.objects.create(
                    domain="sintel.net.co", tenant=public_client, is_primary=True
                )
    ...
    def test_private_tenant_anonymous_loads_landing_page(self):
        """
        Escenario Privado Anónimo: Petición a cliente.sintel.net.co/ con usuario anónimo
        ...
        with schema_context("public"):
            ...
            self.assertTrue(
                domain_exists,
                f"El dominio {domain_host} debe existir en el esquema public",
            )
    ...
    def test_private_tenant_authenticated_redirects_to_dashboard(self):
        """
        Escenario Privado Logueado: Petición a cliente.sintel.net.co/ con usuario autenticado
    ...
    def test_urlconf_separation_in_settings(self):
        ...
        [WARNING] NOTA: En TenantTestCase, Django cambia temporalmente ROOT_URLCONF,
