"""
[WARNING] FASE 6: Tests críticos de Aislamiento de Datos.
...
class DataIsolationCriticalTests(TenantTestCase):
    ...
    [WARNING] FASE 6: Validaciones mínimas pero críticas del aislamiento por esquema.
    """
    ...
    def setUp(self):
        ...
        self.empresa_a = Empresa.objects.create(
            razon_social="Empresa Tenant A", nit="900111111", dv="1"
        )
    ...
    def test_tenant_a_no_ve_datos_tenant_b(self):
        ...
        Domain.objects.create(
            domain="tenant-b.localhost", tenant=tenant_b, is_primary=True
        )

        # Crear datos en Tenant B
        with schema_context("test_tenant_b"):
            empresa_b = Empresa.objects.create(
                razon_social="Empresa Tenant B", nit="900333333", dv="1"
            )
        ...
        self.assertIsNone(
            factura_b_from_a, "Tenant A no debe poder acceder a facturas de Tenant B"
        )
    ...
    def test_queries_ejecutan_esquema_correcto(self):
        ...
        empresa = Empresa.objects.create(
            razon_social="Test Schema", nit="900555555", dv="1"
        )
    ...
    def test_acceso_directo_por_id_imposible(self):
        ...
        Domain.objects.create(
            domain="tenant-b-isolation.localhost", tenant=tenant_b, is_primary=True
        )
    ...
    def test_empresa_aislamiento_por_tenant(self):
        ...
        Domain.objects.create(
            domain="tenant-b-empresa.localhost", tenant=tenant_b, is_primary=True
        )

        # Crear Empresa en Tenant B
        with schema_context("test_tenant_b_empresa"):
            empresa_b = Empresa.objects.create(
                razon_social="Empresa Tenant B", nit="900888888", dv="1"
            )
