...
def test_hard_delete_bloquea_tenant_activo(active_tenant):
    ...
    assert (
        "is_active=False" in str(exc_info.value).lower()
        or "suspendido" in str(exc_info.value).lower()
    )
...
def test_hard_delete_api_bloquea_activo(api_client, admin_user, active_tenant):
    ...
    assert (
        "is_active=False" in str(response.data.get("error", "")).lower()
        or "suspendido" in str(response.data.get("error", "")).lower()
    )
...
def test_hard_delete_preserva_usuarios_globales(inactive_tenant, tenant_owner):
    ...
    assert not TenantMembership.objects.filter(
        client_id=tenant_id, user_id=user_id
    ).exists()
