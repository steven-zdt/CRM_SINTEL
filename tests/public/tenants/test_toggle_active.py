...
def test_tenant(db):
    ...
    return Client.objects.create(
        nombre="Test Tenant", schema_name="test_tenant", is_active=True, on_trial=True
    )
...
def test_toggle_active_requires_admin(api_client, admin_user, test_tenant):
    ...
    response = api_client.post(reverse("tenant-toggle-active", args=[test_tenant.id]))
...
def test_toggle_active_not_found(api_client, admin_user):
    ...
    response = api_client.post(reverse("tenant-toggle-active", args=[99999]))
