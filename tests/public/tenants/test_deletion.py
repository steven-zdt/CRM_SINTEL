...
class TestTenantDeletion:
    ...
    ⚠️ IMPORTANTE: Estos tests se ejecutan en el esquema público (public)
    ...
    def test_tenant_hard_delete(self):
        ...
        assert schema_exists(schema_name), f"El esquema '{schema_name}' no existe después de crear el tenant"
        ...
        assert not Client.objects.filter(schema_name=schema_name).exists(), "El Client aún existe"
        
        # 5. Verificar que el objeto Domain ya no existe (CASCADE)
        assert not Domain.objects.filter(domain=domain.domain).exists(), "El Domain aún existe"
        
        # 6. Verificar que la TenantMembership ya no existe (CASCADE)
        assert not TenantMembership.objects.filter(client=client).exists(), "La TenantMembership aún existe"
        
        # 7. Verificar que el esquema YA NO existe en PostgreSQL
        assert not schema_exists(schema_name), f"El esquema '{schema_name}' aún existe en PostgreSQL"
        
        # 8. Verificar que el usuario global NO fue eliminado (debe seguir existiendo)
        assert User.objects.filter(pk=owner.pk).exists(), "El usuario global fue eliminado (no debería)"
    ...
    def test_public_tenant_protection(self):
        ...
        try:
            ...
            pytest.skip(f"El tenant '{public_schema}' no existe. Ejecuta primero: python manage.py setup_public_tenant")
        ...
        assert "No se puede eliminar el tenant público" in error_message or "núcleo del sistema" in error_message, (
            f"El mensaje de error no es el esperado. Mensaje: {error_message}"
        )
        
        # Verificar que el tenant público sigue existiendo
        assert Client.objects.filter(schema_name=public_schema).exists(), "El tenant público fue eliminado (no debería)"
    ...
    def test_delete_cascades_to_domain(self):
        ...
        assert not Domain.objects.filter(pk=domain_id).exists(), "El Domain no fue eliminado por CASCADE"
        assert not Domain.objects.filter(domain=domain_domain).exists(), "El Domain no fue eliminado por CASCADE"
    ...
    def test_delete_cascades_to_membership(self):
        ...
        assert not TenantMembership.objects.filter(pk=membership_id).exists(), "La TenantMembership no fue eliminada por CASCADE"
    ...
    def test_delete_removes_schema_from_postgresql(self):
        ...
        assert not schema_exists(schema_name), f"El esquema '{schema_name}' aún existe después de eliminar el Client"
    ...
    def test_delete_preserves_global_users(self):
        ...
        assert User.objects.filter(pk=owner_id).exists(), "El usuario global fue eliminado (no debería)"
        
        # Verificar que el usuario puede ser usado en otro tenant
        owner2 = User.objects.get(pk=owner_id)
        assert owner2.email == 'owner_preserve@test.com', "El usuario global fue modificado"