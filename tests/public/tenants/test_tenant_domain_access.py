...
def test_routing_por_hostname_activa_tenant_urlconf(django_client, sample_tenant):
    ...
    response = django_client.get(
        "/",
        HTTP_HOST=domain.domain  # empresa-test.localhost
    )
...
def test_domain_sin_puerto_es_obligatorio():
    ...
    assert "puerto" in str(serializer.errors).lower() or "port" in str(serializer.errors).lower(), (
...
def test_normalize_domain_removes_port():
    ...
    test_cases = [
        ...
        ("cliente.sintel.com:8000", "cliente.sintel.com"),
...
def test_tenant_accessible_only_by_its_domain(django_client, sample_tenant):
    ...
    response_correcto = django_client.get(
        "/",
        HTTP_HOST=domain.domain
    )
    assert response_correcto.status_code in (200, 302), (
    ...
    assert response_incorrecto.status_code in (200, 302, 404), (
