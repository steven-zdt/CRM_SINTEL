...
def test_delete_suspended_tenant_drops_schema(api_client, staff_user, suspended_tenant_with_data):
    ...
    assert not Client.objects.filter(id=tenant.id).exists(), \
        "El tenant debería haber sido eliminado de la base de datos"
    
    # Verificar que el dominio fue eliminado (CASCADE)
    assert not Domain.objects.filter(id=suspended_tenant_with_data['domain'].id).exists(), \
        "El dominio debería haber sido eliminado por CASCADE"
    
    # Verificar que la membresía fue eliminada (CASCADE)
    assert not TenantMembership.objects.filter(id=suspended_tenant_with_data['membership'].id).exists(), \
        "La membresía debería haber sido eliminada por CASCADE"
    
    # El usuario global NO debe eliminarse (puede pertenecer a otros tenants)
    assert User.objects.filter(id=suspended_tenant_with_data['user'].id).exists(), \
        "El usuario global NO debe eliminarse (puede pertenecer a otros tenants)"
...
def test_delete_suspended_tenant_service_layer(suspended_tenant_with_data):
    ...
    assert not Client.objects.filter(id=tenant_id).exists(), \
        "El tenant debería haber sido eliminado"
    
    # Verificar que el dominio fue eliminado
    assert not Domain.objects.filter(id=suspended_tenant_with_data['domain'].id).exists(), \
        "El dominio debería haber sido eliminado"
    
    # Verificar que la membresía fue eliminada
    assert not TenantMembership.objects.filter(id=suspended_tenant_with_data['membership'].id).exists(), \
        "La membresía debería haber sido eliminada"
