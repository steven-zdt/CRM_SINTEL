...
def test_public_tenant_cannot_be_deleted_by_service(public_tenant):
    ...
    assert (
        "público" in str(exc_info.value).lower()
        or "public" in str(exc_info.value).lower()
    )
    assert (
        "no puede eliminarse" in str(exc_info.value).lower()
        or "indeletable" in str(exc_info.value).lower()
    )
...
def test_api_hard_delete_public_forbidden(api_client, admin_user, public_tenant):
    ...
    ), f"Expected 403, got {response.status_code}. Response: {response.data}"
...
def test_api_hard_delete_public_forbidden_even_if_inactive(
    api_client, admin_user, public_tenant
):
    """
