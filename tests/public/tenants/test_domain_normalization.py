...
class TestDomainNormalizationProtocolAndCase:
    ...
    def test_normalize_https_uppercase_with_slash(self, api_client):
        ...
        assert domain_in_db is not None, (
            "El dominio 'mi.empresa.com' debe existir en la base de datos"
        )
        assert domain_in_db.domain == "mi.empresa.com", (
...
class TestDomainNormalizationRegression:
    ...
    def test_onboard_with_valid_domain(self, api_client):
        ...
        assert client_data["nombre"] == "Empresa Regresión", (
        ...
        assert domain_data["domain"] == "empresa.test.com", (
...
class TestDomainNormalizationInvalid:
    ...
    def test_invalid_domain_with_spaces(self, api_client):
        ...
        has_domain_error = (
            "dominio" in error_data or
            any("dominio" in str(key).lower() for key in error_data.keys()) or
            any("fqdn" in str(value).lower() for value in error_data.values()) or
            any("válido" in str(value).lower() for value in error_data.values())
        )
