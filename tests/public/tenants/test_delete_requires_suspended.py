...
def test_cannot_delete_active_tenant(api_client, staff_user, active_tenant):
    ...
    data = response.json()
    assert (
        "is_active" in str(data.get("error", "")).lower()
        or "suspendido" in str(data.get("error", "")).lower()
...
def test_service_forbids_active_delete(active_tenant):
    ...
    assert (
        "is_active" in error_msg.lower() or "suspendido" in error_msg.lower()
...
def test_can_delete_suspended_tenant(api_client, staff_user, suspended_tenant):
    ...
    ).exists(), "El tenant suspendido debería haber sido eliminado"
