...
class TestAdminProvisioning:
    ...
    def test_admin_save_model_creates_membership(self):
        ...
        ), f"El rol debe ser 'ADMIN', pero es '{membership.rol}'"
    ...
    def test_form_requires_user_on_create(self):
        ...
        assert (
            is_valid is False
        ), "El formulario debería ser inválido sin admin_user en creación"
        ...
        expected_message = (
            "Debe asignar un usuario propietario al crear un nuevo tenant."
        )
    ...
    def test_edit_does_not_require_user(self):
        ...
        with schema_context(get_public_schema_name()):
            ...
            TenantMembership.objects.create(
                client=client, user=owner, rol="ADMIN", is_primary_admin=True
            )
    ...
    def test_save_model_updates_existing_membership(self):
        ...
        with schema_context(get_public_schema_name()):
            ...
            TenantMembership.objects.create(
                client=client, user=owner1, rol="ADMIN", is_primary_admin=True
            )
        ...
        admin_instance.save_model(request=request, obj=client, form=form, change=True)
        ...
        assert (
            membership.user == owner2
        ), "El usuario de la membresía debería ser owner2"
        ...
        owner1_membership = TenantMembership.objects.filter(
            client=client, user=owner1, is_primary_admin=True
        ).first()
