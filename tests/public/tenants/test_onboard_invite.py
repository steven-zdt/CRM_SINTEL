...
def test_onboard_creates_user_with_unusable_password(client_factory):
    ...
    # ⚠️ CRÍTICO: El usuario debe tener password unusable
    assert not user.has_usable_password(), "El usuario debe tener password unusable después del onboarding"
...
def test_invitation_token_has_ttl():
    ...
    token = generate_invitation_token(user_id, tenant_id, ttl_hours=1/3600)  # 1 segundo
...
def test_onboard_existing_user_with_password(client_factory):
    ...
    assert user.has_usable_password(), "El usuario existente debe mantener su password usable"
    ...
    assert "activation_url" in result, "Debe generarse activation_url incluso si el usuario ya tiene password"
...
def test_onboard_with_admin_user_id_skips_invitation(client_factory):
    ...
    assert "activation_url" not in result, "No debe generarse activation_url cuando se usa admin_user_id"
