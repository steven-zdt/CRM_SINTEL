...
def test_normalize_domain_removes_port():
    ...
    assert _normalize_domain_helper("sintel.com:8000") == "sintel.com"
    assert _normalize_domain_helper("sintel.com") == "sintel.com"
    assert _normalize_domain_helper("localhost:8000") == "localhost"
    assert _normalize_domain_helper("mi-empresa.localhost:3000") == "mi-empresa.localhost"
...
def test_console_tenants_status_page_renders(admin_client, admin_user):
    ...
    assert "task_id" in response.content.decode() or task_id in response.content.decode()
...
def test_onboard_async_creates_tenant_in_public_schema(admin_user):
    ...
    assert data["state"] == "SUCCESS", f"Estado esperado SUCCESS, obtenido: {data['state']}"
...
def test_onboard_async_normalizes_domain_in_result(admin_user):
    ...
    assert result["login_url"].endswith("/"), f"login_url debe terminar en '/', got '{result['login_url']}'"
    assert "/login" not in result["login_url"], f"login_url NO debe contener '/login', got '{result['login_url']}'"
