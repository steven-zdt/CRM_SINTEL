...
class TestDataTablesDomainValidation:
    ...
    def test_datatables_returns_correct_domain_from_db(self, admin_user):
        ...
        tenant = ClientFactory(
            nombre="Empresa Validación",
            schema_name="validacion_test",
            is_active=True
        )
        
        # Crear dominio específico en BD
        expected_domain = "validacion-test.sintel.com"
        ...
        assert domain_from_db.domain == expected_domain, \
    ...
    def test_datatables_handles_multiple_domains_correctly(self, admin_user):
        ...
        primary_domain = DomainFactory(
            tenant=tenant,
            domain="primary.sintel.com",
            is_primary=True
        )
        
        secondary_domain = DomainFactory(
            tenant=tenant,
            domain="secondary.sintel.com",
            is_primary=False
        )
        
        tertiary_domain = DomainFactory(
            tenant=tenant,
            domain="tertiary.sintel.com",
            is_primary=False
        )
        ...
        for row in data:
            if row[0] == tenant.id:
                ...
                assert row_domain == primary_domain.domain, \
                    ...
                    f"no '{row_domain}'"
                assert row_domain != secondary_domain.domain, \
                    "DataTables NO debe retornar el dominio secundario"
                assert row_domain != tertiary_domain.domain, \
