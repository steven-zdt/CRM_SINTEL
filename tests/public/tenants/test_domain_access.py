...
configurados (ej: {schema}.sintel.net.co) y que el esquema correcto está activo.
"""
...
class TestDomainAccess:
    ...
    def test_access_from_subdomain_returns_ok(self):
        ...
        # Crear tenant con dominio de sintel.net.co
        tenant = ClientFactory(
            nombre="Empresa Test Access", schema_name="test_access", is_active=True
        )

        # Crear dominio asociado
        domain = DomainFactory(
            tenant=tenant, domain="test-access.sintel.net.co", is_primary=True
        )

        # Verificar que el tenant y dominio fueron creados
        assert tenant.schema_name == "test_access"
        assert domain.domain == "test-access.sintel.net.co"
    ...
    def test_access_from_subdomain_uses_correct_schema(self):
        ...
        # Crear tenant con dominio de sintel.net.co
        tenant = ClientFactory(
            nombre="Empresa Schema Test", schema_name="schema_test", is_active=True
        )

        # Crear dominio asociado
        domain = DomainFactory(
            tenant=tenant, domain="schema-test.sintel.net.co", is_primary=True
        )
    ...
    def test_access_from_multiple_subdomains(self):
        ...
        tenants_data = [
            ("empresa1", "empresa1.sintel.net.co"),
            ("empresa2", "empresa2.sintel.net.co"),
            ("empresa3", "empresa3.sintel.net.co"),
        ]

        tenants = []
        for schema_name, domain_name in tenants_data:
            tenant = ClientFactory(
                nombre=f"Empresa {schema_name}", schema_name=schema_name, is_active=True
            )
            domain = DomainFactory(tenant=tenant, domain=domain_name, is_primary=True)
    ...
    def test_access_from_base_domain(self):
        """Test: Verificar acceso desde el dominio base (sintel.net.co)."""
        from apps.public.tenants.models import Client as TenantClient
        from apps.public.tenants.models import Domain
    ...
    def test_suspended_tenant_blocks_access_from_subdomain(self):
        ...
        # Crear dominio asociado
        domain = DomainFactory(
            tenant=tenant, domain="suspended-test.sintel.net.co", is_primary=True
        )
