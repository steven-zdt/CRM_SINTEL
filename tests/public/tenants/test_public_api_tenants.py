...
def test_public_tenants_list_ok(api_client, test_tenants):
    ...
    if response.status_code == status.HTTP_200_OK:
        ...
        forbidden_keys = (
            "memberships",
            "users",
            "tenantmembership_set",
            "tenant_memberships",
        )
        for key in forbidden_keys:
            assert (
                key not in sample
            ), f"El campo sensible '{key}' no debe estar en la respuesta pública"
...
def test_public_tenants_queryset_optimized(api_client, test_tenants):
    ...
    ), "El queryset debe estar optimizado"
    ...
    for lookup in prefetch_lookups:
        assert (
            "membership" not in str(lookup).lower() or "domain" in str(lookup).lower()
