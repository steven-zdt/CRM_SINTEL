...
from tests.public.tenants.factories import ClientFactory, UserFactory
...
class TestCrearTenant:
    ...
    def test_crear_tenant_success(self, admin_user):
        ...
        client, domain, login_url = crear_tenant(
            nombre=nombre, schema_name=schema_name, admin_user_id=admin_user.id
        )
    ...
    def test_crear_tenant_creates_schema(self, admin_user):
        ...
        client, domain, login_url = crear_tenant(
            nombre="Test Corp", schema_name=schema_name, admin_user_id=admin_user.id
        )
    ...
    def test_crear_tenant_normalizes_schema_name(self, admin_user):
        ...
        client, domain, login_url = crear_tenant(
            nombre="Test Corp", schema_name="  TESTCORP  ", admin_user_id=admin_user.id
        )
    ...
    def test_crear_tenant_duplicate_schema_name(self, admin_user):
        ...
        # Crear primer tenant
        crear_tenant(
            nombre="Test Corp 1", schema_name=schema_name, admin_user_id=admin_user.id
        )
    ...
    def test_crear_tenant_returns_login_url(self, admin_user):
        ...
        assert (
            "/admin/login" not in login_url
        ), f"login_url NO debe contener '/admin/login', got '{login_url}'"
...
class TestValidaciones:
    ...
    def test_schema_name_too_long(self, admin_user):
        ...
        with pytest.raises(ValidationError):
            crear_tenant(
                nombre="Test Corp", schema_name=long_schema, admin_user_id=admin_user.id
            )
