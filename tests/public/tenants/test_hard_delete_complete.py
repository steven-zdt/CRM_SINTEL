...
def suspended_tenant(db, staff_user):
    ...
    tenant = Client.objects.create(
        ...
        is_active=False,  # ⚠️ CRÍTICO: Debe estar suspendido
        on_trial=True
    )
    
    Domain.objects.create(
        tenant=tenant,
        domain="test-delete-me.localhost",
        is_primary=True
    )
...
def test_hard_delete_forbids_public_tenant(api_client, staff_user):
    ...
    public_tenant, _ = Client.objects.get_or_create(
        schema_name="public",
        defaults={"nombre": "Tenant Público", "is_active": True}
    )
...
def test_hard_delete_requires_suspended_tenant(api_client, staff_user):
    ...
    active_tenant = Client.objects.create(
        ...
        is_active=True,  # ⚠️ Activo (no puede eliminarse)
        on_trial=True
    )
...
@pytest.mark.django_db
def test_hard_delete_success_and_schema_dropped(api_client, staff_user, suspended_tenant):
    """
    ...
    assert schema_exists(schema_name), f"El schema '{schema_name}' debe existir antes de eliminarlo"
    ...
    assert not Client.objects.filter(id=tenant_id).exists(), "El Client debe eliminarse del esquema public"
    
    # Verificar que el Domain fue eliminado (CASCADE)
    assert not Domain.objects.filter(tenant_id=tenant_id).exists(), "El Domain debe eliminarse (CASCADE)"
    
    # Verificar que el TenantMembership fue eliminado (CASCADE)
    assert not TenantMembership.objects.filter(client_id=tenant_id).exists(), "El TenantMembership debe eliminarse (CASCADE)"
    
    # Verificar que el schema PostgreSQL fue eliminado
    assert not schema_exists(schema_name), f"El schema '{schema_name}' debe eliminarse (drop schema)"
