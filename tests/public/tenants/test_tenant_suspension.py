...
class TestTenantSuspensionRegression:
    ...
    def test_active_tenant_allows_access(self):
        ...
        domain = DomainFactory(
            tenant=tenant,
            domain="empresa-activa.localhost",
            is_primary=True
        )
...
class TestTenantSuspensionBlocking:
    ...
    def test_suspended_tenant_blocks_access(self):
        ...
        domain = DomainFactory(
            tenant=tenant,
            domain="empresa-suspendida.localhost",
            is_primary=True
        )
        ...
        assert "Servicio Suspendido" in response.content.decode('utf-8') or \
               "Service Suspended" in response.content.decode('utf-8'), \
...
class TestToggleStatusAPI:
    ...
    def test_toggle_status_activates_tenant(self, admin_user):
        ...
        assert 'activado' in data['message'].lower() or 'activated' in data['message'].lower()
    ...
    def test_toggle_status_suspends_tenant(self, admin_user):
        ...
        assert 'message' in data, "Debe incluir mensaje descriptivo"
        assert 'suspendido' in data['message'].lower() or 'suspended' in data['message'].lower()
    ...
    def test_toggle_status_prevents_public_tenant_deactivation(self, admin_user):
        ...
        assert 'público' in error_message.lower() or 'public' in error_message.lower(), \
        ...
        public_tenant.refresh_from_db()
        assert public_tenant.is_active is True, \
...
class TestTenantSuspensionIntegration:
    ...
    def test_full_suspension_reactivation_flow(self, admin_user):
        ...
        domain = DomainFactory(
            tenant=tenant,
            domain="empresa-integracion.localhost",
            is_primary=True
        )
        ...
        assert "Servicio Suspendido" in response3.content.decode('utf-8') or \
               "Service Suspended" in response3.content.decode('utf-8')
