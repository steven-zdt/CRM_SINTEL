...
def test_onboard_creates_user_with_valid_username():
    ...
    assert len(user.username.strip()) > 0, "El username no debe ser solo espacios en blanco"
    
    # Verificar que el username se generó desde el email
    assert "owner" in user.username.lower(), "El username debe contener el local-part del email"
    ...
    assert user.has_usable_password(), "El usuario debe tener una contraseña usable (hasheada)"
...
def test_onboard_username_collision_handling():
    ...
    assert user.username != "owner" or User.objects.filter(username="owner").count() == 1
