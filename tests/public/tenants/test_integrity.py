...
class TestTenantIntegrity:
    ...
    def test_service_rollback_on_membership_failure(self):
        ...
        assert (
            "no existe" in error_message.lower()
            or "no está activo" in error_message.lower()
        )

        # VERIFICACIÓN CRÍTICA: El Client NO debe existir
        assert not Client.objects.filter(
            schema_name=schema_name
        ).exists(), "[ERROR] CRÍTICO: El Client se creó a pesar del fallo en Membership"
        ...
        ).exists(), "El Domain se creó a pesar del fallo"
    ...
    def test_admin_creates_membership_atomically(self):
        ...
        client = Client(
            schema_name=schema_name, nombre=nombre, is_active=True, on_trial=True
        )
        ...
        ), "[ERROR] CRÍTICO: No se creó la TenantMembership desde Admin"
        assert membership.user == owner, "El usuario de la membresía no coincide"
        assert membership.rol == "ADMIN", "El rol debe ser 'ADMIN'"
    ...
    def test_no_orphan_tenants_allowed(self):
        ...
        assert (
            is_valid is False
        ), "El formulario debería rechazar creación sin admin_user"
        ...
        assert not Client.objects.filter(
            schema_name="test_orphan"
        ).exists(), "[ERROR] CRÍTICO: Se creó un tenant huérfano sin administrador"
    ...
    def test_membership_required_for_tenant_existence(self):
        ...
        assert (
            memberships.exists()
        ), "[ERROR] CRÍTICO: El tenant no tiene ninguna Membership"
        ...
        assert (
            primary_admins.exists()
        ), "[ERROR] CRÍTICO: El tenant no tiene Primary Admin"
