...
def test_create_user_password_mismatch(api_client, staff_user):
    ...
    data = response.json()
    assert "password2" in str(data) or "coinciden" in str(data).lower(), \
...
def test_create_user_duplicate_email(api_client, staff_user):
    ...
    data = response.json()
    assert "email" in str(data).lower() or "existente" in str(data).lower() or "duplicado" in str(data).lower(), \
