...
def test_dt_tenants_includes_primary_domain(tenant_with_domain):
    ...
    primary_domain_sq = Subquery(
        Domain.objects.filter(tenant=OuterRef('pk'), is_primary=True)
            .values('domain')[:1]
    )
    
    qs = (
        Client.objects
        .only("id", "nombre", "schema_name", "is_active")
        .annotate(primary_domain=primary_domain_sq)
    )
    
    tenant = qs.get(schema_name="acme")
    assert tenant.primary_domain == "acme.localhost", \
    ...
    assert "primary_domain" in serializer.data, "El serializer debe incluir 'primary_domain'"
    assert serializer.data["primary_domain"] == "acme.localhost", \
...
def test_dt_tenants_primary_domain_null_when_no_domain(tenant_without_domain):
    ...
    assert tenant.primary_domain is None or tenant.primary_domain == "", \
    ...
    serializer = TenantListSerializer(tenant)
    assert "primary_domain" in serializer.data, "El serializer debe incluir 'primary_domain'"
    assert serializer.data["primary_domain"] is None or serializer.data["primary_domain"] == "", \
...
def test_dt_tenants_search_by_domain(tenant_with_domain):
    ...
    assert found, "El tenant con dominio 'acme.localhost' debe aparecer en los resultados de búsqueda"
...
def test_dt_tenants_primary_domain_uses_primary_flag(db):
    ...
    Domain.objects.create(
        tenant=tenant,
        domain="secondary.multi-domain.localhost",
        is_primary=False
    )
    ...
    assert tenant_annotated.primary_domain == "multi-domain.localhost", \
    ...
    assert serializer.data["primary_domain"] == "multi-domain.localhost", \
