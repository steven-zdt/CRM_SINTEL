...
def enable_celery_always_eager(settings):
    """Enable eager Celery execution for these tests via pytest-django `settings` fixture."""
...
class TestTenantUICreationFlow:
    ...
    def test_get_new_tenant_form_renders_correctly(self, admin_client):
        ...
        # [OK] VALIDACIÓN CRÍTICA: Verificar que el formulario usa schema_name (NO subdomain)
    ...
    def test_create_tenant_successful_flow(self, admin_client, admin_user):
        ...
        # [OK] Verificar redirección (302) a status page
        assert response.status_code == 302
        assert "tenants/status" in response.url
        assert "task_id" in response.url

        # [OK] VALIDACIÓN CRÍTICA: Verificar integridad de datos en BD
        ...
        ), "Domain.tenant debe apuntar a Client (NO usar .client)"
        ...
        ), f"Domain esperado: {expected_domain}, obtenido: {domain.domain}"

        # 3. TenantMembership fue creado para el admin_user
        membership = TenantMembership.objects.filter(
            client=client, user=admin_user, rol="ADMIN"
        ).first()
    ...
    def test_create_tenant_duplicate_schema_name(self, admin_client, admin_user):
        ...
        Client.objects.create(
            schema_name=schema_name, nombre="Primera Empresa", auto_create_schema=True
        )
        ...
        # [OK] Verificar que NO se creó otro tenant
        assert Client.objects.filter(schema_name=schema_name).count() == 1

        # [OK] Verificar redirección con mensaje de error
        assert response.status_code == 302
    ...
    def test_create_tenant_domain_validation(self, admin_client, admin_user):
        ...
        existing_client = Client.objects.create(
            schema_name="existing", nombre="Empresa Existente", auto_create_schema=True
        )
        Domain.objects.create(
            domain="existing.example.com", tenant=existing_client, is_primary=True
        )
    ...
    def test_create_tenant_signal_creates_domain_automatically(
        ...
        assert (
            domain.tenant == client
        ), "Domain debe usar 'tenant' como FK (convención django-tenants)"
...
class TestTenantUIFormValidation:
    ...
    def test_form_requires_staff(self, client):
        ...
        user = User.objects.create_user(email="regular@test.local", password="test123")
