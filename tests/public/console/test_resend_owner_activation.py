...
def test_resend_activation_200_when_unusable_password(client, admin_user):
    ...
    with schema_context(get_public_schema_name()):
        ...
        owner = User.objects.create(email="owner@test.com", is_active=True)
    ...
    with patch(
        "apps.public.console.api.views.ResendOwnerActivationAPIView._send_email"
    ) as mock_send:
    ...
    with schema_context(get_public_schema_name()):
        log = ConsoleActionLog.objects.filter(
            action="RESEND_OWNER_ACTIVATION", tenant=tenant, target_user=owner
        ).first()
...
def test_resend_activation_404_owner_not_found(client, admin_user):
    ...
    data = response.json()
    assert (
        "owner" in data["detail"].lower() and "no encontrado" in data["detail"].lower()
    )
