...
def test_nueva_ingesta_programar_url_form_urlencoded(csrf_client, user_admin):
    ...
    assert doc.estado in ("EN_PROCESO", "PROCESADO", "RECIBIDO", "ERROR")  # descarga→ETL (Celery eager)
    assert IngestaLog.objects.filter(documento=doc).exists()
...
def test_consola_eliminar_ingesta(csrf_client, user_admin):
    ...
    # Crear documento
    from apps.public.impuestos.api.ingesta.serializers import DocumentoFuenteCreateSerializer
    serializer = DocumentoFuenteCreateSerializer(data={"fuente": "DIAN", "archivo": file_obj})
...
def test_consola_lista_ingesta_carga_correctamente(csrf_client, user_admin):
    ...
    assert b"DataTables" in resp.content or b"table" in resp.content  # Verificar que DataTables está incluido
