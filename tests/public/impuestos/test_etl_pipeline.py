...
try:
    ...
    pytest.skip("cryptography/CFFI not available in test environment; skipping ETL tests", allow_module_level=True)
...
class TestETLPipeline:
    ...
    def test_etl_csv_crea_normas(self, tmp_path):
        ...
        with open(p, "rb") as fh:
            doc.archivo.save("tarifas.csv", SimpleUploadedFile("tarifas.csv", fh.read()), save=False)
    ...
    def test_etl_atomicidad_en_error(self, tmp_path, monkeypatch):
        ...
        doc = DocumentoFuente.objects.create(fuente="DIAN", tipo="CSV")
        with open(p, "rb") as fh:
            doc.archivo.save("bad.csv", SimpleUploadedFile("bad.csv", fh.read()), save=False)
            doc.content_type = "text/csv"
            doc.extension = ".csv"
            doc.save()
    ...
    def test_etl_html_procesa_correctamente(self, tmp_path):
        ...
        # Crear HTML simple
        html_content = ("<html><h1>Artículo 1</h1><p>IVA exento para bienes básicos</p></html>").encode('utf-8')
        p = tmp_path / "norma.html"
        p.write_bytes(html_content)
        
        doc = DocumentoFuente.objects.create(fuente="DIAN", tipo="HTML")
        with open(p, "rb") as fh:
            doc.archivo.save("norma.html", SimpleUploadedFile("norma.html", fh.read()), save=False)
    ...
    def test_etl_retorna_stats(self, tmp_path):
        ...
        doc = DocumentoFuente.objects.create(fuente="DIAN", tipo="CSV")
        with open(p, "rb") as fh:
            doc.archivo.save("test.csv", SimpleUploadedFile("test.csv", fh.read()), save=False)
            doc.content_type = "text/csv"
            doc.extension = ".csv"
            doc.save()
