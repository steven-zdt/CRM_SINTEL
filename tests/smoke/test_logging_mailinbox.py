...
def test_mailinbox_logger_exists():
    ...
    assert len(logger.handlers) > 0, "Logger mailinbox.api debe tener al menos un handler"
    
    # Verificar que el nivel es INFO o superior
    assert logger.level <= logging.INFO, "Logger mailinbox.api debe estar en nivel INFO o inferior"
...
def test_mailinbox_logger_handlers():
    ...
    for handler in logger.handlers:
        ...
        assert hasattr(handler, 'emit'), f"Handler {type(handler)} debe tener método emit"
