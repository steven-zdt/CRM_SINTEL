...
⚠️ Usa modo eager (CELERY_TASK_ALWAYS_EAGER=True) para tests sin workers.
"""
import pytest
...
def test_fetch_and_process_billing_mail_eager(monkeypatch):
    ...
    def fake_upsert_factura_desde_ubl(file_or_text, naturaleza=None, xml_raw=None, dian_response_xml=None):
        nonlocal call_count
        call_count += 1
        ...
        if call_count == 2:
            raise DuplicateNumero("FAC-124", "La factura con número 'FAC-124' ya existe.")
