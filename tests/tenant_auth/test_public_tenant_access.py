...
[WARNING] IMPORTANTE: Estos tests previenen que el middleware bloquee incorrectamente
...
def private_tenant(db):
    ...
    Domain.objects.create(tenant=tenant, domain="cliente.localhost", is_primary=True)
...
def test_login_pages_do_not_loop(client, private_tenant):
    ...
    assert r1.status_code != 403, "La página de login NO debe recibir 403 Forbidden."
