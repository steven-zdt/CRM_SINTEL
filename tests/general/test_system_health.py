...
class TestSystemHealth(SintelTenantTestCase):
    ...
    def test_public_health(self):
        ...
        with schema_context(get_public_schema_name()):
            ...
            self.assertIsNotNone(primary_domain, "[ERROR] No existe dominio principal para el tenant público")
        ...
        try:
            ...
            print(f"[OK] API pública responde correctamente (status: {response.status_code})")
    ...
    def test_tenant_lifecycle(self):
        ...
        with schema_context(get_public_schema_name()):
            ...
            self.assertIsNotNone(created_domain, "[ERROR] No se creó el dominio principal")
        ...
        with schema_context(get_public_schema_name()):
            ...
            self.assertIsNotNone(membership, "[ERROR] No se creó la membresía de admin para el tenant")
            self.assertEqual(membership.rol, 'ADMIN', "[ERROR] El rol del admin no es 'ADMIN'")
            print(f"[OK] Membresía de admin creada: {admin_user.email} -> {client.nombre}")
    ...
    def test_private_access(self):
        ...
        with schema_context(schema_name):
            ...
            print(f"[OK] Empresa creada en tenant '{schema_name}': {empresa.razon_social} (ID: {empresa_id})")
        ...
        with schema_context(schema_name):
            ...
            print(f"[OK] Empresa encontrada en esquema del tenant: {empresa_found.razon_social}")
        ...
        with schema_context('other_tenant'):
            ...
            print(f"[OK] Aislamiento cross-tenant verificado: 0 empresas en 'other_tenant'")
