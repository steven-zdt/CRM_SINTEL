...
⚠️ IMPORTANTE: Estos tests deben ejecutarse DESPUÉS de la reconstrucción de la DB.
"""
import pytest
...
class TestSystemHealth(SintelTenantTestCase):
    ...
    def test_public_health(self):
        ...
        with schema_context(get_public_schema_name()):
            ...
            self.assertIsNotNone(public_tenant, "❌ El tenant 'public' no existe")
        ...
        with schema_context(get_public_schema_name()):
            ...
            self.assertIsNotNone(primary_domain, "❌ No existe dominio principal para el tenant público")
            print(f"✅ Dominio principal existe: {primary_domain.domain}")
        ...
        try:
            ...
            self.assertNotEqual(
                ...
                "❌ El servidor está devolviendo error 500 (Internal Server Error)"
            )
            print(f"✅ API pública responde correctamente (status: {response.status_code})")
        except Exception as e:
            # Si hay un error de conexión o similar, registrar pero no fallar
            print(f"⚠️  No se pudo verificar la API pública: {e}")
    ...
    def test_tenant_lifecycle(self):
        ...
        # ⚠️ v2.17: El dominio se construye automáticamente como {schema_name}.{TENANT_DOMAIN_BASE}
        ...
        try:
            ...
            self.fail(f"❌ Error al crear tenant: {e}")
        ...
        with schema_context(schema_name):
            ...
            self.assertEqual(
                ...
                f"❌ El esquema '{schema_name}' no existe o no se puede acceder"
            )
            print(f"✅ Esquema '{schema_name}' existe en PostgreSQL")
        ...
        with schema_context(get_public_schema_name()):
            ...
            self.assertIsNotNone(created_domain, "❌ No se creó el dominio principal")
            
            # ⚠️ v2.17: Verificar que el dominio es un subdominio
            from django.conf import settings
            expected_domain = f"{schema_name}.{settings.TENANT_DOMAIN_BASE}"
            self.assertEqual(
                created_domain.domain, expected_domain,
                f"❌ El dominio '{created_domain.domain}' no coincide con el esperado '{expected_domain}'"
            )
            print(f"✅ Dominio correcto (subdominio): {created_domain.domain}")
        ...
        with schema_context(get_public_schema_name()):
            ...
            self.assertIsNotNone(membership, "❌ No se creó la membresía de admin para el tenant")
            self.assertEqual(membership.rol, 'ADMIN', "❌ El rol del admin no es 'ADMIN'")
            print(f"✅ Membresía de admin creada: {admin_user.email} -> {client.nombre}")
    ...
    def test_private_access(self):
        ...
        with schema_context(schema_name):
            ...
            print(f"✅ Empresa creada en tenant '{schema_name}': {empresa.razon_social} (ID: {empresa_id})")
        ...
        self.assertEqual(
            ...
            f"❌ Se esperaba redirección 302, pero se recibió {response.status_code}"
        )
        print(f"✅ Acceso sin login redirige correctamente (302)")
        ...
        self.assertTrue(login_success, "❌ No se pudo loguear al usuario admin")
        print(f"✅ Usuario logueado: {admin_user.username}")
        ...
        print(f"✅ Acceso con login funciona correctamente (200 OK)")
        ...
        with schema_context(get_public_schema_name()):
            ...
            self.assertEqual(
                empresas_in_public, 0,
                f"❌ VIOLACIÓN DE AISLAMIENTO: Se encontraron {empresas_in_public} empresas en el esquema public"
            )
            print(f"✅ Aislamiento verificado: 0 empresas en esquema public")
        
        # Verificar que la empresa SÍ existe en el esquema del tenant
        with schema_context(schema_name):
            empresas_in_tenant = Empresa.objects.count()
            self.assertGreater(
                empresas_in_tenant, 0,
                "❌ La empresa no existe en el esquema del tenant"
            )
            empresa_found = Empresa.objects.get(id=empresa_id)
            self.assertEqual(empresa_found.razon_social, 'Empresa de Prueba')
            print(f"✅ Empresa encontrada en esquema del tenant: {empresa_found.razon_social}")
        ...
        with schema_context('other_tenant'):
            ...
            self.assertEqual(
                empresas_in_other, 0,
                f"❌ VIOLACIÓN DE AISLAMIENTO: Se encontraron {empresas_in_other} empresas en otro tenant"
            )
            print(f"✅ Aislamiento cross-tenant verificado: 0 empresas en 'other_tenant'")
        
        print("✅ Todas las pruebas de aislamiento pasaron correctamente")
    ...
    def test_subdomain_strictness(self):
        ...
        admin_user, _ = User.objects.get_or_create(
            ...
            defaults={
                'email': 'subdomain_test@sintel.com',
                'is_staff': True
            }
        )
        admin_user.set_password('test_password')
        ...
        self.assertEqual(
            ...
            f"❌ El dominio '{domain.domain}' no coincide con el formato esperado '{expected_domain}'"
        )
        print(f"✅ Dominio creado correctamente como subdominio: {domain.domain}")
        
        # Verificar que el dominio contiene el schema_name
        self.assertIn(
            schema_name, domain.domain,
            f"❌ El dominio '{domain.domain}' no contiene el schema_name '{schema_name}'"
        )
        
        # Verificar que el dominio contiene el dominio base
        self.assertIn(
            settings.TENANT_DOMAIN_BASE, domain.domain,
            f"❌ El dominio '{domain.domain}' no contiene el dominio base '{settings.TENANT_DOMAIN_BASE}'"
        )
        ...
        self.assertGreaterEqual(
            len(parts), 2,
            f"❌ El dominio '{domain.domain}' no tiene el formato correcto de subdominio"
        )
        
        print(f"✅ Validación de subdominios estrictos pasada")