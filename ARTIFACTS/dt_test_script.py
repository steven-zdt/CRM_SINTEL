...
if facturas_count == 0:
    print("[WARNING]  No hay facturas en la base de datos. El endpoint funcionará pero retornará 0 registros.")
...
try:
    ...
    if response.status_code == 200:
        ...
        if missing_keys:
            print(f"\n[ERROR] Faltan claves en la respuesta: {missing_keys}")
        else:
            print(f"\n[OK] Contrato DataTables correcto (todas las claves presentes)")
    ...
    print(f"[ERROR] Error ejecutando endpoint: {e}")
    import traceback
    traceback.print_exc()
