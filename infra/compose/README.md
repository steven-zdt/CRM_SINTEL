...
⚠️ **IMPORTANTE:** Siempre usar el archivo `docker-compose.yml` desde este directorio para evitar contenedores huérfanos:
...
**⚠️ Prevención de Contenedores Huérfanos:**
- Todos los servicios tienen `container_name` definido (ej: `crm_sintel-web`, `crm_sintel-db`)
- Esto previene que Docker genere nombres aleatorios como `busy_greider`
- Si encuentras contenedores con nombres aleatorios, deténlos y elimínalos:
  ```bash
  docker ps -a | grep -v crm_sintel
